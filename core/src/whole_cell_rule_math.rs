...
pub(crate) fn populate_base_rule_context(ctx: &mut RuleContext, inputs: BaseRuleContextInputs) {
    ...
    ctx.set(RuleSignal::ReplicatedFraction, inputs.replicated_fraction);
    ctx.set(
    ...
    ctx.set(RuleSignal::TranslationSupport, inputs.translation_support);
    ctx.set(RuleSignal::NucleotideSupport, inputs.nucleotide_support);
...
pub(crate) fn populate_process_rule_context(
    ...
    ctx.set(RuleSignal::AtpBandComplexes, inventory.atp_band_complexes);
    ctx.set(RuleSignal::RibosomeComplexes, inventory.ribosome_complexes);
    ctx.set(RuleSignal::RnapComplexes, inventory.rnap_complexes);
    ctx.set(
        RuleSignal::ReplisomeComplexes,
        inventory.replisome_complexes,
    );
    ctx.set(RuleSignal::MembraneComplexes, inventory.membrane_complexes);
    ctx.set(RuleSignal::FtszPolymer, inventory.ftsz_polymer);
    ctx.set(RuleSignal::DnaaActivity, inventory.dnaa_activity);
}

pub(crate) fn populate_stage_rule_context(ctx: &mut RuleContext, inputs: StageRuleContextInputs) {
    ctx.set(RuleSignal::EnergyCapacity, inputs.fluxes.energy_capacity);
    ctx.set(
    ...
    ctx.set(RuleSignal::ConstrictionSignal, inputs.constriction_signal);
    ctx.set(
