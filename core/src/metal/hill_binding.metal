...
constant float RESTING_NT[6]   = {20.0f, 10.0f, 15.0f, 50.0f, 200.0f, 500.0f};
constant float HALF_LIVES[6]   = {200.0f, 350.0f, 250.0f, 2.0f, 100.0f, 50.0f};
...
kernel void hill_binding(
    device       float* nt_conc      [[buffer(0)]],   // AoS: 6 floats per neuron
    ...
    float dt = max(params.dt, 0.0f);
    for (uint nt = 0; nt < NT_COUNT; ++nt) {
        float rest = RESTING_NT[nt];
        float decay_rate = LN2 / max(HALF_LIVES[nt], 1.0e-3f);
        float relax = clamp(1.0f - decay_rate * dt, 0.0f, 1.0f);
        float delta = nt_conc[base + nt] - rest;
        nt_conc[base + nt] = max(rest + delta * relax, 0.0f);
    }
}
