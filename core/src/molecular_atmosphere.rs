...
// odorant_channel_params() — REMOVED: all odorant params now derived
// by terrarium::emergent_rates::emergent_odorant_params() from molecular
// structure via Chapman-Enskog gas diffusion + OH radical decay kinetics.
...
mod tests {
    use super::{
        step_molecular_atmosphere_fields, step_molecular_world_fields,
        FruitSourceState, OdorantChannelParams, PlantSourceState, WaterSourceState,
    };
    use crate::terrarium::emergent_rates::emergent_odorant_params;

    /// Helper: get emergent odorant params at 20°C (matches default terrarium temperature).
    fn odorant_params(name: &str) -> Option<OdorantChannelParams> {
        emergent_odorant_params(name, 20.0)
    }

    #[test]
    fn o2_channel_params_exist() {
        let params = odorant_params("oxygen");
        ...
        // Also test "o2" alias
        assert!(odorant_params("o2").is_some());
    }
    ...
    fn odorant_transport_stays_bounded() {
        ...
        let params = vec![odorant_params("ethanol").unwrap()];
    ...
    fn water_sources_raise_humidity() {
        ...
        let total = width * height * depth;
        let mut odorants = vec![vec![0.0f32; total]];
        let params = vec![odorant_params("ammonia").unwrap()];
        let mut temperature = vec![22.0f32; total];
        let mut humidity = vec![0.4f32; total];
        let wind_x = vec![0.0f32; total];
        let wind_y = vec![0.0f32; total];
    ...
    fn world_step_updates_sources_and_wind() {
        ...
        let total = width * height * depth;
        let mut odorants = vec![vec![0.0f32; total]];
        let params: Vec<OdorantChannelParams> = vec![odorant_params("ethanol").unwrap()];
        let mut temperature = vec![22.0f32; total];
        let mut humidity = vec![0.4f32; total];
