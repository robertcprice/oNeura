...
use crate::terrarium::field::TerrariumSensoryField;
use crate::terrarium::substrate::{BatchedAtomTerrarium, TerrariumSpecies};
use crate::types::*;
...
fn step_molecular_atmosphere(py: Python<'_>, world: &Bound<'_, PyAny>, dt: f32) -> PyResult<()> {
    ...
    for (key, grid) in odorant_grids.iter() {
        let name = key.extract::<String>()?;
        let params = crate::terrarium::emergent_rates::emergent_odorant_params(&name, 20.0)
            .ok_or_else(|| {
...
impl PyTerrariumSensoryField {
    ...
    fn sample_fly<'py>(
        ...
        dict.set_item("left_odorant", sample.left_odorant)?;
        dict.set_item("right_odorant", sample.right_odorant)?;
...
impl PyDrosophilaSim {
    ...
    fn step_terrarium<'py>(
        ...
        reward_valence: f32,
        air_density: f32,
        left_odorant: Option<f32>,
        right_odorant: Option<f32>,
    ) -> PyResult<Bound<'py, PyDict>> {
        let report = self.inner.body_step_terrarium(
            odorant,
            left_odorant.unwrap_or(odorant),
            right_odorant.unwrap_or(odorant),
            left_light,
            right_light,
            temperature,
            sugar_taste,
...
impl PyWholeCellSimulator {
    ...
    #[staticmethod]
    fn compile_bundle_manifest_genome_asset_package_json(manifest_path: &str) -> PyResult<String> {
...
#[pymodule]
fn oneura_core(m: &Bound<'_, PyModule>) -> PyResult<()> {
