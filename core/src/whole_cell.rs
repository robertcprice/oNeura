...
use crate::gpu::whole_cell_rdme::{
    ...
    IntracellularSpatialState, IntracellularSpecies, WholeCellRdmeContext, WholeCellRdmeDriveField,
    WholeCellRdmeDriveState,
};
...
use crate::whole_cell_data::{
    ...
    WholeCellComplexAssemblyState, WholeCellComplexSpec, WholeCellContractSchema,
...
use crate::whole_cell_submodels::{
    ...
    WholeCellChemistryBridge, WholeCellDerivationCalibration, WholeCellDerivationCalibrationFit,
    WholeCellDerivationCalibrationSample, WholeCellSubsystemState,
};
...
impl Clone for WholeCellSimulator {
    fn clone(&self) -> Self {
        Self::new(WholeCellConfig::default())
    }
}
...
impl WholeCellSimulator {
    ...
    pub fn apply_environment_inputs(
        &mut self,
        inputs: &crate::terrarium::WholeCellEnvironmentInputs,
    ) {
        let blend = 0.22;
        let clamp_mm = |value: f32, fallback: f32| -> f32 {
            if value.is_finite() {
                value.max(0.0)
            } else {
                fallback.max(0.0)
        ...
        self.glucose_mm =
            self.glucose_mm * (1.0 - blend) + clamp_mm(inputs.glucose_mm, self.glucose_mm) * blend;
        self.oxygen_mm =
            self.oxygen_mm * (1.0 - blend) + clamp_mm(inputs.oxygen_mm, self.oxygen_mm) * blend;
        self.amino_acids_mm = self.amino_acids_mm * (1.0 - blend)
            + clamp_mm(inputs.amino_acids_mm, self.amino_acids_mm) * blend;
        self.nucleotides_mm = self.nucleotides_mm * (1.0 - blend)
            + clamp_mm(inputs.nucleotides_mm, self.nucleotides_mm) * blend;
        self.membrane_precursors_mm = self.membrane_precursors_mm * (1.0 - blend)
            + clamp_mm(inputs.membrane_precursors_mm, self.membrane_precursors_mm) * blend;

        let load = Self::finite_scale(inputs.metabolic_load, 0.4, 0.05, 3.0);
        self.metabolic_load = self.metabolic_load * (1.0 - blend) + load * blend;

        let resource_energy =
            (inputs.glucose_mm * 0.030 + inputs.oxygen_mm * 0.020 + inputs.amino_acids_mm * 0.010)
                .clamp(0.0, 0.28);
        let load_penalty = (inputs.metabolic_load * 0.018).clamp(0.0, 0.12);
        self.atp_mm = (self.atp_mm + resource_energy - load_penalty).clamp(0.02, 8.0);
        self.adp_mm = (self.adp_mm + load_penalty * 0.75 - resource_energy * 0.45).clamp(0.0, 8.0);

        self.chemistry_report = LocalChemistryReport {
            atp_support: Self::finite_scale(
                0.78 + self.atp_mm * 0.14 - self.metabolic_load * 0.08,
                1.0,
                0.65,
                1.50,
            ),
            translation_support: Self::finite_scale(
                0.74 + inputs.amino_acids_mm * 0.10 + inputs.glucose_mm * 0.04
                    - self.metabolic_load * 0.06,
                1.0,
                0.65,
                1.50,
            ),
            nucleotide_support: Self::finite_scale(
                0.76 + inputs.nucleotides_mm * 0.10 - self.metabolic_load * 0.05,
                1.0,
                0.65,
                1.50,
            ),
            membrane_support: Self::finite_scale(
                0.76 + inputs.membrane_precursors_mm * 0.10 + inputs.oxygen_mm * 0.03
                    - self.metabolic_load * 0.04,
                1.0,
                0.65,
                1.50,
            ),
            crowding_penalty: Self::finite_scale(
                1.0 + inputs.metabolic_load * 0.20 + inputs.proton_concentration * 1.0e4,
                1.0,
                0.65,
                1.20,
            ),
            mean_glucose: inputs.glucose_mm.max(0.0),
            mean_oxygen: inputs.oxygen_mm.max(0.0),
            mean_atp_flux: (resource_energy * 6.0).clamp(0.0, 2.0),
            mean_carbon_dioxide: (inputs.metabolic_load * 0.18).clamp(0.0, 2.0),
        };
    }

    pub fn pending_division(&self) -> bool {
        self.division_progress >= 0.985
            && self.atp_mm >= 0.70
            && self.glucose_mm >= 0.35
            && self.oxygen_mm >= 0.28
    }

    pub fn environment_dt_ms(&self) -> f32 {
        0.1
    }

    pub fn split_into_daughters(&self) -> (Self, Self) {
        let restore = || {
            self.save_state_json()
                .ok()
                .and_then(|json| Self::from_saved_state_json(&json).ok())
                .unwrap_or_else(Self::new_default)
        };
        let mut left = restore();
        let mut right = restore();

        for daughter in [&mut left, &mut right] {
            daughter.division_progress = 0.0;
            daughter.ftsz *= 0.55;
            daughter.dnaa *= 0.55;
            daughter.adp_mm *= 0.60;
            daughter.atp_mm *= 0.58;
            daughter.glucose_mm *= 0.55;
            daughter.oxygen_mm *= 0.55;
            daughter.amino_acids_mm *= 0.55;
            daughter.nucleotides_mm *= 0.55;
            daughter.membrane_precursors_mm *= 0.55;
            daughter.radius_nm = (daughter.radius_nm * 0.82).max(120.0);
            daughter.surface_area_nm2 = Self::surface_area_from_radius(daughter.radius_nm);
            daughter.volume_nm3 = Self::volume_from_radius(daughter.radius_nm);
            daughter.chromosome_separation_nm *= 0.35;
        }
    ...
    fn has_explicit_expression_state(&self) -> bool {
    ...
    fn current_diagnostic_pool_summaries(&self) -> (f32, f32, f32, f32) {
        if self.organism_assets.is_some()
            || !self.named_complexes.is_empty()
            || self.complex_assembly.total_complexes() > 1.0e-6
        {
    ...
    fn normalize_runtime_species_bulk_fields(&mut self) {
        ...
        for species in &mut self.organism_species {
            ...
            if let Some(bulk_field) = registry_bulk_fields.get(&species.id).copied().flatten() {
    ...
    fn process_fluxes(&self, inventory: WholeCellAssemblyInventory) -> WholeCellProcessFluxes {
        let base = self.base_rule_context(0.0);
        let localized_supply = base.get(WholeCellRuleSignal::LocalizedSupplyScale);
        let oxygen_signal = base.get(WholeCellRuleSignal::OxygenSignal);
        let glucose_signal = base.get(WholeCellRuleSignal::GlucoseSignal);
        let amino_signal = base.get(WholeCellRuleSignal::AminoSignal);
        let nucleotide_signal = base.get(WholeCellRuleSignal::NucleotideSignal);
        let membrane_signal = base.get(WholeCellRuleSignal::MembraneSignal);
        ...
        let md_membrane_scale = base.get(WholeCellRuleSignal::MdMembraneScale);
        let atp_support = base.get(WholeCellRuleSignal::AtpSupport);
        let translation_support = base.get(WholeCellRuleSignal::TranslationSupport);
        let nucleotide_support = base.get(WholeCellRuleSignal::NucleotideSupport);
        let membrane_support = base.get(WholeCellRuleSignal::MembraneSupport);

        WholeCellProcessFluxes {
            energy_capacity: ((1.0 / 24.0)
                * inventory.atp_band_complexes.max(0.0)
                * atp_support
                * atp_band_scale
                * localized_supply
                * (0.55 + 0.45 * oxygen_signal)
                * (0.55 + 0.45 * glucose_signal))
                .clamp(0.40, 2.40),
            transcription_capacity: ((1.0 / 24.0)
                * inventory.rnap_complexes.max(0.0)
                * nucleotide_support
                * localized_supply
                * (0.55 + 0.45 * energy_signal))
                .clamp(0.35, 2.40),
            translation_capacity: ((1.0 / 46.0)
                * inventory.ribosome_complexes.max(0.0)
                * translation_support
                * ribosome_translation_scale
                * md_translation_scale
                * localized_supply
                * (0.55 + 0.45 * amino_signal))
                .clamp(0.35, 2.60),
            replication_capacity: ((1.0 / 28.0)
                * inventory.replisome_complexes.max(0.0)
                * replisome_replication_scale
                * nucleotide_support
                * (0.55 + 0.45 * nucleotide_signal))
                .clamp(0.30, 2.40),
            ...
            membrane_capacity: ((1.0 / 24.0)
                * inventory.membrane_complexes.max(0.0)
                * membrane_support
                * membrane_assembly_scale
                * md_membrane_scale
                * (0.55 + 0.45 * membrane_signal))
                .clamp(0.35, 2.40),
            constriction_capacity: ((1.0 / 90.0)
                * inventory.ftsz_polymer.max(0.0)
                * ftsz_constriction_scale
                * membrane_support
                * md_translation_scale
                * (0.55 + 0.45 * membrane_signal))
                .clamp(0.30, 2.60),
    ...
    /// Rebuild a minimal explicit expression layer from persisted runtime
    /// species and reactions when bundle descriptors are unavailable. This keeps
    /// bundle-less restore on the explicit-state path instead of collapsing back
    /// to empty default summaries.
    fn synthesize_expression_state_from_runtime_process(
        &self,
    ) -> Option<WholeCellOrganismExpressionState> {
        #[derive(Default)]
        struct OperonRuntimeSummary {
        ...
        let mut summaries: HashMap<String, OperonRuntimeSummary> = HashMap::new();
        let has_runtime_species = !self.organism_species.is_empty();
        let has_runtime_reactions = !self.organism_reactions.is_empty();
        ...
        } else if let Some(registry) = self.organism_process_registry() {
        ...
        let mut transcription_units = Vec::with_capacity(summaries.len());
        ...
        for (operon, summary) in entries {
            ...
            let unit_weights = if summary.process_signal.total() > 1.0e-6 {
                summary.process_signal
            } else {
                Self::asset_class_process_template(WholeCellAssetClass::Generic)
            };
            ...
            let stress_penalty = Self::finite_scale(
            ...
            let effective_activity = Self::finite_scale(
            ...
            let mut unit = WholeCellTranscriptionUnitState {
                name: operon,
                gene_count,
                copy_gain: 1.0,
                basal_activity,
                ...
                transcription_progress_nt: promoter_open_fraction * transcription_length_nt,
                nascent_transcript_abundance,
                mature_transcript_abundance,
                damaged_transcript_abundance,
                active_ribosome_occupancy,
                translation_length_aa,
                translation_progress_aa: promoter_open_fraction * translation_length_aa,
                nascent_protein_abundance,
                mature_protein_abundance,
                damaged_protein_abundance,
                process_drive: unit_weights.clamped(),
            };
            Self::normalize_expression_execution_state(&mut unit);
            transcription_units.push(unit);
        }
        ...
        Some(WholeCellOrganismExpressionState {
            ...
            process_scales: WholeCellProcessWeights {
                energy: Self::process_scale(process_signal.energy, process_mean),
                transcription: Self::process_scale(process_signal.transcription, process_mean),
                translation: Self::process_scale(process_signal.translation, process_mean),
                replication: Self::process_scale(process_signal.replication, process_mean),
                segregation: Self::process_scale(process_signal.segregation, process_mean),
                membrane: Self::process_scale(process_signal.membrane, process_mean),
                constriction: Self::process_scale(process_signal.constriction, process_mean),
            },
    ...
    fn lattice_species_for_bulk_field(field: WholeCellBulkField) -> Option<IntracellularSpecies> {
        match field {
            ...
            | WholeCellBulkField::NadOxidized
            | WholeCellBulkField::NadReduced
            | WholeCellBulkField::CoA => None,
        }
    }
    ...
    fn localized_pool_transfer_hint(
        ...
        let (demand, support, base, max_scale) = match field {
            ...
            | WholeCellBulkField::NadOxidized
            | WholeCellBulkField::NadReduced
            | WholeCellBulkField::CoA => (0.0, 0.0, 0.10, 1.8),
        };
        Self::finite_scale(
    ...
    fn aggregate_named_complex_assembly_state_without_assets(
        &self,
    ) -> WholeCellComplexAssemblyState {
        let mut aggregate = WholeCellComplexAssemblyState::default();
    ...
    fn named_complex_inventory_shares_from_state(
        &self,
    ...
    pub fn organism_expression_state(&self) -> Option<WholeCellOrganismExpressionState> {
        if self.has_explicit_expression_state() || self.organism_data.is_some() {
    ...
    /// Reactions auto-discovered as quantum-eligible.
    pub fn discovered_quantum_reactions(&self) -> &[QuantumDiscoveredReaction] {
        &self.discovered_quantum_reactions
    }

    fn run_quantum_auto_discovery(&mut self) {
        self.discovered_quantum_reactions.clear();
        for reaction in &self.organism_reactions {
            let channel = match reaction.asset_class {
                WholeCellAssetClass::Energy => Some(QuantumProfileChannel::Oxphos),
                _ => Self::channel_from_subsystem_targets(&reaction.subsystem_targets),
            };
            ...
            if let Some(ch) = channel {
                if eligible {
                    self.discovered_quantum_reactions
                        .push(QuantumDiscoveredReaction {
    ...
    fn channel_from_subsystem_targets(
        targets: &[Syn3ASubsystemPreset],
    ) -> Option<QuantumProfileChannel> {
        for t in targets {
            match t {
                Syn3ASubsystemPreset::AtpSynthaseMembraneBand => {
                    return Some(QuantumProfileChannel::Oxphos)
                }
                Syn3ASubsystemPreset::RibosomePolysomeCluster => {
                    return Some(QuantumProfileChannel::Translation)
                }
                Syn3ASubsystemPreset::ReplisomeTrack => {
                    return Some(QuantumProfileChannel::NucleotidePolymerization)
                }
                Syn3ASubsystemPreset::FtsZSeptumRing => {
                    return Some(QuantumProfileChannel::ChromosomeSegregation)
                }
            }
        }
        None
    ...
    pub(super) fn refine_quantum_corrections_from_probe(&mut self, probe: &LocalMDProbeReport) {
        const ALPHA: f32 = 0.12;
        let th = probe.thermal_stability.clamp(0.0, 2.0);
        let st = probe.structural_order.clamp(0.0, 2.0);
        let el = probe.electrostatic_order.clamp(0.0, 2.0);
        let corrections = [
            (0.40 * th + 0.35 * probe.recommended_atp_scale + 0.25 * el).clamp(0.5, 2.5),
            (0.45 * st + 0.30 * probe.recommended_translation_scale + 0.25 * th).clamp(0.5, 2.5),
            (0.40 * el + 0.35 * probe.recommended_replication_scale + 0.25 * st).clamp(0.5, 2.5),
            (0.45 * probe.recommended_membrane_scale + 0.30 * th + 0.25 * st).clamp(0.5, 2.5),
            (0.40 * probe.recommended_segregation_scale
                + 0.35 * st
                + 0.25 * probe.recommended_constriction_scale)
                .clamp(0.5, 2.5),
        ];
        let blend = |cur: f32, c: f32| ((1.0 - ALPHA) * cur + ALPHA * c).clamp(0.5, 2.5);
        self.quantum_profile.oxphos_efficiency =
            blend(self.quantum_profile.oxphos_efficiency, corrections[0]);
        self.quantum_profile.translation_efficiency =
            blend(self.quantum_profile.translation_efficiency, corrections[1]);
        self.quantum_profile.nucleotide_polymerization_efficiency = blend(
            self.quantum_profile.nucleotide_polymerization_efficiency,
            corrections[2],
        );
        self.quantum_profile.membrane_synthesis_efficiency = blend(
            self.quantum_profile.membrane_synthesis_efficiency,
            corrections[3],
        );
        self.quantum_profile.chromosome_segregation_efficiency = blend(
            self.quantum_profile.chromosome_segregation_efficiency,
            corrections[4],
        );
        let channel_corrections = [
            QuantumProfileChannel::Oxphos,
            QuantumProfileChannel::Translation,
            QuantumProfileChannel::NucleotidePolymerization,
            QuantumProfileChannel::MembraneSynthesis,
            QuantumProfileChannel::ChromosomeSegregation,
        ];
        for reaction in &mut self.discovered_quantum_reactions {
            let idx = channel_corrections
                .iter()
                .position(|&c| c == reaction.profile_channel)
                .unwrap_or(0);
            reaction.correction_factor = ((1.0 - ALPHA) * reaction.correction_factor
                + ALPHA * corrections[idx])
                .clamp(0.5, 2.5);
            reaction.probe_refined = true;
        }
        self.apply_quantum_discovery_to_profile();
    }

    fn apply_quantum_discovery_to_profile(&mut self) {
        use QuantumProfileChannel::*;
        for &ch in &[
            Oxphos,
            Translation,
            NucleotidePolymerization,
            ...
            let factors: Vec<f32> = self
                ...
                .iter()
                .filter(|r| r.profile_channel == ch)
                .map(|r| r.correction_factor.clamp(0.5, 2.5))
                .collect();
            if factors.is_empty() {
                continue;
            }
            let geo = (factors.iter().map(|f| f.ln()).sum::<f32>() / factors.len() as f32)
                .exp()
                .clamp(0.5, 2.5);
            match ch {
                Oxphos => self.quantum_profile.oxphos_efficiency = geo,
                Translation => self.quantum_profile.translation_efficiency = geo,
                NucleotidePolymerization => {
                    self.quantum_profile.nucleotide_polymerization_efficiency = geo
                }
                MembraneSynthesis => self.quantum_profile.membrane_synthesis_efficiency = geo,
                ChromosomeSegregation => {
                    self.quantum_profile.chromosome_segregation_efficiency = geo
                }
            }
        }
    }

    fn observe(&self, kind: ObservableKind) -> f32 {
        match kind {
            ObservableKind::AtpMm => self.atp_mm,
            ObservableKind::AminoAcidsMm => self.amino_acids_mm,
            ObservableKind::NucleotidesMm => self.nucleotides_mm,
            ObservableKind::MembranePrecursorsMm => self.membrane_precursors_mm,
            ObservableKind::GlucoseMm => self.glucose_mm,
            ObservableKind::OxygenMm => self.oxygen_mm,
            ObservableKind::ActiveRibosomes => self.active_ribosomes,
            ObservableKind::ActiveRnap => self.active_rnap,
            ObservableKind::RadiusNm => self.radius_nm,
            ObservableKind::SurfaceAreaNm2 => self.surface_area_nm2,
            ObservableKind::VolumeNm3 => self.volume_nm3,
            ObservableKind::DivisionProgress => self.division_progress,
            ObservableKind::GrowthRatePerMs => {
                let vol = self.volume_nm3.max(1.0);
                let dt = self.config.dt_ms.max(0.001);
                (vol.ln() / (self.step_count.max(1) as f32 * dt)).max(0.0)
            }
        }
    }

    fn observable_loss(&self, targets: &[ObservableTarget]) -> (f32, Vec<f32>) {
        let mut total = 0.0f32;
        let mut per = Vec::with_capacity(targets.len());
        for t in targets {
            let err = (self.observe(t.kind) - t.target_value) / t.tolerance.max(1e-6);
            let sq = err * err;
            per.push(sq);
            total += t.weight * sq;
        }
    ...
    /// Run observable-driven auto-calibration.
    /// Run observable-driven auto-calibration of the quantum profile.
    ///
    /// Optimizes the 5 quantum efficiency parameters using coordinate descent.
    /// For each candidate, creates a fresh simulator with the same config,
    /// runs `eval_steps` steps, and measures weighted error against targets.
    pub fn calibrate_from_observables(
        config: &WholeCellConfig,
        targets: &[ObservableTarget],
        eval_steps: usize,
    ) -> ObservableCalibrationResult {
        let step_scales = [0.20f32, 0.10, 0.05, 0.02];
        let eff_min = 0.5f32;
        let eff_max = 2.5f32;

        let profile_get = |p: &WholeCellQuantumProfile, i: usize| -> f32 {
            match i {
                0 => p.oxphos_efficiency,
                1 => p.translation_efficiency,
                2 => p.nucleotide_polymerization_efficiency,
                3 => p.membrane_synthesis_efficiency,
                4 => p.chromosome_segregation_efficiency,
                _ => 1.0,
            }
        };
        let profile_set = |p: &mut WholeCellQuantumProfile, i: usize, v: f32| match i {
            0 => p.oxphos_efficiency = v,
            1 => p.translation_efficiency = v,
            2 => p.nucleotide_polymerization_efficiency = v,
            3 => p.membrane_synthesis_efficiency = v,
            4 => p.chromosome_segregation_efficiency = v,
            _ => {}
        };

        let evaluate = |profile: WholeCellQuantumProfile| -> f32 {
            let mut trial = Self::new(config.clone());
            trial.quantum_profile = profile;
            for _ in 0..eval_steps {
                trial.step();
            }
            let (loss, _) = trial.observable_loss(targets);
            if loss.is_finite() {
                loss
            } else {
                1e6
        ...
        let initial_profile = WholeCellQuantumProfile::default();
        let initial_loss = evaluate(initial_profile);
        ...
        for &ss in &step_scales {
            ...
            while improved {
                improved = false;
                for pi in 0..5usize {
                    let cv = profile_get(&best_profile, pi);
                    let step = ss * cv.abs().max(0.1);
                    for &dir in &[-1.0f32, 1.0] {
                        let nv = (cv + dir * step).clamp(eff_min, eff_max);
                        if (nv - cv).abs() < 1e-6 {
                            continue;
                        }
                        let mut cand = best_profile;
                        profile_set(&mut cand, pi, nv);
                        let loss = evaluate(cand);
        ...
        let mut final_sim = Self::new(config.clone());
        final_sim.quantum_profile = best_profile;
        for _ in 0..eval_steps {
            final_sim.step();
        }
        let (final_loss, per_target_error) = final_sim.observable_loss(targets);

        ObservableCalibrationResult {
            targets: targets.to_vec(),
            initial_loss,
            final_loss,
            iterations: step_scales.len(),
            per_target_error,
            calibration: default_whole_cell_derivation_calibration(),
        }
    }
