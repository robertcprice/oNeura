...
//!
//! ```rust
//! use oneura_core::metabolic_flux::{MetabolicNetwork, FbaStatus};
//!
...
impl MetabolicNetwork {
    ...
    pub fn essential_reactions(&self, objective_reaction: usize, threshold: f64) -> Vec<usize> {
        let fba_result = self.fba(objective_reaction, true);
        if fba_result.status != FbaStatus::Optimal {
            return Vec::new();
        }
...
fn solve_lp(a: &[Vec<f64>], m: usize, n: usize, c: &[f64], lb: &[f64], ub: &[f64]) -> LpResult {
    // -----------------------------------------------------------------------
    // Validate bounds.
    // -----------------------------------------------------------------------
    for j in 0..n {
...
pub fn e_coli_core() -> MetabolicNetwork {
    ...
    net.add_reaction(
        ...
        8.39,
        1000.0,
        false, // minimum maintenance flux
    ); // 21

    // -- Exchange reactions --
    // 23. EX_O2: O2_ext -> (unlimited uptake)
    net.add_reaction("EX_O2".into(), s(&[(o2_e, -1.0)]), -1000.0, 0.0, false); // 22 -- note: negative = uptake convention
    ...
    net.add_reaction("EX_NH4".into(), s(&[(nh4_e, -1.0)]), -1000.0, 0.0, false); // 24
...
pub fn soil_microbe_generic() -> MetabolicNetwork {
    ...
    // 12. ATP maintenance
    net.add_reaction("ATPM".into(), s(&[(atp, -1.0)]), 5.0, 1000.0, false); // 11

    // 13. Exchange: O2 uptake
    net.add_reaction("EX_O2".into(), s(&[(o2_e, -1.0)]), -1000.0, 0.0, false); // 12

    // 14. Exchange: CO2 export (handled via TCA/PDH directly to external)
    // Not needed since CO2 is external.

    // 15. Exchange: N2 uptake
    net.add_reaction("EX_N2".into(), s(&[(n2_e, -1.0)]), -1000.0, 0.0, false); // 13
...
mod tests {
    ...
    fn simple_fba_feasible() {
        ...
        assert!(
            (result.objective_value - 10.0).abs() < 0.01,
            "Expected ~10.0, got {}",
    ...
    fn e_coli_core_runs() {
        ...
        assert!(
            net.metabolites.len() >= 20,
            "Expected >= 20 metabolites, got {}",
            net.metabolites.len()
        );
        assert!(
            net.reactions.len() >= 25,
            "Expected >= 25 reactions, got {}",
            net.reactions.len()
        );
        ...
        assert_eq!(
            result.status,
            FbaStatus::Optimal,
            "E. coli FBA failed with status {:?}",
            result.status
        );
        assert!(
            result.objective_value > 0.0,
            "Expected positive biomass, got {}",
            result.objective_value
        );
    ...
    fn soil_microbe_runs() {
        ...
        assert_eq!(
            result.status,
            FbaStatus::Optimal,
            "Soil microbe FBA failed with status {:?}",
            result.status
        );
        assert!(
            result.objective_value > 0.0,
    ...
    fn dead_end_metabolites_found() {
        ...
        assert!(
            dead_ends.contains(&a),
            "A should be a dead end (only consumed)"
        );
        assert!(
            dead_ends.contains(&c),
            "C should be a dead end (only produced)"
        );
        assert!(
            !dead_ends.contains(&b),
            "B should NOT be a dead end (both consumed and produced)"
        );
    }
    ...
    fn flux_variability_bounds() {
        ...
        assert!(
            ranges[0].0 >= -0.01,
            "Uptake min should be >= 0, got {}",
            ranges[0].0
        );
        assert!(
            ranges[0].1 <= 10.01,
            "Uptake max should be <= 10, got {}",
            ranges[0].1
        );
    ...
    fn infeasible_network_detected() {
        ...
        assert_eq!(
            result.status,
            FbaStatus::Infeasible,
            "Expected infeasible but got {:?} with obj={}",
            result.status,
            result.objective_value
        );
    }
    ...
    fn reversible_reactions_handle_negative_flux() {
        ...
        assert!(
            (max_result.objective_value - 10.0).abs() < 0.01,
            "Expected max ~10.0, got {}",
            max_result.objective_value
        );
        ...
        assert!(
            (min_result.objective_value - (-10.0)).abs() < 0.01,
            "Expected min ~-10.0, got {}",
            min_result.objective_value
        );
    }
    ...
    fn branched_pathway_fba() {
        ...
        assert!(
            (result.objective_value - 20.0).abs() < 0.1,
            "Expected ~20.0, got {}",
            result.objective_value
        );
    }
    ...
    fn atp_coupled_pathway() {
        ...
        net.add_reaction(
            "ferment".into(),
            s(&[(b, -1.0), (c_ext, 1.0), (atp, 2.0)]),
            0.0,
            1000.0,
            false,
        );
        let r_bio = net.add_reaction(
            "biomass".into(),
            s(&[(b, -1.0), (atp, -5.0), (bio_ext, 1.0)]),
            0.0,
            1000.0,
            false,
        );
        ...
        assert!(
            result.objective_value > 0.0 && result.objective_value < 10.0,
            "Expected biomass constrained by ATP, got {}",
            result.objective_value
        );
    }
    ...
    fn reversible_reactions_not_dead_end() {
        ...
        assert!(
            !dead_ends.contains(&a),
            "A should not be dead end with reversible reaction"
        );
        assert!(
            !dead_ends.contains(&b),
            "B should not be dead end with reversible reaction"
        );
    }
}
