...
pub struct GPUMolecularDynamicsCheckpoint {
    pub device: String,
    pub n_atoms: usize,
...
impl GPUMolecularDynamics {
    ...
    pub fn checkpoint(&self) -> GPUMolecularDynamicsCheckpoint {
        GPUMolecularDynamicsCheckpoint {
    ...
    pub fn from_checkpoint(checkpoint: GPUMolecularDynamicsCheckpoint) -> Result<Self, String> {
        let xyz_len = checkpoint.n_atoms.saturating_mul(3);
        if checkpoint.positions.len() != xyz_len {
            return Err(format!(
                "restore MD checkpoint: expected {} position scalars, got {}",
                xyz_len,
                checkpoint.positions.len()
            ));
        }
        if checkpoint.velocities.len() != xyz_len {
            return Err(format!(
                "restore MD checkpoint: expected {} velocity scalars, got {}",
                xyz_len,
                checkpoint.velocities.len()
            ));
        }
        if checkpoint.masses.len() != checkpoint.n_atoms {
            return Err(format!(
                "restore MD checkpoint: expected {} masses, got {}",
                checkpoint.n_atoms,
                checkpoint.masses.len()
            ));
        }
        if checkpoint.charges.len() != checkpoint.n_atoms {
            return Err(format!(
                "restore MD checkpoint: expected {} charges, got {}",
                checkpoint.n_atoms,
                checkpoint.charges.len()
            ));
        }
        if checkpoint.sigma.len() != checkpoint.n_atoms {
            return Err(format!(
                "restore MD checkpoint: expected {} sigma values, got {}",
                checkpoint.n_atoms,
                checkpoint.sigma.len()
            ));
        }
        if checkpoint.epsilon.len() != checkpoint.n_atoms {
            return Err(format!(
                "restore MD checkpoint: expected {} epsilon values, got {}",
                checkpoint.n_atoms,
                checkpoint.epsilon.len()
            ));
        }

        let mut md = Self::new(checkpoint.n_atoms, &checkpoint.device);
        md.positions = checkpoint.positions;
        md.velocities = checkpoint.velocities;
        md.forces.fill(0.0);
        md.masses = checkpoint.masses;
        md.charges = checkpoint.charges;
        md.sigma = checkpoint.sigma;
        md.epsilon = checkpoint.epsilon;
        md.bonds = checkpoint.bonds;
        md.angles = checkpoint.angles;
        md.dihedrals = checkpoint.dihedrals;
        md.box_size = checkpoint.box_size;
        md.cutoff = checkpoint.cutoff;
        md.temperature = checkpoint.temperature;
        md.gamma = checkpoint.gamma;
        md.rng = checkpoint
            .rng
