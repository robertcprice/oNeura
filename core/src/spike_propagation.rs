...
const AMBIENT_NT_EXPOSURE_GAIN: f32 = 0.02;
const AMBIENT_NT_MAX_EXCESS_MULT: f32 = 12.0;
...
pub fn process_delayed_releases(
    ...
    for syn_idx in 0..synapses.n_synapses {
        ...
        neurons.synaptic_current[post] += sign * psc;
        expose_cleft_nt_to_postsynaptic_ambient(neurons, synapses, syn_idx, cleft_conc, dt);
    }
}

/// Project synaptic cleft transmitter into the postsynaptic ambient NT field.
///
/// This keeps metabotropic signaling and dopamine-gated plasticity subordinate
/// to actual synaptic release rather than requiring an external NT write.
pub fn expose_cleft_nt_to_postsynaptic_ambient(
    neurons: &mut NeuronArrays,
    ...
    let post = synapses.col_indices[synapse_idx] as usize;
    if post >= neurons.count || neurons.alive[post] == 0 {
        return;
    }

    let nt = NTType::from_u8(synapses.nt_type[synapse_idx]);
    let nt_idx = nt.index();
    let rest = nt.resting_conc_nm();
    let delta = (cleft_conc * AMBIENT_NT_EXPOSURE_GAIN * dt.max(0.0)).max(0.0);
    let max_conc = rest * AMBIENT_NT_MAX_EXCESS_MULT;
    neurons.nt_conc[post][nt_idx] = (neurons.nt_conc[post][nt_idx] + delta).clamp(rest, max_conc);
}
