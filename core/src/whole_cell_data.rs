...
pub struct WholeCellOperonState {
    ...
    #[serde(default = "default_scale")]
    pub fork_support: f32,
    #[serde(default)]
    pub fork_pressure: f32,
    #[serde(default = "default_scale")]
    pub promoter_accessibility: f32,
    #[serde(default)]
    pub rnap_occupancy: f32,
    #[serde(default)]
    pub elongation_occupancy: f32,
    #[serde(default)]
    pub blockage_pressure: f32,
    #[serde(default)]
...
pub struct WholeCellProcessOccupancyState {
    #[serde(default)]
    pub current: WholeCellProcessWeights,
    #[serde(default)]
    pub target: WholeCellProcessWeights,
    #[serde(default)]
    pub assembly_rate: WholeCellProcessWeights,
    #[serde(default)]
    pub degradation_rate: WholeCellProcessWeights,
}
...
impl WholeCellProcessOccupancyState {
    pub fn add_projection(
        ...
        self.degradation_rate
            .add_weighted(weights, degradation_rate);
    }
}
...
pub struct WholeCellSavedState {
    ...
    #[serde(default)]
    pub stochastic_config: crate::whole_cell::stochastic_expression::StochasticExpressionConfig,
    #[serde(default)]
    pub stochastic_operon_states:
        Vec<crate::whole_cell::stochastic_expression::StochasticOperonState>,
    #[serde(default)]
    pub stochastic_rng: crate::whole_cell::stochastic_expression::StochasticRng,
}
...
fn synthesize_legacy_local_chemistry_report_from_core(
    ...
    let crowding_penalty = legacy_saved_state_finite_scale(
        1.0 - 0.08 * (metabolic_load - 1.0).max(0.0)
...
fn legacy_saved_state_complex_assembly(
    state: &WholeCellSavedState,
...
fn legacy_saved_state_membrane_state(
    state: &WholeCellSavedState,
...
fn legacy_saved_state_site_coordinates(
    ...
    match preset {
        Syn3ASubsystemPreset::AtpSynthaseMembraneBand => {
            let membrane_band_bias = legacy_saved_state_saturating_signal(
                membrane.membrane_band_lipid_inventory_nm2,
                5_000.0,
            );
...
fn legacy_saved_state_site_report(
    ...
    let (
        ...
        Syn3ASubsystemPreset::AtpSynthaseMembraneBand => {
            ...
            let stability =
                (0.60 + 0.25 * membrane.envelope_integrity + 0.15 * localization).clamp(0.0, 1.5);
            let satisfaction = (0.35 * atp
                + 0.30 * membrane_support
                + 0.20 * stability
        ...
        Syn3ASubsystemPreset::RibosomePolysomeCluster => {
            ...
            let localization =
                (0.40 + 0.35 * occupancy + 0.25 * chemistry.translation_support).clamp(0.0, 1.0);
            ...
            let satisfaction = (0.34 * translation
                + 0.24 * chemistry.atp_support
                + 0.22 * occupancy
        ...
        Syn3ASubsystemPreset::ReplisomeTrack => {
            ...
            let satisfaction = (0.34 * nucleotide
                + 0.24 * chemistry.atp_support
    ...
    if let Some(local) = state.local_chemistry.as_ref() {
        if !local.scheduled_subsystem_probes.is_empty() {
            ...
            for probe in &local.scheduled_subsystem_probes {
                if !probes
                    .iter()
                    .any(|existing: &ScheduledSubsystemProbe| existing.preset == probe.preset)
                {
                    probes.push(*probe);
                }
            }
...
fn synthesize_legacy_md_probe_report_from_state(
    ...
    let selected_preset = state
        ...
        .filter_map(|subsystem| {
            subsystem
                .last_probe_step
                .map(|step| (step, subsystem.preset))
        })
    ...
    let localization = site_report
        ...
        .or_else(|| {
            subsystem_state
                .as_ref()
                .map(|state| state.localization_score)
        })
        .unwrap_or(0.5)
        .clamp(0.0, 1.0);
    let structural_order = subsystem_state
        ...
        .unwrap_or_else(|| {
            let occupancy = site_report
                .map(|report| report.assembly_occupancy)
                .unwrap_or(0.6);
            let stability = site_report
    ...
    let assembly_occupancy = site_report
        ...
        .or_else(|| {
            subsystem_state
                .as_ref()
                .map(|state| state.assembly_occupancy)
        })
        .unwrap_or(0.6)
        .clamp(0.0, 1.5);
    let assembly_stability = site_report
        ...
        .or_else(|| {
            subsystem_state
                .as_ref()
                .map(|state| state.assembly_stability)
        })
    ...
    let demand_satisfaction = site_report
        ...
        .or_else(|| {
            subsystem_state
                .as_ref()
                .map(|state| state.demand_satisfaction)
        })
        .unwrap_or(0.8)
        .clamp(0.35, 1.0);
    let axis_anisotropy = site_report
        .map(|report| {
            let x_fraction =
                report.site_x as f32 / state.config.x_dim.saturating_sub(1).max(1) as f32;
    ...
    let electrostatic_order = site_report
        .map(|report| {
            (0.48 + 0.24 * report.mean_phosphorus + 0.18 * report.mean_nitrate
                - 0.10 * report.mean_proton)
                .clamp(0.1, 1.0)
        })
        .unwrap_or((0.50 + 0.20 * localization).clamp(0.1, 1.0));
    let vdw_cohesion =
        (0.45 * compactness + 0.35 * assembly_stability + 0.20 * structural_order).clamp(0.1, 1.0);
    ...
    let dihedral_density = (0.24 + 0.22 * vdw_cohesion + 0.16 * localization).clamp(0.0, 1.0);
    let charge_density =
        (0.10 + 0.18 * electrostatic_order + 0.08 * polar_fraction).clamp(0.0, 1.0);
    let mean_temperature = (310.0 - 7.0 * (structural_order - 0.5)
        + 9.0 * (1.0 - crowding_penalty))
        .clamp(285.0, 330.0);
    let mean_total_energy =
        (-10.0 - 6.0 * vdw_cohesion - 4.0 * electrostatic_order).clamp(-40.0, 5.0);
    ...
    let recommended_atp_scale = subsystem_state
        ...
        .unwrap_or_else(|| {
            legacy_saved_state_finite_scale(
    ...
    let recommended_translation_scale = subsystem_state
        ...
        .unwrap_or_else(|| {
            legacy_saved_state_finite_scale(
                0.78 + 0.26 * state.chemistry_report.translation_support
                    + 0.12 * demand_satisfaction,
                1.0,
                0.70,
                1.45,
            )
...
fn synthesize_legacy_expression_summaries_from_assets(
    ...
    for complex in &assets.complexes {
        ...
        match complex.family {
            ...
            WholeCellAssemblyFamily::Replisome | WholeCellAssemblyFamily::ReplicationInitiator => {
...
fn synthesize_legacy_expression_summaries_from_named_complexes(
    ...
    for complex in named_complexes {
        ...
        entry.process_signal.add_weighted(
            legacy_expression_unit_weights(WholeCellProcessWeights::default(), complex.asset_class),
...
fn synthesize_legacy_expression_state_from_saved_state(
    ...
    for (operon, summary) in entries {
        ...
        let mature_transcript_abundance =
            (transcript_abundance - nascent_transcript_abundance - damaged_transcript_abundance)
                .max(0.0);
        let mature_protein_abundance =
            (protein_abundance - nascent_protein_abundance - damaged_protein_abundance).max(0.0);
    ...
    Some(WholeCellOrganismExpressionState {
        ...
        process_scales: WholeCellProcessWeights {
            ...
            constriction: legacy_expression_process_scale(
...
fn hydrate_legacy_saved_state_explicit_state(state: &mut WholeCellSavedState) {
    ...
    for preset in state
        .subsystem_states
        .iter()
        .map(|subsystem| subsystem.preset)
    {
        if !explicit_probe_presets.contains(&preset) {
            explicit_probe_presets.push(preset);
        }
    ...
    if state.scheduled_subsystem_probes.is_empty() {
        ...
        let summary_only_probe_recovery =
            !had_explicit_site_reports && !had_explicit_subsystem_state;
...
pub fn finalize_saved_state(state: WholeCellSavedState) -> Result<WholeCellSavedState, String> {
    finalize_parsed_saved_state(state)
}
...
mod tests {
    ...
    fn minimal_saved_state_from_spec(spec: &WholeCellProgramSpec) -> WholeCellSavedState {
        WholeCellSavedState {
            ...
            stochastic_rng: crate::whole_cell::stochastic_expression::StochasticRng::default(),
        }
    }
    ...
    fn parse_legacy_saved_state_json_prefers_site_reports_for_missing_chemistry_report() {
        ...
        assert_eq!(
            reparsed.chemistry_site_reports,
            saved.chemistry_site_reports
        );
    }
    ...
    fn parse_legacy_saved_state_json_promotes_last_md_probe_into_subsystem_state() {
        ...
        expected_replisome
            .apply_probe_report(saved.last_md_probe.expect("probe"), saved.core.step_count);
    ...
    fn parse_legacy_saved_state_preserves_explicit_site_probe_subset() {
        ...
        let mut saved = minimal_saved_state_from_spec(&spec);
        saved.chemistry_site_reports =
            synthesize_legacy_local_chemistry_site_reports_from_state(&saved)
                .into_iter()
                .filter(|report| {
                    matches!(
                        report.preset,
