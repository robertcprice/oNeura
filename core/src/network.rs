...
pub struct MolecularBrainCheckpoint {
    pub neurons: NeuronArrays,
    pub synapses: SynapseArrays,
    pub circadian: CircadianClock,
    pub glia: GliaState,
    ...
    pub enable_microtubules: bool,
    pub enable_gpu: bool,
    pub gs_input: Vec<f32>,
    pub gi_input: Vec<f32>,
    pub gq_input: Vec<f32>,
}
...
impl MolecularBrain {
    ...
    pub fn step(&mut self) {
        ...
        if use_gpu {
            ...
            for i in 0..self.synapses.n_synapses {
                let cleft_conc = self.synapses.update_cleft(i, self.time, dt);
                crate::spike_propagation::expose_cleft_nt_to_postsynaptic_ambient(
                    &mut self.neurons,
                    &self.synapses,
                    i,
                    cleft_conc,
    ...
    pub fn checkpoint(&mut self) -> MolecularBrainCheckpoint {
        self.sync_shadow_from_gpu();
        MolecularBrainCheckpoint {
            neurons: self.neurons.clone(),
            synapses: self.synapses.clone(),
            circadian: self.circadian.clone(),
            glia: self.glia.clone(),
            pharmacology: self.pharmacology.clone(),
            psc_scale: self.psc_scale,
            dt: self.dt,
            time: self.time,
            step_count: self.step_count,
            channel_g_max: self.channel_g_max,
            kleak_reversal_mv: self.kleak_reversal_mv,
            membrane_capacitance_uf: self.membrane_capacitance_uf,
            spike_threshold_mv: self.spike_threshold_mv,
            refractory_period_ms: self.refractory_period_ms,
            enable_glia: self.enable_glia,
            enable_circadian: self.enable_circadian,
            enable_pharmacology: self.enable_pharmacology,
            enable_gene_expression: self.enable_gene_expression,
            enable_metabolism: self.enable_metabolism,
            enable_microtubules: self.enable_microtubules,
            enable_gpu: self.enable_gpu,
            gs_input: self.gs_input.clone(),
            gi_input: self.gi_input.clone(),
            gq_input: self.gq_input.clone(),
        }
    }

    pub fn from_checkpoint(checkpoint: MolecularBrainCheckpoint) -> Self {
        let mut brain = Self::new(checkpoint.neurons.count);
        brain.neurons = checkpoint.neurons;
        brain.synapses = checkpoint.synapses;
        brain.circadian = checkpoint.circadian;
        brain.glia = checkpoint.glia;
        brain.pharmacology = checkpoint.pharmacology;
        brain.psc_scale = checkpoint.psc_scale;
        brain.dt = checkpoint.dt;
        brain.time = checkpoint.time;
        brain.step_count = checkpoint.step_count;
        brain.channel_g_max = checkpoint.channel_g_max;
        brain.kleak_reversal_mv = checkpoint.kleak_reversal_mv;
        brain.membrane_capacitance_uf = checkpoint.membrane_capacitance_uf;
        brain.spike_threshold_mv = checkpoint.spike_threshold_mv;
        brain.refractory_period_ms = checkpoint.refractory_period_ms;
        brain.enable_glia = checkpoint.enable_glia;
        brain.enable_circadian = checkpoint.enable_circadian;
        brain.enable_pharmacology = checkpoint.enable_pharmacology;
        brain.enable_gene_expression = checkpoint.enable_gene_expression;
        brain.enable_metabolism = checkpoint.enable_metabolism;
        brain.enable_microtubules = checkpoint.enable_microtubules;
        brain.enable_gpu = checkpoint.enable_gpu;
        brain.gs_input = checkpoint.gs_input;
        brain.gi_input = checkpoint.gi_input;
        brain.gq_input = checkpoint.gq_input;
        #[cfg(target_os = "macos")]
        brain.invalidate_gpu_resident_state();
        brain
    }
    ...
    fn step_gpu(&mut self, dt: f32, global_bias: f32) {
        ...
        if let (Some(gpu), Some(gpu_neurons), Some(gpu_synapses)) = (
            ...
            gpu::receptor_binding::encode_receptor_binding(gpu, &cmd, gpu_neurons, dt);
    ...
    fn step_cpu_with_bias(&mut self, dt: f32, global_bias: f32) {
        ...
        gpu::receptor_binding::cpu_receptor_binding(&mut self.neurons, dt);
    ...
    fn run_gpu_steps_batched_if_possible(&mut self, steps: u64) -> bool {
        ...
        while remaining > 0 {
            ...
            for _ in 0..batch {
                ...
                gpu::receptor_binding::encode_receptor_binding(gpu, &cmd, gpu_neurons, self.dt);
                gpu::membrane_integration::encode_membrane_integration(
                    gpu,
                    &cmd,
                    gpu_neurons,
    ...
    /// Count how many neurons in a contiguous range fired this step.
    pub fn fired_count_range(&self, range: std::ops::Range<usize>) -> usize {
        #[cfg(target_os = "macos")]
        if self.should_use_gpu() {
            if let Some(gpu_neurons) = self.gpu_neurons.as_ref() {
                return gpu_neurons.sum_fired_range(range.start, range.end) as usize;
            }
        }

        let start = range.start.min(self.neurons.count);
        let end = range.end.min(self.neurons.count);
        self.neurons.fired[start..end]
            .iter()
            .filter(|&&fired| fired != 0)
            .count()
    }
    ...
    /// Sum cumulative spike counters across a contiguous range without forcing
    /// a full host shadow sync.
    pub fn spike_count_range_sum(&self, range: std::ops::Range<usize>) -> u64 {
        #[cfg(target_os = "macos")]
        if self.should_use_gpu() {
            if let Some(gpu_neurons) = self.gpu_neurons.as_ref() {
                return gpu_neurons.sum_spike_count_range(range.start, range.end);
            }
        }

        let start = range.start.min(self.neurons.count);
        let end = range.end.min(self.neurons.count);
        self.neurons.spike_count[start..end]
            .iter()
