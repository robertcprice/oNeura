...
impl SeedCellularStateSim {
    ...
    pub fn step(
        ...
        carbon_dioxide: f32,
        acidity: f32,
        microbial_pressure: f32,
        dormancy_s: f32,
        reserve_carbon: f32,
    ) -> SeedCellularFeedback {
        let dormancy_t = clamp(dormancy_s / 26_000.0, 0.0, 1.0);
        let reserve_t = clamp(reserve_carbon / 0.2, 0.0, 1.0);
        let exposure_t = clamp(surface_exposure, 0.0, 1.0);
        let oxygen_t = clamp(oxygen_gas / 0.45, 0.0, 1.4);
        let co2_t = clamp(carbon_dioxide / 0.35, 0.0, 1.6);
        let acidity_t = clamp(acidity / 0.35, 0.0, 1.8);
        let microbial_t = clamp(microbial_pressure / 0.40, 0.0, 1.8);
        ...
        let decay_flux = (((0.24 - oxygen_t).max(0.0) * 0.0007)
            + acidity_t * 0.00018
            + co2_t * 0.00006
            + microbial_t * 0.00028)
            * dt;
        ...
        self.coat.state_glucose = (self.coat.state_glucose
            - microbial_t * (0.00008 + acidity_t * 0.00002) * dt
            - decay_flux * 0.12)
            .max(0.0);
        ...
        self.endosperm.state_glucose = (self.endosperm.state_glucose
            - microbial_t * (0.00022 + co2_t * 0.00004) * dt
            - decay_flux * 0.26)
            .max(0.0);
        self.endosperm.state_starch = (self.endosperm.state_starch
            - reserve_release * (0.88 + germination_drive * 0.24)
            - microbial_t * 0.00012 * dt
            - decay_flux * 0.85)
            .max(0.0);
        ...
        self.cotyledon.state_glucose = (self.cotyledon.state_glucose
            - microbial_t * (0.00006 + acidity_t * 0.00002) * dt
            - decay_flux * 0.12)
            .max(0.0);
        self.cotyledon.state_starch =
            (self.cotyledon.state_starch + reserve_release * 0.05 - decay_flux * 0.08).max(0.0);
        ...
        self.coat.cell_count = (self.coat.cell_count
            - (imbibition * 0.0008 + microbial_t * 0.0009 + acidity_t * 0.0003) * dt
            - decay_flux * 0.55)
            .max(4.0);
        self.endosperm.cell_count = (self.endosperm.cell_count - decay_flux * 0.25).max(4.0);
        self.radicle.cell_count = (self.radicle.cell_count - decay_flux * 0.18).max(4.0);
        self.cotyledon.cell_count = (self.cotyledon.cell_count - decay_flux * 0.18).max(4.0);
...
mod tests {
    ...
    fn seed_cellular_state_stays_bounded() {
        ...
        for _ in 0..180 {
            let feedback = state.step(
                30.0, 0.7, 0.5, 0.18, 0.08, 0.3, 0.06, 1.0, 0.42, 0.10, 0.08, 0.04, 9_000.0, 0.14,
            );
