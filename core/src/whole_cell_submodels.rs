    ...
    fn add_species(&mut self, species: TerrariumSpecies, weight: f32) {
        ...
        match species {
            ...
            TerrariumSpecies::AminoAcidPool => {
                self.glucose += weight * 0.18;
                self.atp_flux += weight * 0.16;
                self.ammonium += weight * 0.56;
                self.phosphorus += weight * 0.10;
            }
            TerrariumSpecies::NucleotidePool => {
                self.glucose += weight * 0.10;
                self.atp_flux += weight * 0.18;
                self.nitrate += weight * 0.42;
                self.ammonium += weight * 0.08;
                self.phosphorus += weight * 0.34;
            }
            TerrariumSpecies::MembranePrecursorPool => {
                self.glucose += weight * 0.30;
                self.oxygen += weight * 0.06;
                self.atp_flux += weight * 0.28;
                self.phosphorus += weight * 0.18;
            }
            _ => {}
        }
    }
...
fn representative_calibration_snapshot() -> WholeCellSnapshot {
    WholeCellSnapshot {
        ...
        membrane_division_state: crate::whole_cell_data::WholeCellMembraneDivisionState::default(),
        radius_nm: 210.0,
