...
pub const GENOTYPE_PACKET_MAX_PER_CELL: usize = 12;
pub const GENOTYPE_PACKET_POPULATION_MAX_CELLS: usize = 128;
...
pub enum GenotypePacketEcology {
    #[default]
    Decomposer,
    Nitrifier,
    Denitrifier,
...
impl GenotypePacketEcology {
    #[allow(dead_code)]
    pub(crate) fn label(self) -> &'static str {
        match self {
            Self::Decomposer => "decomposer",
            Self::Nitrifier => "nitrifier",
            Self::Denitrifier => "denitrifier",
        }
    }
...
pub struct GenotypePacket {
    ...
    /// Ecological role carried by this genotype packet.
    pub(crate) ecology: GenotypePacketEcology,
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct GenotypePacketPopulation {
    /// Grid position.
    pub(crate) x: usize,
    pub(crate) y: usize,
    pub(crate) z: usize,
...
impl GenotypePacketPopulation {
    ...
    pub(crate) fn seed_from_secondary_entries(
        &mut self,
        bank_entries: &[(SecondaryCatalogBankEntry, GenotypePacketEcology)],
        total_coarse_cells: f32,
    ) {
        self.packets.clear();
        for (slot, (entry, ecology)) in bank_entries.iter().enumerate() {
    ...
    pub(crate) fn dominant_packet(&self) -> Option<&GenotypePacket> {
        self.packets.iter().max_by(|a, b| {
            let a_score = a.represented_cells * (0.7 + a.activity * 0.3);
            let b_score = b.represented_cells * (0.7 + b.activity * 0.3);
            a_score.total_cmp(&b_score)
        })
    }

    pub(crate) fn dominant_ecology(&self) -> Option<GenotypePacketEcology> {
        let mut totals = [0.0f32; 3];
        for packet in &self.packets {
            totals[packet.ecology.index()] += packet.represented_cells.max(0.0);
        }
        totals
            .iter()
            .enumerate()
    ...
    pub(crate) fn ecology_totals(&self) -> [f32; 3] {
        let mut totals = [0.0f32; 3];
        for packet in &self.packets {
            totals[packet.ecology.index()] += packet.represented_cells.max(0.0);
        }
    ...
    pub(crate) fn ecology_diversity(&self) -> f32 {
        let totals = self.ecology_totals();
        let sum = totals.iter().sum::<f32>().max(1.0e-6);
        let squared = totals
            .iter()
            .map(|value| {
                let share = (*value / sum).clamp(0.0, 1.0);
                share * share
            })
            .sum::<f32>();
        ((1.0 - squared) / (1.0 - 1.0 / 3.0)).clamp(0.0, 1.0)
    }

    /// Try to bud a new packet into a neighboring cell.
    /// Returns `Some((target_x, target_y, new_packet))` if budding occurs.
    pub(crate) fn try_bud(
        &mut self,
        width: usize,
        height: usize,
    ) -> Option<(usize, usize, GenotypePacket)> {
        ...
        let bud_idx = self.packets.iter().position(|p| {
            p.activity > 0.7 && p.reserve > 0.6 && p.represented_cells > 5.0 && p.dormancy < 0.1
        })?;
