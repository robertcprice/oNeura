...
impl SoilBroadSecondaryBanks {
    pub(crate) fn len(&self) -> usize {
        self.bank_packets.len()
    }
    pub(crate) fn bank_packets(&self, idx: usize) -> &[f32] {
        &self.bank_packets[idx]
    }
    pub(crate) fn bank_trait_a(&self, idx: usize) -> &[f32] {
        &self.bank_trait_a[idx]
    }
    pub(crate) fn bank_trait_b(&self, idx: usize) -> &[f32] {
        &self.bank_trait_b[idx]
    }
    pub(crate) fn bank_catalog_entries(&self, idx: usize) -> &[SecondaryCatalogBankEntry] {
        &self.bank_catalog_entries[idx]
    }
    pub(crate) fn bank_catalog_slots(&self, idx: usize) -> &[u32] {
        &self.bank_catalog_slots[idx]
    }
}
...
pub(crate) fn refresh_secondary_local_catalog_identity(
    ...
    entries
        .iter()
        .map(|_| SecondaryCatalogBankEntry::default())
        .collect()
}
...
pub(crate) const MICROBIAL_GENE_DORMANCY_MAINTENANCE_WEIGHTS: [f32;
    INTERNAL_SECONDARY_GENOTYPE_AXES] = [-0.10, 0.12, 0.34, 0.28, 0.10, 0.18];
pub(crate) const MICROBIAL_GENE_EXTRACELLULAR_SCAVENGING_WEIGHTS: [f32;
    INTERNAL_SECONDARY_GENOTYPE_AXES] = [0.14, 0.04, 0.10, 0.06, 0.40, 0.22];
pub(crate) const NITRIFIER_GENE_OXYGEN_RESPIRATION_WEIGHTS: [f32;
    INTERNAL_SECONDARY_GENOTYPE_AXES] = [0.62, -0.06, 0.16, -0.02, 0.18, -0.02];
pub(crate) const NITRIFIER_GENE_AMMONIUM_TRANSPORT_WEIGHTS: [f32;
    INTERNAL_SECONDARY_GENOTYPE_AXES] = [-0.04, 0.56, 0.10, 0.14, 0.08, 0.12];
pub(crate) const NITRIFIER_GENE_STRESS_PERSISTENCE_WEIGHTS: [f32;
    INTERNAL_SECONDARY_GENOTYPE_AXES] = [0.02, 0.14, 0.32, 0.28, 0.12, 0.18];
...
pub(crate) const DENITRIFIER_GENE_ANOXIA_RESPIRATION_WEIGHTS: [f32;
    INTERNAL_SECONDARY_GENOTYPE_AXES] = [0.60, -0.06, 0.10, 0.18, 0.16, -0.02];
pub(crate) const DENITRIFIER_GENE_NITRATE_TRANSPORT_WEIGHTS: [f32;
    INTERNAL_SECONDARY_GENOTYPE_AXES] = [-0.02, 0.58, 0.08, 0.16, 0.08, 0.10];
pub(crate) const DENITRIFIER_GENE_STRESS_PERSISTENCE_WEIGHTS: [f32;
    INTERNAL_SECONDARY_GENOTYPE_AXES] = [0.04, 0.14, 0.30, 0.30, 0.10, 0.20];
pub(crate) const DENITRIFIER_GENE_REDUCTIVE_FLEXIBILITY_WEIGHTS: [f32;
    INTERNAL_SECONDARY_GENOTYPE_AXES] = [0.18, 0.16, 0.16, 0.18, 0.34, 0.20];
...
pub fn packet_surface_factor(cells: f32, packets: f32, target_cells: f32) -> f32 {
    (0.78 + (target_cells / packet_load(cells, packets).max(0.25)).sqrt() * 0.26).clamp(0.62, 1.35)
}

pub fn trait_match(current: f32, target: f32) -> f32 {
...
pub(crate) fn secondary_bank_catalog_divergence(
    genes: &[f32; INTERNAL_SECONDARY_GENOTYPE_AXES],
) -> f32 {
...
impl PublicSecondaryBanks {
    ...
    pub(crate) fn catalog_bank_entries(
        &self,
    ) -> [&[SecondaryCatalogBankEntry]; PUBLIC_STRAIN_BANKS] {
...
pub(crate) fn nitrifier_aerobic_target(
    oxygen_factor: f32,
    aeration_factor: f32,
    anoxia_factor: f32,
) -> f32 {
...
pub(crate) fn denitrifier_anoxic_target(
    anoxia_factor: f32,
    deep_moisture: f32,
    oxygen_factor: f32,
) -> f32 {
