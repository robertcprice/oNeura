...
use crate::metabolic_flux::{soil_microbe_generic, MetabolicNetwork};
use crate::microbiome_assembly::{
    shannon_diversity, CommunityAssembler, MicrobialTaxon, Resource as MicrobeResource,
};
use crate::nutrient_cycling::NutrientCycler;
use crate::phylogenetic_tracker::{PhyloTraits, PhyloTree};
use crate::population_genetics::WrightFisherSim;
use crate::resistance_evolution::{
    Antibiotic, AntibioticClass, ModeOfAction, ResistanceMechanism, ResistanceSimulator,
    ResistanceType,
};
...
impl IntegratedEcosystem {
    ...
    pub fn new(config: EcosystemConfig) -> Self {
        ...
        // 6. Biofilm dynamics
        let mut biofilm =
            BiofilmSimulator::new(config.biofilm_width, config.biofilm_height, seed + 4);
        biofilm.seed_cells(0, 30, 3.3e-4);

        // 7. Eco-evolutionary feedback
        let eco_evo = EcoEvoSimulator::new(config.initial_pop_size, config.n_traits, seed + 5);
    ...
    pub fn step(&mut self) -> EcosystemSnapshot {
        let timing_enabled = std::env::var_os("ONEURA_ECOSYSTEM_TIMING").is_some();
        let mut stage_start = Instant::now();
        let log_stage = |label: &str, start: &mut Instant| {
            if timing_enabled {
                eprintln!(
                    "[ecosystem-step] {:<18} {:>8.3}s",
                    label,
        ...
        log_stage("climate", &mut stage_start);
        ...
        log_stage("nutrients", &mut stage_start);
        ...
        log_stage("microbiome", &mut stage_start);
        ...
        log_stage("resistance", &mut stage_start);
        ...
        let mean_mic_fold = if !self.resistance.strains.is_empty() {
            self.resistance
                .strains
                .iter()
                .map(|(s, _)| s.resistance_mechanisms.len() as f64 * 4.0)
                .sum::<f64>()
                / self.resistance.strains.len() as f64
        } else {
            1.0
        };
        ...
        log_stage("biofilm", &mut stage_start);

        // ── 7. Eco-evolutionary feedback ──
        let eco_evo_result = self.eco_evo.step();
        log_stage("eco_evo", &mut stage_start);

        // ── 8. Population genetics ──
        let allele_freq = self.pop_gen.step();
        log_stage("pop_gen", &mut stage_start);

        // ── 9. Phylogenetic tracker ──
        let parent_id = if self.step_count > 1 {
            Some(self.step_count - 1)
        } else {
            None
        };
    ...
    pub fn phylo_tree(&self) -> &PhyloTree {
        &self.phylo
    }
    pub fn nutrient_cycler(&self) -> &NutrientCycler {
        &self.nutrients
    }
    pub fn climate_engine(&self) -> &ClimateEngine {
        &self.climate
    }
    pub fn resistance_sim(&self) -> &ResistanceSimulator {
        &self.resistance
    }
    pub fn resistance_sim_mut(&mut self) -> &mut ResistanceSimulator {
        &mut self.resistance
    }
    pub fn eco_evo_sim(&self) -> &EcoEvoSimulator {
        &self.eco_evo
    }
    pub fn biofilm_sim(&self) -> &BiofilmSimulator {
        &self.biofilm
    }
    pub fn hgt_population(&self) -> &HgtPopulation {
        &self.hgt
    }
    pub fn metabolic_network(&self) -> &MetabolicNetwork {
        &self.fba_network
    }
}
...
impl EcosystemSnapshot {
    pub fn summary_line(&self) -> String {
        format!(
            ...
            self.temperature_c,
            self.co2_ppm,
            self.soil_organic_c_g,
            self.soil_mineral_n_g,
            self.soil_mineral_p_g,
            self.microbial_richness,
            self.microbial_shannon,
            self.resistance_events,
            self.mdr_strain_count,
            self.biofilm_cell_count,
            self.eps_coverage * 100.0,
            self.mean_fitness,
            self.allele_freq,
            self.phylo_tree_size,
            self.phylo_diversity,
            self.fba_growth_rate,
            self.fba_status,
        )
    }
}
...
impl EcosystemTimeSeries {
    pub fn print_report(&self) {
        ...
        println!(
            "  Temperature: {:.1}C -> {:.1}C",
            first.temperature_c, last.temperature_c
        );
        println!("  CO2: {:.0} -> {:.0} ppm", first.co2_ppm, last.co2_ppm);
        println!("--- NUTRIENT CYCLING ---");
        println!(
            "  Organic C: {:.1} -> {:.1} g",
            first.soil_organic_c_g, last.soil_organic_c_g
        );
        println!(
            "  Mineral N: {:.2} -> {:.2} g",
            first.soil_mineral_n_g, last.soil_mineral_n_g
        );
        println!(
            "  Mineral P: {:.2} -> {:.2} g",
            first.soil_mineral_p_g, last.soil_mineral_p_g
        );
        ...
        println!(
            "  Richness: {} -> {}",
            first.microbial_richness, last.microbial_richness
        );
        println!(
            "  Shannon H': {:.2} -> {:.2}",
            first.microbial_shannon, last.microbial_shannon
        );
        println!(
            "  Total biomass: {:.1} -> {:.1}",
            first.total_microbial_biomass, last.total_microbial_biomass
        );
        ...
        println!(
            "  MDR strains: {} -> {}",
            first.mdr_strain_count, last.mdr_strain_count
        );
        println!("--- HGT ---");
        println!(
            "  Resistance frequency: {:.3} -> {:.3}",
            first.hgt_resistance_freq, last.hgt_resistance_freq
        );
        println!("--- BIOFILM ---");
        println!(
            "  Cells: {} -> {}",
            first.biofilm_cell_count, last.biofilm_cell_count
        );
        println!(
            "  EPS coverage: {:.1}% -> {:.1}%",
            first.eps_coverage * 100.0,
            last.eps_coverage * 100.0
        );
        println!(
            "  Quorum activated: {:.1}% -> {:.1}%",
            first.quorum_activated * 100.0,
            last.quorum_activated * 100.0
        );
        println!("--- ECO-EVOLUTIONARY DYNAMICS ---");
        println!(
            "  Mean fitness: {:.3} -> {:.3}",
            first.mean_fitness, last.mean_fitness
        );
        println!(
            "  Genetic variance: {:.4} -> {:.4}",
            first.genetic_variance_sum, last.genetic_variance_sum
        );
        println!("--- POPULATION GENETICS ---");
        println!(
            "  Allele freq: {:.3} -> {:.3}",
            first.allele_freq, last.allele_freq
        );
        println!("--- PHYLOGENETICS ---");
        println!(
            "  Tree size: {} -> {} nodes",
            first.phylo_tree_size, last.phylo_tree_size
        );
        println!(
            "  Faith's PD: {:.1} -> {:.1}",
            first.phylo_diversity, last.phylo_diversity
        );
        ...
        println!(
            "  Growth rate: {:.4} -> {:.4}",
            first.fba_growth_rate, last.fba_growth_rate
        );
    ...
    pub fn to_json(&self) -> String {
        ...
        for s in &self.snapshots {
            entries.push(format!(
                ...
                s.soil_mineral_n_g,
                s.soil_mineral_p_g,
                s.microbial_richness,
                s.microbial_shannon,
                s.resistance_events,
                s.mdr_strain_count,
                s.biofilm_cell_count,
                s.eps_coverage,
                s.mean_fitness,
                s.allele_freq,
                s.phylo_diversity,
...
mod tests {
    ...
    fn ecosystem_climate_drives_co2() {
        ...
        assert!(
            last.co2_ppm > first.co2_ppm,
            "CO2 should increase under RCP 8.5: {} -> {}",
            first.co2_ppm,
            last.co2_ppm
        );
    }
    ...
    fn ecosystem_nutrient_cycling_active() {
        ...
        let snap = eco.step();
        assert!(
            snap.soil_organic_c_g >= 0.0,
            "Organic C should be non-negative"
        );
    }
    ...
    fn ecosystem_phylo_tree_grows() {
        ...
        assert!(
            eco.phylo_tree().len() >= 10,
            "Phylo tree should grow: {} nodes",
            eco.phylo_tree().len()
        );
    }
    ...
    fn ecosystem_fba_optimal() {
        ...
        let snap = eco.step();
        assert_eq!(
            snap.fba_status, "Optimal",
            "FBA should find optimal, got: {}",
            snap.fba_status
        );
        assert!(
            snap.fba_growth_rate > 0.0,
            "FBA growth should be positive: {}",
            snap.fba_growth_rate
        );
    }
