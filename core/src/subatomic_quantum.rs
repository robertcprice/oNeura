...
#[derive(Clone, Copy, Debug, PartialEq, Serialize, Deserialize)]
pub struct QuantumElementDescriptor {
    pub element: PeriodicElement,
    pub formal_charge: i8,
    pub valence_electrons: u32,
    pub ionization_proxy_ev: f64,
    pub mean_valence_orbital_energy_ev: f64,
    pub mean_effective_nuclear_charge: f64,
    pub mean_orbital_radius_angstrom: f64,
    pub valence_occupancy_fraction: f64,
    pub open_shell_fraction: f64,
    pub effective_charge_index: f64,
    pub hardness_index: f64,
    pub polarizability_index: f64,
}

pub fn quantum_element_descriptor(
    element: PeriodicElement,
    formal_charge: i8,
) -> Result<QuantumElementDescriptor, QuantumMicrodomainError> {
    let mut atom = AtomNode::new(element);
    atom.formal_charge = formal_charge;
    let state = atom.quantum_state()?;
    let active_subshells = state.active_subshells();
    let electron_weight = active_subshells
        .iter()
        .map(|shell| f64::from(shell.electrons))
        .sum::<f64>()
        .max(1.0);
    let capacity = active_subshells
        .iter()
        .map(|shell| f64::from(shell.capacity))
        .sum::<f64>()
        .max(1.0);
    let mean_effective_nuclear_charge = active_subshells
        .iter()
        .map(|shell| shell.effective_nuclear_charge * f64::from(shell.electrons))
        .sum::<f64>()
        / electron_weight;
    let mean_orbital_radius_angstrom = active_subshells
        .iter()
        .map(|shell| shell.orbital_radius_angstrom * f64::from(shell.electrons))
        .sum::<f64>()
        / electron_weight;
    let mean_valence_orbital_energy_ev = active_subshells
        .iter()
        .map(|shell| shell.orbital_energy_ev * f64::from(shell.electrons))
        .sum::<f64>()
        / electron_weight;
    let ionization_proxy_ev = state.ionization_proxy_ev();
    let valence_occupancy_fraction = (electron_weight / capacity).clamp(0.0, 1.0);
    let open_shell_fraction = (1.0 - valence_occupancy_fraction).clamp(0.0, 1.0);
    let effective_charge_index = (mean_effective_nuclear_charge
        / (mean_effective_nuclear_charge + mean_orbital_radius_angstrom.max(0.25)))
    .clamp(0.0, 1.0);
    let hardness_index = (ionization_proxy_ev
        / (ionization_proxy_ev + 12.0 * mean_orbital_radius_angstrom.max(0.25)))
    .clamp(0.0, 1.0);
    let polarizability_proxy = mean_orbital_radius_angstrom.powi(3) / ionization_proxy_ev.max(1.0);
    let polarizability_index =
        (polarizability_proxy / (polarizability_proxy + 0.35)).clamp(0.0, 1.0);

    Ok(QuantumElementDescriptor {
        element,
        formal_charge,
        valence_electrons: state.valence_electrons(),
        ionization_proxy_ev,
        mean_valence_orbital_energy_ev,
        mean_effective_nuclear_charge,
        mean_orbital_radius_angstrom,
...
impl MolecularActiveSpace {
    ...
    fn full_ed_diagonalize(&self) -> Result<ExactDiagonalizationResult, QuantumMicrodomainError> {
...
fn determinant_diagonal_energy(space: &MolecularActiveSpace, state: u64, n_spatial: usize) -> f64 {
    ...
    for spin in 0..2usize {
        for p in 0..n_spatial {
            for q in 0..n_spatial {
                ...
                let amplitude = space.one_body_integrals_ev[matrix_index(p, q, n_spatial)];
                if amplitude.abs() < 1.0e-12 {
                    continue;
                }
                if let Some((hopped_state, sign)) = apply_hop(state, 2 * q + spin, 2 * p + spin) {
...
/// single excitation.
fn hamiltonian_coupling(space: &MolecularActiveSpace, bra: u64, ket: u64) -> f64 {
...
fn cipsi_selected_ci(
    ...
    let mut variational_set: std::collections::HashSet<u64> = std::collections::HashSet::from([hf]);
    ...
    loop {
        ...
        let candidates = generate_singles_doubles(space, &variational_set, &variational_basis);
    ...
    let expected_spatial_occupancies = (0..n_spatial)
        .map(|orbital_idx| {
            spatial_one_particle_density_matrix[matrix_index(orbital_idx, orbital_idx, n_spatial)]
        })
        .collect::<Vec<_>>();
    let expected_atom_effective_charges = expected_atom_effective_charges(
        &space.atoms,
...
fn en_pt2_correction(
    ...
    let external_dets = generate_singles_doubles(space, &variational_set, variational_basis);
...
impl ExactDiagonalizationResult {
    ...
    pub fn natural_orbitals(&self) -> Option<(Vec<f64>, Vec<f64>)> {
        ...
        let occupancies: Vec<f64> = order
            .iter()
            .map(|&i| eigenvalues[i].max(0.0).min(2.0))
            .collect();
...
mod tests {
    ...
    #[test]
    fn quantum_element_descriptor_tracks_charge_and_shell_structure() {
        let sodium_neutral = quantum_element_descriptor(PeriodicElement::Na, 0).unwrap();
        let sodium_cation = quantum_element_descriptor(PeriodicElement::Na, 1).unwrap();
        let iron_cation = quantum_element_descriptor(PeriodicElement::Fe, 3).unwrap();

        assert!(
            sodium_cation.mean_orbital_radius_angstrom
                < sodium_neutral.mean_orbital_radius_angstrom
        );
        assert!(sodium_cation.effective_charge_index > sodium_neutral.effective_charge_index);
        assert!(sodium_cation.hardness_index > sodium_neutral.hardness_index);
        assert!(
            sodium_cation.mean_valence_orbital_energy_ev
                < sodium_neutral.mean_valence_orbital_energy_ev
        );
        assert!(iron_cation.open_shell_fraction > sodium_cation.open_shell_fraction);
        assert!(sodium_cation.valence_occupancy_fraction > iron_cation.valence_occupancy_fraction);
    }
}
