...
const NT_CONC_SCALE: f32 = 1000.0;
...
pub fn encode_synapse_step(
    ...
    if neuron_threads > 0 {
        ...
        let nt_slots = neuron_count * NTType::COUNT;
        let clear_nt_params = ClearIntParams {
            count: nt_slots as u32,
        };
        let clear_nt_bytes = unsafe {
            std::slice::from_raw_parts(
                &clear_nt_params as *const ClearIntParams as *const u8,
                std::mem::size_of::<ClearIntParams>(),
            )
        };
        gpu.encode_1d(
            cmd,
            &gpu.pipelines.clear_i32_buffer,
            &[(&neurons.nt_conc_accum_i32, 0)],
            Some((clear_nt_bytes, 1)),
            nt_slots as u64,
        );
    }
    ...
    if neuron_threads > 0 {
        ...
        let nt_params = NtCommitParams {
            slot_count: (neuron_count * NTType::COUNT) as u32,
            nt_scale: NT_CONC_SCALE,
        };
        let nt_bytes = unsafe {
            std::slice::from_raw_parts(
                &nt_params as *const NtCommitParams as *const u8,
                std::mem::size_of::<NtCommitParams>(),
            )
        };
        gpu.encode_1d(
            cmd,
            &gpu.pipelines.nt_concentration_commit,
            &[(&neurons.nt_conc_accum_i32, 0), (&neurons.nt_conc, 0)],
            Some((nt_bytes, 2)),
            (neuron_count * NTType::COUNT) as u64,
        );
...
pub fn dispatch_synapse_step(
    ...
    for i in 0..synapses.n_synapses {
        let cleft_conc = synapses.update_cleft(i, time, dt);
        crate::spike_propagation::expose_cleft_nt_to_postsynaptic_ambient(
            neurons, synapses, i, cleft_conc, dt,
        );
    }
}
