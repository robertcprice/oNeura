...
use metal::*;
#[cfg(target_os = "macos")]
use objc::{__send_message, runtime::Sel};
#[cfg(target_os = "macos")]
...
pub struct GpuPipelines {
    ...
    pub nt_concentration_commit: ComputePipelineState,
...
impl GpuContext {
    ...
    pub fn new() -> Result<Self, String> {
        ...
        let queue_selector = Sel::register("newCommandQueue");
        let queue_ptr: *mut MTLCommandQueue =
            unsafe { __send_message(device.as_ptr(), queue_selector, ()) }
                .map_err(|err| format!("Metal command queue creation failed: {err}"))?;
        if queue_ptr.is_null() {
            return Err("Metal command queue creation returned a null queue".to_string());
        }
        let queue = unsafe { CommandQueue::from_ptr(queue_ptr) };
    ...
    fn create_pipelines(device: &Device) -> Result<GpuPipelines, String> {
        ...
        Ok(GpuPipelines {
            ...
            nt_concentration_commit: make(
                include_str!("../metal/synapse_step.metal"),
                "commit_nt_concentration",
            )?,
