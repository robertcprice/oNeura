...
impl MetalNeuronState {
    pub fn from_cpu(gpu: &GpuContext, neurons: &NeuronArrays) -> Self {
        Self {
            ...
            nt_conc_accum_i32: {
                let len = neurons.count * NTType::COUNT;
                let buf = gpu.buffer_of_size((len * std::mem::size_of::<i32>()) as u64);
                unsafe {
                    fill_buffer_with(&buf, len, 0i32);
                }
                buf
            },
    ...
    pub fn sum_fired_range(&self, start: usize, end: usize) -> u32 {
        let lo = start.min(self.count);
        let hi = end.min(self.count);
        unsafe {
            let ptr = self.fired.contents() as *const u8;
            (lo..hi).map(|idx| *ptr.add(idx) as u32).sum()
        }
    }
    ...
    pub fn sum_spike_count_range(&self, start: usize, end: usize) -> u64 {
        let lo = start.min(self.count);
        let hi = end.min(self.count);
        unsafe {
            let ptr = self.spike_count.contents() as *const u32;
            (lo..hi).map(|idx| *ptr.add(idx) as u64).sum()
        }
    }
}
