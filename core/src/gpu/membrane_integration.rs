...
pub fn cpu_membrane_integration(
    ...
    for i in 0..neurons.count {
        ...
        // Mg2+ block for NMDA
        let mg_block = 1.0 / (1.0 + crate::constants::NMDA_MG_CONC_MM * (-0.062 * v).exp() / 3.57);
        ...
        let i_gabaa =
            channel_g_max[6] * cs[6] * neurons.gabaa_open[i] * (v - crate::constants::GABAA_E_REV);
        let i_nachr =
            channel_g_max[7] * cs[7] * neurons.nachr_open[i] * (v - crate::constants::NACHR_E_REV);
        ...
        let v_new = (v + dv).clamp(crate::constants::VOLTAGE_MIN, crate::constants::VOLTAGE_MAX);
