...
#[cfg(target_os = "macos")]
pub fn dispatch_receptor_binding(gpu: &GpuContext, neurons: &MetalNeuronState, dt: f32) {
    let cmd = gpu.new_command_buffer();
    encode_receptor_binding(gpu, &cmd, neurons, dt);
...
pub fn dispatch_receptor_binding(_gpu: &super::GpuContext, neurons: &mut NeuronArrays, dt: f32) {
    cpu_receptor_binding(neurons, dt);
}
...
pub fn cpu_receptor_binding(neurons: &mut NeuronArrays, dt: f32) {
    ...
    for i in 0..neurons.count {
        ...
        for nt in NTS {
            ...
            let decay_rate = 0.693 / nt.half_life_ms().max(1.0e-3);
            let relax = (1.0 - decay_rate * dt.max(0.0)).clamp(0.0, 1.0);
            let delta = neurons.nt_conc[i][idx] - rest;
            neurons.nt_conc[i][idx] = (rest + delta * relax).max(0.0);
        }
    }
}
