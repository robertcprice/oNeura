...
impl CircadianState {
    ...
    pub fn step(&mut self, dt: f32, daylight: f32, moonlight: f32, config: &ChronobiologyConfig) {
        ...
        let circadian_factor = 0.3
            + 0.7
                * (std::f32::consts::PI * subjective_hour / 12.0)
                    .sin()
                    .max(0.0);
...
impl FlyMetabolism {
    ...
    /// * `moonlight` - Current moonlight intensity (0-1)
    /// * `config` - Chronobiology configuration
    pub fn step_circadian(
        &mut self,
        dt: f32,
        daylight: f32,
        moonlight: f32,
        config: &ChronobiologyConfig,
    ) {
    ...
    /// Normalized energy fraction derived from the explicit reserve pools.
    pub fn energy_fraction(&self) -> f32 {
        (self.energy_compat_uj() / FLY_ENERGY_MAX).clamp(0.0, 1.0)
    }

    /// Advance all metabolic pathways by `dt` seconds.
    ///
    /// Temperature-dependent: all 6 Vmax rate constants scale via Eyring TST
    /// from bond dissociation energies (insect ectotherm coupling).
    pub fn step(&mut self, temperature_c: f32, dt: f32) {
        use crate::terrarium::emergent_rates::metabolome_rate;

        // Eyring TST-scaled Vmax values (literature reference at 25°C)
        let crop_vmax = metabolome_rate("fly_crop", temperature_c) as f32;
        let trehalase_vmax = metabolome_rate("fly_trehalase", temperature_c) as f32;
        let glycolysis_vmax = metabolome_rate("fly_glycolysis", temperature_c) as f32;
        let glycogenolysis_vmax = metabolome_rate("fly_glycogenolysis", temperature_c) as f32;
        let lipid_mob_vmax = metabolome_rate("fly_lipid_mobilization", temperature_c) as f32;
        let glycogen_storage_vmax = metabolome_rate("fly_glycogen_storage", temperature_c) as f32;
        ...
        let neural_atp_cost =
            self.neural_activity_fraction * BASAL_DEMAND_UW * UW_TO_MM_ATP_PER_S * 0.2 * dt;
        ...
        if self.hemolymph_glucose_mm < GLYCOGENOLYSIS_THRESHOLD {
            let glycogenolysis_flux = michaelis_menten(
                self.fat_body_glycogen_mg,
                glycogenolysis_vmax,
                GLYCOGENOLYSIS_KM,
            ) * dt;
...
impl OrganismMetabolism for FlyMetabolism {
    ...
    fn is_stressed(&self) -> bool {
        self.energy_charge() < 0.5 || self.hemolymph_trehalose_mm < 5.0 || self.muscle_atp_mm < 1.0
    }
...
mod tests {
    ...
    fn flight_depletes_atp_faster() {
        ...
        for _ in 0..100 {
            m_walk.step(25.0, 0.05);
            m_fly.step(25.0, 0.05);
        }
    ...
    fn energy_compat_within_range() {
        ...
        for _ in 0..500 {
            ...
            assert!(
                e >= 0.0 && e <= FLY_ENERGY_MAX,
                "energy_compat_uj={e} out of range"
            );
        }
    }
    ...
    fn o2_deprivation_reduces_atp_yield() {
        ...
        for _ in 0..200 {
            m_normal.step(25.0, 0.05);
            m_hypoxic.step(25.0, 0.05);
        }
        ...
        let hypoxic_consumed_more_fuel = m_hypoxic.hemolymph_trehalose_mm
            + m_hypoxic.hemolymph_glucose_mm
    ...
    fn neural_cost_depletes_atp() {
        ...
        m_base.step(25.0, 1.0);
        m_neural.step(25.0, 1.0);
    ...
    fn circadian_step_advances_phase() {
        ...
        assert!(
            (m.circadian.phase_hours - 6.0).abs() < 0.1,
            "phase should be ~6h after 6h step, got {}",
            m.circadian.phase_hours
        );
    }
    ...
    fn circadian_activity_peaks_at_subjective_day() {
        ...
        assert!(
            m_noon.circadian.activity_level >= m_morning.circadian.activity_level,
            "noon activity >= morning activity"
        );
        assert!(
            m_noon.circadian.activity_level >= m_midnight.circadian.activity_level,
    ...
    fn moonlight_boosts_nocturnal_activity() {
        ...
        assert!(
            ...
            m_moonlit.circadian.activity_level,
            m_dark.circadian.activity_level
        );
    }
    ...
    fn rest_accumulates_during_inactive_periods() {
        ...
        assert!(
            m.circadian.rest_accumulation > 0.0,
    ...
    #[test]
    fn fly_metabolism_cold_slows_down() {
        let mut m_cold = FlyMetabolism::default();
        let mut m_warm = FlyMetabolism::default();
        m_cold.set_activity(FlyActivity::Walking(0.5));
        m_warm.set_activity(FlyActivity::Walking(0.5));
        for _ in 0..200 {
            m_cold.step(10.0, 0.05);
            m_warm.step(25.0, 0.05);
        }
        // Cold fly should have consumed less glucose (slower enzyme kinetics).
        assert!(
            m_cold.hemolymph_trehalose_mm > m_warm.hemolymph_trehalose_mm,
            "Cold fly should retain more trehalose: cold={:.3}, warm={:.3}",
            m_cold.hemolymph_trehalose_mm, m_warm.hemolymph_trehalose_mm
        );
    }

    #[test]
    fn fly_metabolism_warm_speeds_up() {
        let mut m_hot = FlyMetabolism::default();
        let mut m_ref = FlyMetabolism::default();
        m_hot.set_activity(FlyActivity::Walking(0.5));
        m_ref.set_activity(FlyActivity::Walking(0.5));
        for _ in 0..200 {
            m_hot.step(35.0, 0.05);
            m_ref.step(25.0, 0.05);
        }
        // Hot fly should consume circulating sugars faster (higher enzyme kinetics).
        let hot_pool = m_hot.hemolymph_trehalose_mm + m_hot.hemolymph_glucose_mm;
        let ref_pool = m_ref.hemolymph_trehalose_mm + m_ref.hemolymph_glucose_mm;
        assert!(
            hot_pool < ref_pool,
            "Hot fly should deplete circulating sugars faster: hot={:.3}, ref={:.3}",
            hot_pool, ref_pool,
        );
    }

    #[test]
    fn fly_trehalase_temperature_sensitivity() {
        // Verify ~2× or greater rate change per 10°C (ectotherm Q10).
        // Measure trehalose consumption over identical time windows.
        let mut m_15 = FlyMetabolism::default();
        let mut m_25 = FlyMetabolism::default();
        m_15.set_activity(FlyActivity::Resting);
        m_25.set_activity(FlyActivity::Resting);
        let tre_start = m_15.hemolymph_trehalose_mm;
        for _ in 0..100 {
            m_15.step(15.0, 0.05);
            m_25.step(25.0, 0.05);
        }
        let consumed_15 = tre_start - m_15.hemolymph_trehalose_mm;
        let consumed_25 = tre_start - m_25.hemolymph_trehalose_mm;
        // Eyring TST with CO bond at eff=0.91 gives Q10 > 2.
        assert!(
            consumed_25 > consumed_15 * 1.5,
            "Q10 should show >1.5× faster at 25°C vs 15°C: consumed_25={:.4}, consumed_15={:.4}",
            consumed_25, consumed_15
        );
    }

    #[test]
    fn fly_metabolism_extreme_cold_survives() {
        // At 0°C: no NaN, no panic, near-zero flux but pools stay bounded.
        let mut m = FlyMetabolism::default();
        m.set_activity(FlyActivity::Resting);
        for _ in 0..500 {
            m.step(0.0, 0.05);
            assert!(!m.crop_sugar_mg.is_nan(), "crop NaN at 0°C");
            assert!(!m.hemolymph_trehalose_mm.is_nan(), "trehalose NaN at 0°C");
            assert!(!m.hemolymph_glucose_mm.is_nan(), "glucose NaN at 0°C");
            assert!(!m.muscle_atp_mm.is_nan(), "ATP NaN at 0°C");
            assert!(m.muscle_atp_mm >= 0.0);
        }
    }
}
