...
//!
//! ```rust
//! use oneura_core::horizontal_gene_transfer::antibiotic_resistance_spread;
//!
...
pub enum GeneticElementType {
    ...
    Transposon { target_specificity: f64 },
    ...
    Phage { burst_size: u32, lysogeny_rate: f64 },
...
impl HgtPopulation {
    ...
    fn selection(&mut self, dt: f64) {
        ...
        for cell in &self.cells {
            ...
            let survival =
                (relative_fitness * (1.0 - death_prob) * (1.0 - antibiotic_death)).clamp(0.0, 1.0);
...
mod tests {
    ...
    fn element_loss_over_time() {
        ...
        let initial = pop.element_frequency(eid);
        assert!(initial > 0.90, "Should start near 100%, got {:.3}", initial);
    ...
    fn transposon_element_creation() {
        ...
        match &elem.element_type {
            GeneticElementType::Transposon { target_specificity } => {
                assert!(
