...
fn monod_substrate_stays_bounded() {
    ...
    assert!(snap.mean_glucose >= 0.0, "glucose went negative: {}", snap.mean_glucose);
    assert!(snap.mean_oxygen_gas >= 0.0, "O2 went negative: {}", snap.mean_oxygen_gas);
    assert!(snap.mean_atp_flux >= 0.0, "ATP went negative: {}", snap.mean_atp_flux);
    assert!(snap.mean_hydration > 0.0, "hydration went to zero: {}", snap.mean_hydration);
    // Activity fields should stay in the Monod-allowed range [0.001, 5.0].
    assert!(snap.mean_microbes >= 0.001, "heterotroph activity collapsed: {}", snap.mean_microbes);
    assert!(snap.mean_microbes <= 5.0, "heterotroph activity exploded: {}", snap.mean_microbes);
}
...
fn monod_guild_activity_controls_nitrogen_transforms() {
    ...
    nitrify.set_microbial_activity_field(&vec![0.08; total]).unwrap();
    nitrify.set_nitrifier_activity_field(&vec![1.0; total]).unwrap();
    nitrify.set_denitrifier_activity_field(&vec![0.0; total]).unwrap();
    ...
    denit.set_microbial_activity_field(&vec![0.10; total]).unwrap();
    denit.set_nitrifier_activity_field(&vec![0.0; total]).unwrap();
    denit.set_denitrifier_activity_field(&vec![1.0; total]).unwrap();
...
fn monod_starved_heterotrophs_decay() {
    ...
    assert!(initial_activity > 0.01, "initial activity too low to test: {}", initial_activity);
    t.run(4000, 1000.0);
    ...
    assert!(
        ...
        initial_activity, final_activity
    );
}
