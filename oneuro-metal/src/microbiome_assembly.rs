...
impl CommunityAssembler {
    ...
    pub fn diversity(&self) -> DiversityMetrics {
        ...
        let richness = abundances.iter().filter(|&&x| x > EXTINCTION_THRESHOLD).count();
    ...
    fn compute_nodf(matrix: &[Vec<bool>]) -> f64 {
        ...
        for i in 0..n_rows {
            for j in (i + 1)..n_rows {
                ...
                for c in 0..n_cols {
                    let rich_present = rich < matrix.len()
                        && c < matrix[rich].len()
                        && matrix[rich][c];
                    let poor_present = poor < matrix.len()
                        && c < matrix[poor].len()
                        && matrix[poor][c];
    ...
    pub fn stability(
        ...
        let variance = if totals.len() > 1 {
            totals.iter().map(|&t| (t - mean_total).powi(2)).sum::<f64>()
                / (totals.len() - 1) as f64
        } else {
            0.0
        };
    ...
    pub fn detect_syntrophy(&self) -> Vec<(u64, u64)> {
        ...
        for link in &self.cross_feeding {
            if let Some(consumer_idx) = self.taxon_index(link.consumer) {
                ...
                let has_abiotic_growth = consumer
...
mod tests {
    ...
    fn jaccard_similarity_known() {
        ...
        assert!((j2 - 1.0).abs() < 1e-10, "Identical sets: J=1.0, got {}", j2);
