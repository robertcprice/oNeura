    ...
    for i in 0..n {
        ...
        if has_fruit[i] {
            ...
            if current > 0.0 {
                ...
                decay_loss = current.min(
                    decay_driver * (0.00008 * dt) * (1.0 + microbial_biomass[i].max(0.0) * 8.0),
                );
                if decay_loss > 0.0 {
...
pub fn step_seed_bank(
    ...
    for i in 0..n {
        ...
        if dormancy > 0.0 {
            ...
            if germination_score > 0.72 && plant_count < max_plants {
                germinate_seed = true;
                plant_count += 1;
                scale = clamp(0.50 + reserve_carbon[i] * 2.8, 0.45, 1.10);
            } else if dormancy > -28000.0 && age < 200000.0 {
                keep_seed = true;
            }
        }
