...
pub fn build_tripeptide_gag() -> EmbeddedMolecule {
    ...
    let positions: Vec<[f32; 3]> = vec![
        [-3.0, 0.0, 0.0], [-1.5, 0.0, 0.0], [-0.7, 1.2, 0.0], [-1.2, 2.3, 0.0],
        [-3.5, 0.8, 0.0], [-3.5, -0.8, 0.0], [-1.2, -0.5, 0.9], [-1.2, -0.5, -0.9],
        [0.6, 1.1, 0.0], [1.5, 2.2, 0.0], [2.9, 1.7, 0.0], [3.3, 0.5, 0.0],
        [1.3, 3.0, 1.2], [0.9, 0.2, 0.0], [1.3, 2.8, -0.9], [0.3, 3.4, 1.2],
        [2.0, 3.8, 1.2], [1.4, 2.5, 2.1],
        [3.7, 2.7, 0.0], [5.1, 2.5, 0.0], [5.9, 3.7, 0.0], [5.5, 4.8, 0.0],
        [7.1, 3.5, 0.0], [3.3, 3.6, 0.0], [5.4, 2.0, 0.9], [5.4, 2.0, -0.9],
        [7.5, 4.4, 0.0],
    ];
...
pub fn build_cellulase_fragment() -> EmbeddedMolecule {
    ...
    let positions: Vec<[f32; 3]> = vec![
        [-4.0, 0.0, 0.0], [-2.5, 0.0, 0.0], [-1.7, 1.2, 0.0], [-2.2, 2.3, 0.0],
        [-2.3, -0.8, 1.3], [-2.0, -0.2, 2.6], [-1.7, -1.0, 3.8], [-2.0, -2.2, 3.8],
        [-1.2, -0.3, 4.9], [-4.5, 0.8, 0.0], [-4.5, -0.8, 0.0], [-2.2, -0.5, -0.9],
        [-3.0, -1.5, 1.3], [-1.5, -0.2, 1.3], [-2.8, 0.5, 2.8], [-1.2, -0.8, 2.6],
        [-1.0, 0.4, 5.7],
        [0.0, 1.0, 0.0], [1.0, 2.0, 0.0], [2.4, 1.5, 0.0], [2.8, 0.3, 0.0],
        [0.8, 2.8, 1.3], [0.5, 2.2, 2.5], [0.8, 3.3, 2.8], [0.0, 1.5, 3.2],
        [3.5, 2.3, 0.0], [-0.3, 0.2, 0.0], [1.2, 2.6, -0.9], [0.0, 3.5, 1.1],
        [1.5, 2.5, 1.5], [-0.5, 0.8, 3.8], [4.0, 3.0, 0.0], [0.8, 1.5, -0.9],
        [-4.0, 0.0, 0.9], [0.5, 0.3, 0.0],
    ];
...
pub fn build_urease_fragment() -> EmbeddedMolecule {
    ...
    let positions: Vec<[f32; 3]> = vec![
        [-4.0, 0.0, 0.0], [-2.5, 0.0, 0.0], [-1.7, 1.2, 0.0], [-2.2, 2.3, 0.0],
        [-2.3, -0.8, 1.3], [-2.0, -0.2, 2.6], [-1.4, 0.9, 2.8], [-0.9, 1.2, 4.0],
        [-1.1, 0.2, 4.9], [-1.7, -0.8, 4.0], [-4.5, 0.8, 0.0], [-4.5, -0.8, 0.0],
        [-2.2, -0.5, -0.9], [-3.0, -1.5, 1.3], [-0.8, 0.3, 5.8],
        [0.0, 1.0, 0.0], [1.0, 2.0, 0.0], [2.4, 1.5, 0.0], [2.8, 0.3, 0.0],
        [0.8, 2.8, 1.3], [0.5, 2.2, 2.5], [0.8, 3.3, 2.8], [0.0, 1.5, 3.2],
        [3.5, 2.3, 0.0], [-0.3, 0.2, 0.0], [1.2, 2.6, -0.9], [0.0, 3.5, 1.1],
        [1.5, 2.5, 1.5], [-0.5, 0.8, 3.8], [4.0, 3.0, 0.0],
    ];
...
pub fn build_phosphatase_fragment() -> EmbeddedMolecule {
    ...
    let positions: Vec<[f32; 3]> = vec![
        [-3.0, 0.0, 0.0], [-1.5, 0.0, 0.0], [-0.7, 1.2, 0.0], [-1.2, 2.3, 0.0],
        [-1.2, -0.8, 1.2], [-0.5, -0.2, 2.3], [-3.5, 0.8, 0.0], [-3.5, -0.8, 0.0],
        [-1.2, -0.5, -0.9], [-1.8, -1.5, 1.0], [-0.4, -1.3, 1.5], [-0.8, 0.6, 2.8],
        [0.6, 1.0, 0.0], [1.5, 2.0, 0.0], [2.9, 1.5, 0.0], [3.3, 0.3, 0.0],
        [1.3, 2.8, 1.3], [1.0, 2.2, 2.5], [1.3, 3.3, 2.8], [0.5, 1.5, 3.2],
        [3.6, 2.3, 0.0], [0.2, 0.2, 0.0], [1.7, 2.6, -0.9], [0.5, 3.5, 1.1],
        [1.8, 2.5, 1.5], [0.0, 0.8, 3.8], [4.1, 3.0, 0.0], [1.3, 1.5, -0.9],
    ];
...
pub fn score_enzyme_drug_properties(mol: &EmbeddedMolecule) -> f32 {
    let mw: f32 = mol.graph.atoms.iter().map(|a| a.element.mass_daltons()).sum();
    let heavy_atoms = mol.graph.atoms.iter().filter(|a| a.element != PeriodicElement::H).count();
    let mw_score: f32 = if mw < 500.0 { 1.0 } else { ((700.0 - mw) / 200.0).clamp(0.0, 1.0) };
...
pub fn apply_probe_catalytic_feedback(world: &mut TerrariumWorld) {
    ...
    let probe_data: Vec<(usize, usize, usize, f32, f32)> = world.probes().iter().map(|probe| {
        ...
        let catalytic_factor = if t < 270.0 {
            ...
            ((370.0 - t) / 30.0).max(0.0)
        };
        (probe.grid_x, probe.grid_y, probe.footprint_radius, catalytic_factor, stats.total_energy)
    }).collect();

    for (gx, gy, r, catalytic_factor, total_energy) in probe_data {
        if total_energy.abs() < 1e-10 { continue; }
        ...
        for cy in y_lo..y_hi {
            for cx in x_lo..x_hi {
                let idx = cy * w + cx;
                if idx >= world.organic_matter.len() { continue; }
                ...
                world.moisture[idx] = (world.moisture[idx] + 0.001 * catalytic_factor).min(1.0);
            }
        }
    }
}
...
pub fn compute_enzyme_efficacy(world: &TerrariumWorld) -> f32 {
    ...
    for probe in world.probes() {
        let stability = (1.0 / (1.0 + probe.last_stats.total_energy.abs() * 0.001)).min(1.0);
        let thermal = if probe.last_stats.temperature > 250.0 && probe.last_stats.temperature < 350.0 {
            1.0
        } else {
            0.0
        };
...
mod tests {
    ...
    fn enzyme_drug_score_lipinski() {
        ...
        assert!(score > 0.0, "Drug score should be positive for small peptide");
        assert!(score <= 1.0, "Drug score should be <= 1.0");
    }

    #[test]
    fn enzyme_probe_seeded_in_world() {
        use crate::terrarium_world::{TerrariumWorld, TerrariumWorldConfig};
    ...
    fn enzyme_catalytic_feedback_decomposes() {
        ...
        if om_before > 0.0 {
            assert!(om_after <= om_before, "Organic matter should decrease from enzyme catalysis");
        }
    }
