...
impl BiofilmSimulator {
        ...
        for _ in 0..n_sub {
            ...
            for r in 0..rows {
                for c in 0..cols {
                    ...
                    let laplacian = (up + down + left + right
                        - 4.0 * self.nutrients.concentration[r][c])
                        / h2;

                    new_conc[r][c] =
                        (self.nutrients.concentration[r][c] + d_eff * laplacian * dt_sub)
                            .max(0.0);
                }
            }
    ...
    pub fn run_simulation(&mut self, steps: usize, dt: f64) -> BiofilmTimeSeries {
        ...
        for cell in &self.cells {
            ts.species_counts.entry(cell.species).or_insert_with(Vec::new);
        }
    ...
    pub fn mushroom_structure_count(&self) -> usize {
        ...
        for c in 0..self.width {
            let col_profile: Vec<f64> = (0..self.height)
                .map(|r| self.eps.density[r][c])
                .collect();
...
mod tests {
    ...
    fn seed_cells_adds_cells() {
        ...
        for cell in &sim.cells {
            assert!(cell.y < 2.0 * sim.cell_size + 0.01,
                "Cell y={} exceeds initial attachment zone", cell.y);
            assert!(cell.alive);
        }
    }
    ...
    fn eps_production_increases_over_time() {
        ...
        let total_eps: f64 = sim.eps.density.iter()
            .flat_map(|row| row.iter())
            .sum();
        assert!(total_eps > 0.0, "Total EPS density should be positive after simulation");
    }
    ...
    fn antibiotic_kills_planktonic_more_than_biofilm() {
        ...
        let biofilm_survival = (biofilm_count_before - biofilm_killed) as f64
            / biofilm_count_before as f64;
        let planktonic_survival = (planktonic_count_before - planktonic_killed) as f64
            / planktonic_count_before as f64;
    ...
    fn nutrient_gradient_forms() {
        ...
        let top_mean: f64 = profile.last().unwrap().iter().sum::<f64>()
            / profile.last().unwrap().len() as f64;
    ...
    fn catheter_preset_has_shear() {
        ...
        assert!(sim.shear_stress > 0.0, "Catheter biofilm should have flow shear");
    ...
    fn run_simulation_records_time_series() {
        ...
        for i in 1..ts.times.len() {
            assert!(ts.times[i] > ts.times[i - 1], "Time must increase monotonically");
        }

        // Species counts should be tracked
        assert!(ts.species_counts.contains_key(&0), "Species 0 should be tracked");
    }
}
