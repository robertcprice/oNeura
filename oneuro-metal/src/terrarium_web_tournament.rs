...
pub async fn submit_genome(
    ...
    tokio::task::spawn_blocking(move || {
        evaluate_genome_sync(id, genome, &eval_config)
    })
    .await
    .ok()
...
fn dominates(a: &TournamentFitness, b: &TournamentFitness) -> bool {
    let objs_a = [a.biomass, a.biodiversity, a.stability, a.carbon, a.fruit, a.microbial];
    let objs_b = [b.biomass, b.biodiversity, b.stability, b.carbon, b.fruit, b.microbial];
...
/// Get a genome by ID.
pub async fn get_genome(
    tournament: &Arc<Mutex<TournamentState>>,
    id: u32,
) -> Option<WorldGenome> {
    let state = tournament.lock().await;
    state.entries.iter().find(|e| e.id == id).map(|e| e.genome.clone())
}
