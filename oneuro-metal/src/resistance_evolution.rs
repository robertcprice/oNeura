...
pub enum ResistanceEvent {
    ...
    DeNovoMutation { strain_id: u64, mechanism_idx: usize },
    /// Resistance gene horizontally transferred between strains.
    HorizontalTransfer { donor: u64, recipient: u64, mechanism_idx: usize },
    ...
    ResistanceLoss { strain_id: u64, mechanism_idx: usize },
    ...
    ClinicalResistance { antibiotic_idx: usize, generation: u32 },
}
...
pub fn selection_coefficient(
    ...
    if effective_s.abs() < 1e-15 {
        ...
        if effective_r > 0.0 { 1.0 } else { 0.0 }
    } else {
...
impl ResistanceSimulator {
    ...
    fn strain_fitness(&self, strain: &Strain, concentrations: &[f64]) -> f64 {
        ...
        let raw_cost: f64 = strain.resistance_mechanisms.iter()
    ...
    pub fn mdr_profile(&self, strain: &Strain) -> MdrProfile {
        ...
        let total_classes = self.antibiotics.iter()
    ...
    pub fn step(&mut self, antibiotic_concentrations: &[f64]) -> Vec<ResistanceEvent> {
        ...
        for i in 0..n_strains {
            if self.rng_bernoulli(self.mutation_rate) && !self.mechanisms.is_empty() {
                ...
                if !parent_mechs.contains(&mech_idx) {
                    ...
                    let fitness_at_acq = self.strain_fitness(&new_strain, antibiotic_concentrations);
        ...
        if n_strains >= 2 {
            ...
            for _ in 0..hgt_attempts {
                ...
                if donor_idx == recip_idx { continue; }
                ...
                for &mech_idx in &donor_mechs {
                    if mech_idx >= self.mechanisms.len() { continue; }
                    if !self.mechanisms[mech_idx].transferable { continue; }
                    if recip_mechs.contains(&mech_idx) { continue; }
        ...
        for i in 0..self.strains.len() {
            ...
            if !strain.resistance_mechanisms.is_empty() && self.rng_bernoulli(self.compensatory_rate) {
        ...
        for i in 0..self.strains.len() {
            ...
            for (j, &mech_idx) in mech_indices.iter().enumerate() {
                if mech_idx < self.mechanisms.len() {
                    ...
                    let under_selection = self.antibiotics.iter().enumerate().any(|(ab_idx, ab)| {
        ...
        let fitnesses: Vec<f64> = self.strains.iter()
            .map(|(s, _)| self.strain_fitness(s, antibiotic_concentrations))
            .collect();

        let total_fitness: f64 = self.strains.iter().zip(fitnesses.iter())
        ...
        self.strains.retain(|(_, freq)| *freq >= extinction_threshold);
        ...
        for (ab_idx, tracker) in self.mic_trackers.iter_mut().enumerate() {
            ...
            let pop_mic: f64 = self.strains.iter()
                .map(|(s, freq)| {
                    let mut fold = 1.0_f64;
                    ...
                    freq * self.antibiotics.get(ab_idx)
    ...
    pub fn predict_resistance_time(
        ...
        let best_mech = self.mechanisms.iter()
        ...
        let s = selection_coefficient(
            1.0, 1.0, mech.fitness_cost,
            concentration, ab.mic_wild_type, mic_r,
        );
    ...
    fn detect_valley_crossings_internal(&mut self, concentrations: &[f64]) {
        ...
        for (strain, freq) in &self.strains {
                ...
                && current_fitness > ancestor_fitness
            {
                ...
                let first_acq = self.acquisition_log.iter()
    ...
    pub fn mutant_selection_window(&self, antibiotic_idx: usize) -> (f64, f64) {
        ...
        let max_fold = self.mechanisms.iter()
            .filter(|m| m.target_classes.contains(&ab.class))
...
mod tests {
    ...
    fn hgt_transfers_resistance() {
        ...
        assert!(any_hgt, "HGT should occur with transferable mechanism and high rate");
    }
    ...
    fn cycling_delays_resistance() {
        ...
        let constant_mic = sim_constant.mic_trackers[cipro_idx].mic_history
            .last()
            .map(|(_, m)| *m)
            .unwrap_or(0.0);
        ...
        let cycling_mic = sim_cycling.mic_trackers[cipro_idx_c].mic_history
            .last()
            .map(|(_, m)| *m)
            .unwrap_or(0.0);
    ...
    fn resistance_loss_without_selection() {
        ...
        let susceptible_freq: f64 = sim.strains.iter()
        ...
        let any_susceptible = susceptible_freq > 0.0
            || sim.strains.iter().any(|(s, _)| s.resistance_mechanisms.is_empty());
        assert!(
    ...
    fn predict_resistance_time_returns_estimate() {
        ...
        assert!(
            time.is_some(),
            "Should predict resistance time within MSW"
        );
        let t = time.unwrap();
        ...
        assert!(
            time_none.is_none(),
            "Should return None with no drug"
        );
    }
