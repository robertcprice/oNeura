...
impl WrightFisherSim {
    ...
    pub fn new(pop_size: usize, initial_freq: f64, seed: u64) -> Self {
        Self {
            ...
            rng_state: if seed == 0 { 0xCAFE_BABE_1234_5678 } else { seed },
        }
    }
    ...
    pub fn step(&mut self) -> f64 {
        ...
        let p_after_mut = p * (1.0 - self.mutation_rate_forward)
            + (1.0 - p) * self.mutation_rate_backward;
        ...
        let w_bar = p_after_mut * p_after_mut * w_aa
            + 2.0 * p_after_mut * q * w_ab
            + q * q * w_bb;
...
mod tests {
    ...
    fn wright_fisher_drift() {
        ...
        for _ in 0..200 {
            let freq = sim.step();
            assert!(freq >= 0.0 && freq <= 1.0, "frequency out of range: {}", freq);
            if (freq - 0.5).abs() > 0.1 {
                changed = true;
            }
        }
    ...
    fn hardy_weinberg_departure() {
        ...
        assert!(
            !result.excess_het,
            "should show deficit of heterozygotes"
        );
    }
