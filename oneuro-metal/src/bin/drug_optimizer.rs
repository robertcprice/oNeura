...
fn main() {
    ...
    match mode {
        ...
        _ => {
            eprintln!("Unknown mode '{}'. Use --mode compare|validate|optimize|scan", mode);
            std::process::exit(1);
        }
    }
}
...
fn run_compare(args: &[String]) {
    ...
    let protocols = vec![
        ...
        ("Pulsed (3h on / 2h off x3)", DrugProtocol::pulsed(kill_rate, 3.0, 2.0, 3)),
        ("Pulsed (5h on / 1h off x5)", DrugProtocol::pulsed(kill_rate, 5.0, 1.0, 5)),
        ("Combination (A+B, 5h each)", DrugProtocol::combination(kill_rate, kill_rate * 1.5, 5.0)),
        ("High-dose short (2x kill, 5h)", DrugProtocol::single(kill_rate * 2.0, 5.0)),
    ];

    let protocol_list: Vec<DrugProtocol> = protocols.iter().map(|(_, p)| p.clone()).collect();
    let (best_idx, results) = optimize_drug_protocol(&protocol_list, switching_rate, dt);

    if json_output {
        let output: Vec<serde_json::Value> = protocols.iter().zip(results.iter()).enumerate()
            .map(|(i, ((name, _), result))| {
                serde_json::json!({
                    "name": name,
                    "is_best": i == best_idx,
...
fn run_validate() {
    ...
    println!();
    println!("{:<20} {:>12} {:>12} {:>10}", "Switch Rate", "Expected", "Simulated", "Valid?");
    println!("{}", "-".repeat(58));

    let results = validate_against_ecoli();
    let mut all_valid = true;
    for (switch_rate, expected, actual, within_order) in &results {
        println!("{:<20.2e} {:>12.4} {:>12.4} {:>10}",
            switch_rate, expected, actual,
            if *within_order { "YES" } else { "FAIL" },
        );
        if !within_order { all_valid = false; }
    }
    ...
    for (switch_rate, _, actual, _) in &results {
        ...
        println!("  {:.2e} -> survival {:.6} {}", switch_rate, actual, if ok { "OK" } else { "VIOLATED" });
        prev_actual = *actual;
    }
    println!("Monotonicity: {}", if monotonic { "PASSED" } else { "FAILED" });
}
...
fn run_optimize(args: &[String]) {
    ...
    println!("{:<40} {:>12} {:>12} {:>8}", "Protocol", "Final Pop", "Survival%", "Eradic");
    println!("{}", "-".repeat(75));
        ...
        for &rest_h in &rest_hours_range {
            for n_cycles in 1..=max_cycles {
                ...
                if is_better || result.eradication_achieved {
                    println!("{:<40} {:>12.1} {:>11.4}% {:>8}",
                        name, result.final_population,
                        result.survival_fraction * 100.0,
                        if result.eradication_achieved { "YES" } else { "no" },
                    );
                }
    ...
    if let Some((name, result)) = &best_protocol {
        println!();
        println!("OPTIMAL: {} (survival {:.6}%)", name, result.survival_fraction * 100.0);
    }
}
...
fn run_scan(args: &[String]) {
    ...
    if let Some(output_path) = args.iter().position(|a| a == "--output").and_then(|i| args.get(i + 1)) {
        std::fs::write(output_path, serde_json::to_string_pretty(&scan_results).unwrap())
            .expect("Failed to write output file");
        if !json_output {
            println!();
            println!("Results written to {}", output_path);
