    ...
    match scenario {
        ...
        Scenario::DroughtSurvival => {
            ...
            for i in 0..16 {
                ...
                let x = (cx as f32 + angle.cos() * r).clamp(1.0, (world.config.width - 2) as f32) as usize;
                let y = (cy as f32 + angle.sin() * r).clamp(1.0, (world.config.height - 2) as f32) as usize;
                let _ = world.add_plant(x, y, None, None);
            }
            // Few flies
            for i in 0..4 {
        ...
        Scenario::PopulationBoom => {
            ...
            for i in 0..4 {
                let x = 5 + i * 10;
                let y = 5 + i * 7;
                world.add_water(x.min(world.config.width - 2), y.min(world.config.height - 2), 200.0, 0.001);
            }
            for i in 0..10 {
        ...
        Scenario::MicrobialWorld => {
            ...
            for i in 0..4 {
                let x = 8 + i * 10;
                let y = 8 + i * 6;
                let _ = world.add_plant(x.min(world.config.width - 2), y.min(world.config.height - 2), None, None);
            }
