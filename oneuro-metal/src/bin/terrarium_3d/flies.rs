...
pub fn build_fly_meshes(flies: &[DrosophilaSim], gw: usize, gh: usize, moisture: &[f32], seed: u64, frame: usize) -> Vec<Triangle> {
    ...
    for (i, fly) in flies.iter().enumerate() {
        ...
        let mut thorax = make_diamond(center, 0.04 * FLY_SCALE, 0.025 * FLY_SCALE, 0.05 * FLY_SCALE, body_color, 16.0);
        ...
        let mut abd = make_diamond(abd_pos, 0.035 * FLY_SCALE, 0.02 * FLY_SCALE, 0.06 * FLY_SCALE, [body_color[0] * 0.85, body_color[1] * 0.85, body_color[2] * 0.85], 16.0);
        ...
        if b.is_flying {
            ...
            let mut left_wing = make_diamond(lw, 0.035 * FLY_SCALE, 0.005 * FLY_SCALE, 0.025 * FLY_SCALE, wing_color, 8.0);
            tag_all(&mut left_wing, EntityTag::Fly(i));
            tris.extend(left_wing);

            // Right wing
            let rw = add3(center, v3(0.04 * FLY_SCALE, wing_y, 0.0));
            let mut right_wing = make_diamond(rw, 0.035 * FLY_SCALE, 0.005 * FLY_SCALE, 0.025 * FLY_SCALE, wing_color, 8.0);
