...
pub fn export_json(world: &TerrariumWorld, snapshot: &TerrariumWorldSnapshot, frame: usize) -> Result<String, String> {
    let ts = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_secs())
        .unwrap_or(0);
    ...
    let waters_json: Vec<String> = world.waters.iter().enumerate().map(|(i, w)| {
        format!(r#"{{"id":{},"x":{},"y":{},"volume":{:.4},"alive":{}}}"#, i, w.x, w.y, w.volume, w.alive)
    }).collect();

    let fruits_json: Vec<String> = world.fruits.iter().enumerate().map(|(i, f)| {
        format!(
            r#"{{"id":{},"x":{},"y":{},"sugar":{:.4},"ripeness":{:.4},"alive":{}}}"#,
            i, f.source.x, f.source.y, f.source.sugar_content, f.source.ripeness, f.source.alive
        )
    }).collect();
    ...
    let json = format!(
        ...
        frame, ts,
        snapshot.light, snapshot.humidity, snapshot.temperature,
        snapshot.mean_soil_moisture, snapshot.mean_atmospheric_co2, snapshot.mean_atmospheric_o2,
        snapshot.moonlight, snapshot.tidal_moisture_factor,
        snapshot.plants, snapshot.flies, snapshot.seeds, snapshot.fruits,
        plants_json.join(","), flies_json.join(","), waters_json.join(","), fruits_json.join(","),
        soil_json.join(","),
    );
...
pub fn export_metrics(snapshot: &TerrariumWorldSnapshot, frame: usize) -> Result<String, String> {
    ...
    w(&mut f, "plants_total", "Number of living plants", snapshot.plants as f64)?;
    w(&mut f, "flies_total", "Number of living flies", snapshot.flies as f64)?;
    w(&mut f, "seeds_total", "Number of seeds", snapshot.seeds as f64)?;
    w(&mut f, "fruits_total", "Number of fruits", snapshot.fruits as f64)?;
    w(&mut f, "light", "Day/night light level", snapshot.light as f64)?;
    w(&mut f, "humidity", "Environmental humidity", snapshot.humidity as f64)?;
    w(&mut f, "temperature_celsius", "Temperature in Celsius", snapshot.temperature as f64)?;
    w(&mut f, "soil_moisture_mean", "Mean soil moisture", snapshot.mean_soil_moisture as f64)?;
    w(&mut f, "deep_moisture_mean", "Mean deep moisture", snapshot.mean_deep_moisture as f64)?;
    w(&mut f, "cell_vitality_mean", "Mean cell vitality", snapshot.mean_cell_vitality as f64)?;
    w(&mut f, "cell_energy_mean", "Mean cell energy", snapshot.mean_cell_energy as f64)?;
    w(&mut f, "microbes_mean", "Mean microbial density", snapshot.mean_microbes as f64)?;
    w(&mut f, "symbionts_mean", "Mean symbiont density", snapshot.mean_symbionts as f64)?;
    w(&mut f, "atmospheric_co2", "Mean atmospheric CO2", snapshot.mean_atmospheric_co2 as f64)?;
    w(&mut f, "atmospheric_o2", "Mean atmospheric O2", snapshot.mean_atmospheric_o2 as f64)?;
    w(&mut f, "moonlight", "Moonlight intensity", snapshot.moonlight as f64)?;
    w(&mut f, "tidal_factor", "Tidal moisture factor", snapshot.tidal_moisture_factor as f64)?;
    w(&mut f, "plant_cells_total", "Total plant cells", snapshot.total_plant_cells as f64)?;
    w(&mut f, "food_remaining", "Total food remaining", snapshot.food_remaining as f64)?;
