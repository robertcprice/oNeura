...
use oneuro_metal::plant_cellular::PlantTissue;
use super::math::*;
use super::mesh::*;
        ...
        EntityTag::Plant(idx) => {
            if idx < world.plants.len() {
                ...
                let tissues = [
                    ...
                    (PlantTissue::Meristem, [0.85, 0.92, 0.35], v3(0.0, 0.22, 0.0)),
                ];
...
pub fn build_cellular_detail(world: &TerrariumWorld, sel: &Selection, focus: V3) -> Vec<Triangle> {
    ...
    match sel.tag {
        EntityTag::Plant(idx) => {
            if idx < world.plants.len() {
                let plant = &world.plants[idx];
                let tissues = [PlantTissue::Leaf, PlantTissue::Stem, PlantTissue::Root, PlantTissue::Meristem];
...
pub fn build_molecular_detail(world: &TerrariumWorld, _sel: &Selection, focus: V3) -> Vec<Triangle> {
    let mut tris = Vec::new();
