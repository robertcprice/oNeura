...
use oneuro_metal::{
    scan_fitness_landscape, LandscapeAxis, WorldGenome, FitnessObjective,
    GENOME_PARAM_NAMES,
};
use std::env;
use std::fs;
...
fn parse_args() -> Args {
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            ...
            "--resolution" | "-r" => {
                args.resolution = argv.get(i + 1).and_then(|s| s.parse().ok()).unwrap_or(args.resolution);
                i += 1;
            }
            "--frames" | "-f" => {
                args.frames = argv.get(i + 1).and_then(|s| s.parse().ok()).unwrap_or(args.frames);
                i += 1;
            }
            "--seed" | "-s" => {
                args.seed = argv.get(i + 1).and_then(|s| s.parse().ok()).unwrap_or(args.seed);
                i += 1;
            }
            "--fitness" => {
...
fn print_help() {
    ...
    println!("  terrarium_landscape --x respiration_vmax --y photosynthesis_vmax -r 10 -o landscape.csv");
...
fn main() {
    ...
    let x_idx = match GENOME_PARAM_NAMES.iter().position(|&n| n == args.x_param) {
        Some(i) => i,
        None => {
            eprintln!("Unknown x parameter '{}'. Use --list-params to see available names.", args.x_param);
            std::process::exit(1);
        }
    };
    let y_idx = match GENOME_PARAM_NAMES.iter().position(|&n| n == args.y_param) {
        Some(i) => i,
        None => {
            eprintln!("Unknown y parameter '{}'. Use --list-params to see available names.", args.y_param);
            std::process::exit(1);
        }
    };
    ...
    eprintln!("X: {}  Y: {}  Resolution: {}x{}  Frames: {}  Seed: {}  Fitness: {:?}",
        args.x_param, args.y_param, args.resolution, args.resolution,
        args.frames, args.seed, args.fitness);
    ...
    eprintln!("Peak: {}={:.4}, {}={:.4} -> fitness={:.6}",
        ...
        landscape.peak.2);
    eprintln!("Scan time: {:.2}s ({:.1} evals/sec)",
        elapsed.as_secs_f64(),
