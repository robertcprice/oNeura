...
impl Scenario {
    ...
    fn from_str(s: &str) -> Option<Scenario> {
        match s {
            ...
            "nutrient" | "nutrient_depletion" | "nutrient-depletion" => Some(Scenario::NutrientDepletion),
...
fn parse_args() -> Args {
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            ...
            "--all" | "-a" => { args.scenarios = Scenario::all(); }
...
fn main() {
    ...
    eprintln!();
    eprintln!("{:<22} {:>10} {:>10} {:>10} {:>10} {:>10}", "Scenario", "Baseline", "Min", "Final", "Recovery%", "Time(ms)");
    eprintln!("{}", "-".repeat(82));
    for r in &results {
        eprintln!(
            "{:<22} {:>10.2} {:>10.2} {:>10.2} {:>9.1}% {:>10.0}",
            r.scenario, r.baseline_biomass, r.min_biomass,
            r.final_biomass, r.recovery_ratio * 100.0, r.elapsed_ms
        );
    }
    ...
    ranked.sort_by(|a, b| b.recovery_ratio.partial_cmp(&a.recovery_ratio).unwrap_or(std::cmp::Ordering::Equal));
    eprintln!();
    eprintln!("Resilience ranking (best recovery first):");
    for (i, r) in ranked.iter().enumerate() {
        eprintln!("  {}. {} ({:.1}% recovery)", i + 1, r.scenario, r.recovery_ratio * 100.0);
    }
