...
use oneuro_metal::terrarium_evolve::{evaluate_fitness, FitnessObjective, GENOME_PARAM_NAMES};
...
enum SweepTask {
    Continuous { name: &'static str },
    Integer { name: &'static str, lo: usize, hi: usize },
}

fn sweep_continuous(name: &str, resolution: usize, frames: usize, seed: u64, lite: bool) -> SensitivityResult {
...
fn print_multi_results(results: &[MultiObjectiveResult]) {
    ...
    for (obj_idx, (obj_name, _)) in OBJECTIVES.iter().enumerate() {
        let mut ranked: Vec<(&str, f32)> = results
            .iter()
            .map(|r| (r.parameter.as_str(), r.objectives[obj_idx].sensitivity_index))
            .collect();
    ...
    let content = if csv {
        match results {
            OutputResults::Single(r) => {
                let mut csv =
                    String::from("parameter,sensitivity_index,min_fitness,max_fitness,mean_fitness,is_integer\n");
                for row in r {
                    csv.push_str(&format!(
                        "{},{:.6},{:.6},{:.6},{:.6},{}\n",
                        row.parameter,
            ...
            OutputResults::Multi(r) => {
                ...
                for row in r {
                    ...
                    for obj in &row.objectives {
                        csv.push_str(&format!(",{:.6},{:.6}", obj.sensitivity_index, obj.mean_fitness));
                    }
                    csv.push('\n');
                }
                csv
...
fn main() {
    ...
    eprintln!("=== Terrarium Parameter Sensitivity Analysis ({}) ===", mode_str);
    eprintln!(
    ...
    if args.multi_objective {
        ...
        eprintln!();
        eprintln!("Total time: {:.2}s ({} params x {} objectives)", elapsed.as_secs_f64(), n_tasks, OBJECTIVES.len());
        ...
        let elapsed = start.elapsed();
        eprintln!();
        eprintln!("Total time: {:.2}s ({} params)", elapsed.as_secs_f64(), n_tasks);
