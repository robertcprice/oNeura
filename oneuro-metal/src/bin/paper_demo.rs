...
use oneuro_metal::{
    evolve_pareto_stressed, EvolutionConfig, FitnessConfig, FitnessObjective,
    GenomeConstraints, SearchStrategy,
    telemetry_from_pareto_result, run_and_export,
    sparkline, ecosystem_dashboard,
};
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--population" => { population = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(population); i += 1; }
            "--generations" => { generations = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(generations); i += 1; }
            "--frames" => { frames = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(frames); i += 1; }
            "--lite" => { lite = true; }
            "--full" => { lite = false; }
            "--seed" => { seed = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(seed); i += 1; }
            "--telemetry" => { telemetry_path = args.get(i+1).cloned().unwrap_or(telemetry_path); i += 1; }
            "--pareto" => { pareto_path = args.get(i+1).cloned().unwrap_or(pareto_path); i += 1; }
            "--no-export" => { export_best = false; }
            "--help" | "-h" => {
    ...
    eprintln!("  Grid:         {}", if lite { "10x8 (lite)" } else { "20x16 (full)" });
    eprintln!("  Seed:         {}", seed);
    eprintln!();
        ...
        Ok(pareto_result) => {
            ...
            if !pareto_result.pareto_front.is_empty() {
                ...
                for (j, vals) in obj_vecs.iter().enumerate() {
                    ...
                    eprintln!("  {:12} min={:8.3} mean={:8.3} max={:8.3}  {}", obj_names[j], min, mean, max, spark);
                }
                ...
                let param_names = [
                    "proton_scale", "temperature", "water_count", "water_vol",
                    "moisture", "plant_count", "fruit_count", "fly_count",
                    "microbes", "fly_psc", "fly_neural", "fly_scale",
                    "resp_vmax", "nitr_vmax", "photo_vmax", "miner_vmax",
                    "enzyme_x", "enzyme_y", "seed", "time_warp",
                ];
                let norm_vecs: Vec<Vec<f32>> = pareto_result.pareto_front.iter()
                    .map(|p| p.genome.normalized_params())
                    .collect();
                if !norm_vecs.is_empty() {
                    let n_params = norm_vecs[0].len().min(param_names.len());
                    for j in 0..n_params {
                        let vals: Vec<f32> = norm_vecs.iter().map(|v| *v.get(j).unwrap_or(&0.0)).collect();
                        let mean = vals.iter().sum::<f32>() / vals.len().max(1) as f32;
                        let var = vals.iter().map(|v| (v - mean).powi(2)).sum::<f32>() / vals.len().max(1) as f32;
                        let marker = if var > 0.05 { " ** HIGH" } else { "" };
                        eprintln!("  {:12} mean={:.3} var={:.4}{}", param_names[j], mean, var, marker);
                    }
                }
            }
            ...
            // Pareto front JSON
            let pareto_json = serde_json::to_string_pretty(&pareto_result.pareto_front).unwrap_or_default();
            ...
            eprintln!();
            eprintln!("╔══════════════════════════════════════════════════════╗");
            eprintln!("║  Demo complete in {:.1}s                              ║", total_time);
            eprintln!("╚══════════════════════════════════════════════════════╝");
            eprintln!();
            eprintln!("Next steps:");
