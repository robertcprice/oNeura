...
const SCALES: &[ScaleConfig] = &[
    ScaleConfig { name: "tiny",    plants:  2, flies: 0, water: 1, lite: true  },
    ScaleConfig { name: "small",   plants:  4, flies: 1, water: 2, lite: true  },
    ScaleConfig { name: "medium",  plants:  8, flies: 2, water: 3, lite: false },
    ScaleConfig { name: "large",   plants: 16, flies: 4, water: 4, lite: false },
    ScaleConfig { name: "dense",   plants: 16, flies: 6, water: 5, lite: false },
];
...
fn parse_args() -> Args {
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            ...
            "--warmup" | "-w" => { args.warmup = argv.get(i + 1).and_then(|s| s.parse().ok()).unwrap_or(args.warmup); i += 1; }
            "--seed" => { args.seed = argv.get(i + 1).and_then(|s| s.parse().ok()).unwrap_or(args.seed); i += 1; }
...
fn print_help() {
    ...
    for sc in SCALES {
        ...
        println!("  {:8} {} grid, {} plants, {} flies", sc.name, grid, sc.plants, sc.flies);
    }
}
...
fn profile_scale(scale: &ScaleConfig, frames: usize, warmup: usize, seed: u64) -> ProfileResult {
    ...
    let p50 = frame_times.get(frame_times.len() / 2).copied().unwrap_or(0.0);
    let p99_idx = ((frame_times.len() as f64 * 0.99) as usize).min(frame_times.len().saturating_sub(1));
...
fn main() {
    ...
    eprintln!("Warmup: {} frames  Measurement: {} frames  Seed: {}", args.warmup, args.frames, args.seed);
    eprintln!();

    let selected: Vec<&ScaleConfig> = if args.scales.iter().any(|s| s == "all") {
        SCALES.iter().collect()
    } else {
        SCALES.iter().filter(|sc| args.scales.contains(&sc.name.to_string())).collect()
    };

    if selected.is_empty() {
        eprintln!("No scales matched. Available: {}", SCALES.iter().map(|s| s.name).collect::<Vec<_>>().join(", "));
        std::process::exit(1);
    }
    ...
    eprintln!();
    eprintln!("{:<10} {:>12} {:>8} {:>8} {:>10} {:>10} {:>12}", "Scale", "Grid", "Plants", "Flies", "FPS", "Mean(ms)", "Cells/s");
    eprintln!("{}", "-".repeat(80));
    let max_fps = results.iter().map(|r| r.fps).fold(0.0f64, f64::max);
    for r in &results {
        eprintln!("{:<10} {:>12} {:>8} {:>8} {:>10.1} {:>10.2} {:>12.0}",
            r.scale, r.grid_size, r.plants, r.flies, r.fps, r.mean_frame_ms, r.throughput_cells_per_sec);
    }
    ...
    if results.len() >= 2 {
        ...
        let scaling_exponent = if organisms_ratio > 1.0 { time_ratio.ln() / organisms_ratio.ln() } else { 0.0 };
        eprintln!();
        eprintln!("Scaling analysis: {:.1}x organisms -> {:.1}x time (exponent: {:.2})",
    ...
    if let Some(ref path) = args.output {
        let content = if args.csv {
            ...
            for r in &results {
                csv.push_str(&format!(
                    "{},{},{},{},{},{:.2},{:.3},{:.3},{:.3},{:.3},{:.3},{:.0}\n",
                    r.scale, r.grid_size, r.cells, r.plants, r.flies,
                    r.fps, r.mean_frame_ms, r.min_frame_ms, r.max_frame_ms,
                    r.p50_frame_ms, r.p99_frame_ms, r.throughput_cells_per_sec
                ));
            }
            csv
        } else {
