...
use oneuro_metal::{
    evolve, EvolutionConfig, FitnessConfig, FitnessObjective,
    GenomeConstraints, SearchStrategy, WorldGenome,
    telemetry_from_result,
};
use oneuro_metal::atomistic_chemistry::{
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--telemetry" => { telemetry_path = args.get(i+1).cloned(); i += 1; }
            "--help" | "-h" => {
    ...
    eprintln!("Molecule: {} ({} atoms, {} bonds)",
        mol.graph.name, mol.graph.atom_count(), mol.graph.bond_count());
    ...
    eprintln!("Running evolution: {} pop x {} gen x {} frames (lite={})...",
    ...
    match evolve(config) {
        Ok(result) => {
            ...
            eprintln!("Best genome enzyme position: ({:.2}, {:.2})",
                result.global_best_genome.enzyme_probe_x,
                result.global_best_genome.enzyme_probe_y);
            ...
            let build_fn = if lite { WorldGenome::build_world_lite } else { WorldGenome::build_world };
            match build_fn(best) {
                Ok(mut world) => {
                    ...
                    match world.spawn_probe(&mol, gx, gy, 1) {
                        Ok(id) => {
                            eprintln!("Probe spawned: id={}, grid=({},{}), atoms={}", id, gx, gy, mol.graph.atom_count());
