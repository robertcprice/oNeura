...
use oneuro_metal::{
    evolve, telemetry_from_result,
    EvolutionConfig, FitnessConfig, FitnessObjective, GenomeConstraints,
    SearchStrategy, GenerationTelemetry,
};
use oneuro_metal::terrarium_evolve::telemetry_to_csv;
use std::env;
use std::fs;
...
fn parse_args() -> Args {
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            "--population" | "-p" => { args.population = argv.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(args.population); i += 1; }
            "--generations" | "-g" => { args.generations = argv.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(args.generations); i += 1; }
            ...
            "--width" => { args.width = argv.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(args.width); i += 1; }
            "--height" => { args.height = argv.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(args.height); i += 1; }
...
fn draw_line(buf: &mut [u32], bw: usize, bh: usize, x0: i32, y0: i32, x1: i32, y1: i32, color: u32) {
    let dx = (x1 - x0).abs();
    let dy = -(y1 - y0).abs();
...
fn draw_text_tiny(buf: &mut [u32], bw: usize, bh: usize, x: usize, y: usize, text: &str, color: u32) {
...
fn draw_convergence(buf: &mut [u32], bw: usize, bh: usize, ox: usize, oy: usize, pw: usize, ph: usize, data: &[GenerationTelemetry]) {
    fill_rect(buf, bw, ox, oy, pw, ph, BG);
    // Border
    for x in ox..ox + pw { if oy < bh { buf[oy * bw + x.min(bw - 1)] = BORDER; } if oy + ph - 1 < bh { buf[(oy + ph - 1) * bw + x.min(bw - 1)] = BORDER; } }
    for y in oy..oy + ph { if y < bh { buf[y * bw + ox.min(bw - 1)] = BORDER; buf[y * bw + (ox + pw - 1).min(bw - 1)] = BORDER; } }
...
fn draw_diversity(buf: &mut [u32], bw: usize, bh: usize, ox: usize, oy: usize, pw: usize, ph: usize, data: &[GenerationTelemetry]) {
    fill_rect(buf, bw, ox, oy, pw, ph, BG);
    for x in ox..ox + pw { if oy < bh { buf[oy * bw + x.min(bw - 1)] = BORDER; } if oy + ph - 1 < bh { buf[(oy + ph - 1) * bw + x.min(bw - 1)] = BORDER; } }
    for y in oy..oy + ph { if y < bh { buf[y * bw + ox.min(bw - 1)] = BORDER; buf[y * bw + (ox + pw - 1).min(bw - 1)] = BORDER; } }
    ...
    let max_d = diversities.iter().cloned().fold(f32::NEG_INFINITY, f32::max).max(0.01);
    ...
    for (i, &div) in diversities.iter().enumerate() {
        ...
        fill_rect(buf, bw, fx, bar_y, 2.max(plot_w / diversities.len().max(1)), bar_h, DIVERSITY_COLOR);
    }
}

/// Bottom-left: Parameter heatmap
fn draw_heatmap(buf: &mut [u32], bw: usize, bh: usize, ox: usize, oy: usize, pw: usize, ph: usize, data: &[GenerationTelemetry]) {
    fill_rect(buf, bw, ox, oy, pw, ph, BG);
    for x in ox..ox + pw { if oy < bh { buf[oy * bw + x.min(bw - 1)] = BORDER; } if oy + ph - 1 < bh { buf[(oy + ph - 1) * bw + x.min(bw - 1)] = BORDER; } }
    for y in oy..oy + ph { if y < bh { buf[y * bw + ox.min(bw - 1)] = BORDER; buf[y * bw + (ox + pw - 1).min(bw - 1)] = BORDER; } }
...
fn draw_radar(buf: &mut [u32], bw: usize, bh: usize, ox: usize, oy: usize, pw: usize, ph: usize, data: &[GenerationTelemetry]) {
    fill_rect(buf, bw, ox, oy, pw, ph, BG);
    for x in ox..ox + pw { if oy < bh { buf[oy * bw + x.min(bw - 1)] = BORDER; } if oy + ph - 1 < bh { buf[(oy + ph - 1) * bw + x.min(bw - 1)] = BORDER; } }
    for y in oy..oy + ph { if y < bh { buf[y * bw + ox.min(bw - 1)] = BORDER; buf[y * bw + (ox + pw - 1).min(bw - 1)] = BORDER; } }
    ...
    for (i, d) in data.iter().enumerate() {
        ...
        let fitness_norm = d.best_fitness / data.iter().map(|d| d.best_fitness).fold(f32::NEG_INFINITY, f32::max).max(0.01);
        let bar_w = (fitness_norm * (pw - margin * 2) as f32) as usize;
        let t = i as f32 / n.max(1) as f32;
        let color = lerp_color(rgb(60, 60, 120), BEST_COLOR, t);
        fill_rect(buf, bw, ox + margin, by, bar_w, bar_h.saturating_sub(1), color);
    }
}
...
fn main() {
    ...
    eprintln!("Pop: {}  Gens: {}  Frames: {}  Seed: {}  Lite: {}", args.population, args.generations, args.frames, args.seed, args.lite);
    ...
    eprintln!("Evolution complete in {:.2}s — best fitness: {:.4}", elapsed.as_secs_f64(), result.global_best_fitness);
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        let status = format!("Gen {}/{} | Best: {:.2} | Space=show all | Esc=quit",
            visible_gens, telemetry.len(),
