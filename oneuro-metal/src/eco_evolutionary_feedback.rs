...
impl EcoEvoSimulator {
    ...
    pub fn new(pop_size: usize, n_traits: usize, seed: u64) -> Self {
        ...
        for i in 0..pop_size {
            let traits: Vec<f64> = (0..n_traits)
                .map(|_| rng.next_normal(0.0, 0.1))
                .collect();
    ...
    pub fn step(&mut self) -> EcoEvoStepResult {
        ...
        let min_fit = self.population.iter().map(|o| o.fitness).fold(f64::INFINITY, f64::min);
        let total_fit: f64 = self.population.iter().map(|o| (o.fitness - min_fit) + 0.01).sum();
    ...
    pub fn detect_character_displacement(&self) -> Vec<CharacterDisplacement> {
        ...
        for &lin in &lineages {
            let members: Vec<&EcoEvoOrganism> =
                self.population.iter().filter(|o| o.lineage_id == lin).collect();
            if members.is_empty() {
                continue;
            }
            let means: Vec<f64> = (0..n_traits)
                .map(|t| {
                    members.iter().map(|o| o.traits[t]).sum::<f64>() / members.len() as f64
                })
                .collect();
        ...
        for i in 0..lineage_means.len() {
            for j in (i + 1)..lineage_means.len() {
                ...
                for t in 0..n_traits {
                    ...
                    let exists = self.initial_distances.iter().any(|((a, b, ax), _)| {
                        *a == pair_key.0 && *b == pair_key.1 && *ax == t
                    });
                    if !exists {
                        self.initial_distances.push(((pair_key.0, pair_key.1, t), dist));
                    }
                }
            }
        }
...
pub fn compute_qst(subpops: &[&[EcoEvoOrganism]], trait_idx: usize) -> f64 {
    ...
    for subpop in subpops {
        ...
        let mean =
            subpop.iter().map(|o| o.traits[trait_idx]).sum::<f64>() / subpop.len() as f64;
...
pub fn price_equation(
    ...
    let mean_w: f64 =
        before.iter().map(|o| o.fitness).sum::<f64>() / before.len() as f64;
    if mean_w < 1e-15 {
        return (0.0, 0.0);
    }
...
mod tests {
    ...
    fn fitness_landscape_gaussian() {
        ...
        assert!(away < at_optimum, "Fitness should decrease away from optimum");

        // Far away, fitness should be near zero
        let far = landscape.fitness(&[10.0, 10.0]);
        assert!(far < 0.01, "Fitness far from optimum should be near zero, got {}", far);
    }
    ...
    fn price_equation_decomposition() {
        ...
        let mean_after: f64 =
            after.iter().map(|o| o.traits[0]).sum::<f64>() / after.len() as f64;
