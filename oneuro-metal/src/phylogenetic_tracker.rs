...
mod tests {
    ...
    fn speciation_detection() {
        ...
        let _c1 = tree.add_node(Some(root), 1, 1.0, 0x0000_0000_0000_0000, 1.0, PhyloTraits::default());
        let _c2 = tree.add_node(Some(root), 1, 1.0, 0xFFFF_FFFF_FFFF_FFFF, 1.0, PhyloTraits::default());
    ...
    fn newick_format_correct() {
        ...
        assert!(nwk.contains('(') && nwk.contains(')'), "should have clade parens: {nwk}");
    ...
    fn phylogenetic_diversity_computation() {
        ...
        assert!((pd - 5.0).abs() < f32::EPSILON, "PD should be 5.0, got {pd}");

        // Mean branch length = 5.0 / 3 edges = 1.6667
        let mbl = tree.mean_branch_length();
        assert!((mbl - 5.0 / 3.0).abs() < 0.001, "mean BL should be ~1.667, got {mbl}");
