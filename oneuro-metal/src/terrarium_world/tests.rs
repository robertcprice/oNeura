use super::{
    deposit_2d_background_only, field_min_inv, fly_body_state_from_world_translation,
    fly_translation_world_from_body, idx2, idx3, normalize_unit, render_top_surface_y,
    terrarium_render_id_class, TerrariumDynamicBatchKind, TerrariumFlyPartKind,
    TerrariumFruitPartKind, TerrariumFruitPatch, TerrariumFruitPose, TerrariumMeshPrimitive,
    TerrariumRenderPlumeKind, TerrariumSeed, TerrariumSeedPartKind, TerrariumSeedPose,
    TerrariumTopdownView, TerrariumWorld, ATMOS_CO2_BASELINE, ATMOS_CO2_IDX,
    ATMOS_DENSITY_BASELINE_KG_M3, ATMOS_O2_BASELINE, ATMOS_O2_IDX, ATMOS_PRESSURE_BASELINE_KPA,
    ETHYL_ACETATE_IDX, EXPLICIT_MICROBE_PATCH_RADIUS, EXPLICIT_MICROBE_RECRUITMENT_SPACING,
    EXPLICIT_OWNERSHIP_THRESHOLD, HENRY_CO2, HENRY_O2, INTERNAL_SECONDARY_GENOTYPE_AXES,
    NOVEL_BANK_IDX, RENDER_ALTITUDE_SCALE, SHADOW_BANK_IDX, TERRARIUM_RENDER_ID_WATER,
    TERRARIUM_SHADER_FLAG_FLUID, TERRARIUM_SHADER_FLAG_FLY, TERRARIUM_SHADER_FLAG_FRUIT,
    TERRARIUM_SHADER_FLAG_MICROBE, TERRARIUM_SHADER_FLAG_PLANT, TERRARIUM_SHADER_FLAG_PLUME,
    TERRARIUM_SHADER_FLAG_SEED, TERRARIUM_SHADER_FLAG_SUBSTRATE, VARIANT_BANK_IDX,
};
...
use crate::{
    MaterialPhaseDescriptor, MaterialPhaseKind, MaterialPhaseSelector, MaterialRegionKind,
    MoleculeGraph, SeedCellularStateSim, SeedTissue,
};
...
fn explicit_microbes_step_and_report_live_whole_cell_metrics() {
    ...
    assert!(
        (after.mean_explicit_microbe_atp_mm - before.mean_explicit_microbe_atp_mm).abs()
            > 1.0e-6
...
fn owned_cells_use_explicit_chemistry_before_substrate_step() {
    ...
    let _ = world.substrate.deposit_patch_species(
    ...
    assert!(
        inv_glucose_after < inv_glucose_before || inv_co2_after > inv_co2_before
...
fn fruit_decay_routes_owned_detritus_to_substrate_not_coarse_litter() {
    ...
    world.fruits.push(TerrariumFruitPatch {
        ...
        deposited_all: false,
        pose: TerrariumFruitPose::default(),
    });
...
fn latent_strain_banks_remain_active() {
    ...
    assert!(
        snapshot.mean_microbial_bank_simpson_diversity
            >= snapshot.mean_microbial_shadow_fraction
    );
    assert!(
        snapshot.mean_nitrifier_bank_simpson_diversity
            >= snapshot.mean_nitrifier_shadow_fraction
    );
    assert!(
...
fn snapshot_uses_catalog_slots_and_catalog_bank_for_secondary_genotypes() {
    ...
    assert!(
        (corrupted.mean_microbial_gene_catabolic - baseline.mean_microbial_gene_catabolic)
            .abs()
            > 1.0e-4
    );
    assert!(
    ...
    assert!(
        (corrupted.mean_microbial_catalog_generation
            - baseline.mean_microbial_catalog_generation)
            .abs()
            > 1.0e-4
    );
    assert!(
        (corrupted.mean_microbial_lineage_generation
            - baseline.mean_microbial_lineage_generation)
            .abs()
            > 1.0e-4
    );
    assert!(
    ...
    assert!(
        (corrupted.mean_nitrifier_catalog_generation
            - baseline.mean_nitrifier_catalog_generation)
            .abs()
            > 1.0e-4
    );
    assert!(
        (corrupted.mean_nitrifier_lineage_generation
            - baseline.mean_nitrifier_lineage_generation)
            .abs()
            > 1.0e-4
    );
    assert!(
...
fn visual_biomechanics_follow_native_air_state() {
    ...
    world.step_visual_biomechanics(world.config.world_dt_s);
    world.step_visual_biomechanics(world.config.world_dt_s);
...
fn seed_cellular_state_tracks_native_seed_environment() {
    ...
    if world.seeds.is_empty() {
        world.seeds.push(TerrariumSeed {
            ...
            reserve_carbon: 0.18,
            age_s: 3_600.0,
            genome: world.plants[plant_idx].genome.clone(),
            cellular: SeedCellularStateSim::new(
                world.plants[plant_idx].genome.seed_mass,
                0.18,
                14_000.0,
            ),
...
fn render_projection_reports_native_scene_state() {
    ...
    assert!(render.explicit_microbes.iter().all(|microbe| {
        render_ids.insert(microbe.render_id)
            && ((!microbe.body_mesh.positions.is_empty()
                && !microbe.body_mesh.indices.is_empty())
...
fn render_lighting_tracks_scene_air_and_day_cycle() {
    ...
    let expected_pressure_t =
        ((expected_pressure_mean - ATMOS_PRESSURE_BASELINE_KPA) / 4.0 * 0.5 + 0.5)
            .clamp(0.0, 1.0);
    assert!(
...
fn native_pairwise_fly_contacts_separate_overlapping_bodies() {
    ...
    let (world_a, _) = fly_translation_world_from_body(
        &world.config,
        &terrain,
        terrain_min,
        terrain_inv,
    ...
    let (world_b, _) = fly_translation_world_from_body(
        &world.config,
        &terrain,
        terrain_min,
        terrain_inv,
...
fn explicit_microbe_kinetic_flux_stays_in_historic_range() {
    ...
    let (glucose_draw, oxygen_draw, ammonium_draw, nitrate_draw,
         atp_flux, co2_release, proton_release) =
        TerrariumWorld::compute_explicit_microbe_fluxes(
            &k, eco_dt, uptake_scale,
            soil_glucose, soil_oxygen, soil_ammonium, soil_nitrate,
            wcs_glucose_delta, wcs_oxygen_delta, wcs_ammonium_delta,
            wcs_nitrate_delta, wcs_atp_delta, local_atp_flux,
        );
    ...
    assert!(glucose_draw <= 0.0035, "glucose_draw {glucose_draw} exceeds ceiling");
    assert!(oxygen_draw <= 0.0032, "oxygen_draw {oxygen_draw} exceeds ceiling");
    assert!(ammonium_draw <= 0.0020, "ammonium_draw {ammonium_draw} exceeds ceiling");
    assert!(nitrate_draw <= 0.0018, "nitrate_draw {nitrate_draw} exceeds ceiling");
    assert!(atp_flux <= 0.0018, "atp_flux {atp_flux} exceeds ceiling");
    assert!(co2_release <= 0.0040, "co2_release {co2_release} exceeds ceiling");
    assert!(proton_release <= 0.0012, "proton_release {proton_release} exceeds ceiling");

    // CO2 should be stoichiometrically related to glucose draw
    // (at minimum the respiration component: 1:1 ratio)
    assert!(co2_release > 0.0, "CO2 release should be positive when glucose is consumed");
    assert!(
