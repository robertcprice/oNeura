...
const MICROBIAL_NP: f64 = 7.0; // N:P
...
impl NutrientCycler {
    ...
    fn decompose(&mut self, dt: f64) -> StoichiometricBalance {
        ...
        for pool in &mut self.organic_pools {
            ...
            if n_net >= 0.0 {
                ...
                let immobilized = (-n_net).min(
                    self.mineral_pool.ammonium_g + self.mineral_pool.nitrate_g,
                );
    ...
    pub fn plant_uptake(
        ...
        for demand in demands {
            ...
            let n_taken = n_potential.min(self.mineral_pool.ammonium_g + self.mineral_pool.nitrate_g);
    ...
    pub fn mycorrhizal_transfer(
        ...
        for &(i, j) in &network.plant_connections {
            ...
            if sat_n_i != sat_n_j {
                ...
                let n_transfer = gradient
                    * effective_efficiency
                    * network.hyphal_density
                    * 0.001; // scale factor
            ...
            match cohort.stage {
                DecompositionStage::Fresh => {
                        ...
                        || transition_prob > 0.5
                    {
                        ...
                        cohort.lignin_fraction =
                            (cohort.lignin_fraction + 0.1).min(0.8);
                ...
                DecompositionStage::Fragmented => {
                        ...
                        || (transition_prob > 0.5 && cohort.age_days > 30.0)
                    {
                        ...
                        cohort.lignin_fraction =
                            (cohort.lignin_fraction + 0.15).min(0.9);
...
mod tests {
    ...
    fn denitrification_requires_anoxia() {
        ...
        let initial_no3 = c.mineral_pool.nitrate_g;
        let result = c.step(1.0);
    ...
    fn p_sorption_langmuir() {
        ...
        assert!(s2 > s1, "more solution P ({}) -> more sorption ({})", s2, s1);
    ...
    fn mass_balance_conservation() {
        ...
        let n_gained = c.atmospheric_flux.n_fixation * 1.0
            + c.atmospheric_flux.n_deposition * 1.0;
