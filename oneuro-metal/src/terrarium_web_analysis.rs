...
use crate::terrarium_web_protocol::{
    SensitivityProgressData, SensitivityCompleteData,
    StressProgressData, StressCompleteData,
    SensitivityWebConfig, StressWebConfig,
    ServerMsg,
};
...
const SWEEPABLE: &[&str] = &[
    "proton_scale", "temperature_c", "water_volume", "moisture_scale",
    "respiration_vmax", "nitrification_vmax", "photosynthesis_vmax",
    "mineralization_vmax", "time_warp", "psc_scale",
];
    ...
    tokio::spawn(async move {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            let params: Vec<&str> = if let Some(ref p) = config.param {
                ...
                SWEEPABLE.iter().filter(|&&n| n == target).cloned().collect()
            } else {
                SWEEPABLE.to_vec()
            };
            ...
            for (idx, name) in params.iter().enumerate() {
                ...
                let si = if mean_f.abs() > 1e-10 { (max_f - min_f) / mean_f } else { 0.0 };
            ...
            all_results.sort_by(|a, b| b.sensitivity_index.partial_cmp(&a.sensitivity_index).unwrap_or(std::cmp::Ordering::Equal));

            let rankings: Vec<(String, f32)> = all_results.iter()
...
const SCENARIOS: &[&str] = &[
    "drought", "heat_spike", "nutrient_depletion",
    "population_crash", "invasive_species", "combined",
];

fn apply_stress(world: &mut crate::terrarium_world::TerrariumWorld, scenario: &str) {
    match scenario {
        "drought" => {
            for h in world.substrate.hydration.iter_mut() { *h = 0.0; }
            for ws in &mut world.waters { ws.volume = 0.0; }
        }
        "heat_spike" => {
            let n_species = 14;
        ...
        "nutrient_depletion" => {
            for val in world.substrate.current.iter_mut() { *val *= 0.01; }
        }
        "population_crash" => {
        ...
        "invasive_species" => {
            use crate::{TerrariumFruitPatch, FruitSourceState};
            ...
            for _ in 0..10 {
                if world.fruits.len() < world.config.max_fruits {
                    let source = FruitSourceState {
                        x: rng.gen_range(0..w), y: rng.gen_range(0..h), z: 0,
                        ripeness: 1.0, sugar_content: 5.0, odorant_emission_rate: 0.01,
                        decay_rate: 0.001, alive: true, odorant_profile: vec![],
                    };
                    world.fruits.push(TerrariumFruitPatch {
                        source, radius: 1.5, previous_remaining: 5.0, deposited_all: false,
                    });
                }
            }
        }
    ...
    tokio::spawn(async move {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            for (idx, scenario) in scenarios.iter().enumerate() {
                ...
                // Warmup
                for _ in 0..stress_frame { let _ = world.step_frame(); }
                ...
                for f in stress_frame..frames {
                    ...
                    if bm < min_biomass { min_biomass = bm; min_frame = f; }
                    final_biomass = bm;
                }
                if min_biomass == f32::INFINITY { min_biomass = final_biomass; }
                let recovery_ratio = if baseline_biomass > 1e-6 { final_biomass / baseline_biomass } else { 0.0 };
