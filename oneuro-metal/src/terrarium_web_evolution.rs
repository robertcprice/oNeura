...
pub async fn start_evolution(state: Arc<AppState>, web_config: EvolveWebConfig) {
    ...
    tokio::spawn(async move {
        ...
        let result = tokio::task::spawn_blocking(move || {
            run_evolution_with_mode(&mode_str, config)
        })
        .await;
...
fn run_evolution_with_mode(mode: &str, config: EvolutionConfig) -> Result<EvoOutcome, String> {
    match mode {
        ...
        "coevolution" => {
            ...
            let mut cfg = config;
            if let SearchStrategy::Evolutionary { ref mut mutation_rate, ref mut crossover_rate, .. } = cfg.strategy {
