...
impl WholeCellSimulator {
    ...
    fn chromosome_domain_index(&self, midpoint_bp: u32, genome_bp: u32) -> u32 {
        let genome_bp = genome_bp.max(1);
    ...
    fn chromosome_domain_value(values: &[f32], domain_index: usize) -> f32 {
        values
            .get(domain_index)
            .copied()
...
mod tests {
    use super::*;
    use crate::whole_cell_data::{
        WholeCellAssetClass, WholeCellBulkField, WholeCellComplexComponentSpec,
        WholeCellOperonSpec, WholeCellProteinProductSpec,
    };
    use crate::whole_cell_submodels::{
    ...
    fn test_chromosome_domain_support_tracks_domain_loaded_pools() {
        ...
        assert!(
            sim.chromosome_domain_nucleotide_support(0)
                > sim.chromosome_domain_nucleotide_support(3)
        );
    }
    ...
    fn test_restore_saved_state_prefers_explicit_state_over_stale_core_summary() {
        ...
        assert!(
            (snapshot.chromosome_separation_nm - expected.chromosome_separation_nm).abs() < 1.0e-6
        );
    ...
    fn test_from_saved_state_json_without_organism_prefers_explicit_saved_state() {
        ...
        simulator.surface_area_nm2 =
            WholeCellSimulator::surface_area_from_radius(simulator.radius_nm);
