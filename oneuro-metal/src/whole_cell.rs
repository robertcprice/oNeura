...
use crate::whole_cell_submodels::{
    ...
    WholeCellChemistryBridge, WholeCellDerivationCalibration,
...
impl WholeCellSimulator {
    ...
    fn run_quantum_auto_discovery(&mut self) {
        ...
        for reaction in &self.organism_reactions {
            ...
            if let Some(ch) = channel {
                if eligible {
                    self.discovered_quantum_reactions.push(QuantumDiscoveredReaction {
    ...
    fn channel_from_subsystem_targets(targets: &[Syn3ASubsystemPreset]) -> Option<QuantumProfileChannel> {
        for t in targets {
            match t {
                Syn3ASubsystemPreset::AtpSynthaseMembraneBand => return Some(QuantumProfileChannel::Oxphos),
                Syn3ASubsystemPreset::RibosomePolysomeCluster => return Some(QuantumProfileChannel::Translation),
                Syn3ASubsystemPreset::ReplisomeTrack => return Some(QuantumProfileChannel::NucleotidePolymerization),
                Syn3ASubsystemPreset::FtsZSeptumRing => return Some(QuantumProfileChannel::ChromosomeSegregation),
            }
        }
        None
    }
    ...
    pub(super) fn refine_quantum_corrections_from_probe(&mut self, probe: &LocalMDProbeReport) {
        ...
        let corrections = [
            ...
            (0.40 * probe.recommended_segregation_scale + 0.35 * st + 0.25 * probe.recommended_constriction_scale).clamp(0.5, 2.5),
        ];
        let blend = |cur: f32, c: f32| ((1.0 - ALPHA) * cur + ALPHA * c).clamp(0.5, 2.5);
        self.quantum_profile.oxphos_efficiency = blend(self.quantum_profile.oxphos_efficiency, corrections[0]);
        self.quantum_profile.translation_efficiency = blend(self.quantum_profile.translation_efficiency, corrections[1]);
        self.quantum_profile.nucleotide_polymerization_efficiency = blend(self.quantum_profile.nucleotide_polymerization_efficiency, corrections[2]);
        self.quantum_profile.membrane_synthesis_efficiency = blend(self.quantum_profile.membrane_synthesis_efficiency, corrections[3]);
        self.quantum_profile.chromosome_segregation_efficiency = blend(self.quantum_profile.chromosome_segregation_efficiency, corrections[4]);
        let channel_corrections = [
            QuantumProfileChannel::Oxphos, QuantumProfileChannel::Translation,
            QuantumProfileChannel::NucleotidePolymerization, QuantumProfileChannel::MembraneSynthesis,
            QuantumProfileChannel::ChromosomeSegregation,
        ];
        for reaction in &mut self.discovered_quantum_reactions {
            let idx = channel_corrections.iter().position(|&c| c == reaction.profile_channel).unwrap_or(0);
            reaction.correction_factor = ((1.0 - ALPHA) * reaction.correction_factor + ALPHA * corrections[idx]).clamp(0.5, 2.5);
    ...
    fn apply_quantum_discovery_to_profile(&mut self) {
        ...
        for &ch in &[Oxphos, Translation, NucleotidePolymerization, MembraneSynthesis, ChromosomeSegregation] {
            let factors: Vec<f32> = self.discovered_quantum_reactions.iter()
                .filter(|r| r.profile_channel == ch)
                .map(|r| r.correction_factor.clamp(0.5, 2.5))
                .collect();
            if factors.is_empty() { continue; }
            let geo = (factors.iter().map(|f| f.ln()).sum::<f32>() / factors.len() as f32).exp().clamp(0.5, 2.5);
            match ch {
                Oxphos => self.quantum_profile.oxphos_efficiency = geo,
                Translation => self.quantum_profile.translation_efficiency = geo,
                NucleotidePolymerization => self.quantum_profile.nucleotide_polymerization_efficiency = geo,
                MembraneSynthesis => self.quantum_profile.membrane_synthesis_efficiency = geo,
                ChromosomeSegregation => self.quantum_profile.chromosome_segregation_efficiency = geo,
            }
        }
    }
    ...
    pub fn calibrate_from_observables(
        ...
        let profile_set = |p: &mut WholeCellQuantumProfile, i: usize, v: f32| {
            match i {
        ...
        let evaluate = |profile: WholeCellQuantumProfile| -> f32 {
            ...
            for _ in 0..eval_steps { trial.step(); }
            let (loss, _) = trial.observable_loss(targets);
            if loss.is_finite() { loss } else { 1e6 }
        };
        ...
        for _ in 0..eval_steps { final_sim.step(); }
