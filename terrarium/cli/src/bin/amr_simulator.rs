...
use oneura_core::terrarium::BiofilmSimulator;
...
use oneura_core::terrarium::{GeneticElement, GeneticElementType, HgtPopulation};
...
fn parse_args() -> Config {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            other => {
                eprintln!(
                    "{}Warning{}: Unknown argument '{}', ignoring.",
                    YELLOW, RESET, other
                );
            }
        }
    ...
    for ev in events {
        let s = match ev {
            ...
            ResistanceEvent::MdrAcquired { num_classes, .. } => {
                format!("{}{}MDR ({}cl)!{}", BG_RED, WHITE, num_classes, RESET)
            }
            ResistanceEvent::ClinicalResistance { antibiotic_idx, .. } => {
                let name = ab_names
                    .get(*antibiotic_idx)
                    .map(|s: &String| s.as_str())
                    .unwrap_or("?");
                format!("{}{}R: {}!{}", BG_YELLOW, RED, name, RESET)
            }
        };
        parts.push(s);
    }
...
fn run_hgt_simulation(config: &Config, transferable_mechs: &[(&str, f64)]) {
    ...
    for (eid, name) in &element_ids {
        if let Some(freqs) = ts.element_frequencies.get(eid) {
            let initial = freqs.first().copied().unwrap_or(0.0f64);
...
fn run_biofilm_simulation(config: &Config) {
    eprintln!();
    eprintln!(
        "{}{}=== Biofilm Impact on Antibiotic Tolerance ==={}",
        BOLD, GREEN, RESET
    );
    eprintln!();

    // Use the Pseudomonas preset -- clinically relevant biofilm former
    let mut biofilm = oneura_core::biofilm_dynamics::pseudomonas_biofilm(config.seed + 200);
    ...
    eprintln!(
        "  {}Biofilm maturation ({} steps x {}s):{}",
        BOLD, maturation_steps, dt, RESET
    );
    eprintln!(
...
fn main() {
    ...
    for gen in 0..config.generations {
        ...
        let print_row =
            gen == 0 || gen == config.generations - 1 || gen % 10 == 0 || !events.is_empty();
    ...
    eprintln!();
    eprintln!("{}{}=== Results ==={}", BOLD, CYAN, RESET);
    ...
    match mdr_emergence_gen {
        ...
        None => eprintln!("  MDR emergence: {}Not detected{}", GREEN, RESET),
    }

    if xdr_detected {
        eprintln!("  {}{}XDR strain detected!{}", BG_RED, WHITE, RESET);
    }

    // Mutant selection windows
    eprintln!();
    eprintln!("{}Mutant Selection Windows:{}", BOLD, RESET);
    for i in 0..n_abs {
        let (msw_lo, msw_hi) = sim.mutant_selection_window(i);
        eprintln!("  {}: [{:.2}, {:.2}] ug/mL", ab_names[i], msw_lo, msw_hi);
    }
    ...
    if !valleys.is_empty() {
        eprintln!();
        eprintln!("{}Fitness Valley Crossings:{}", BOLD, RESET);
        for v in &valleys {
            eprintln!(
    ...
    eprintln!();
    eprintln!(
        "{}Analytical Resistance Predictions (deterministic approx.):{}",
        BOLD, RESET
    );
    for i in 0..n_abs {
        let conc = base_concentrations[i];
        match sim.predict_resistance_time(i, conc) {
            Some(t) => eprintln!("  {} at {:.1} ug/mL: ~{} generations", ab_names[i], conc, t),
            None => eprintln!(
