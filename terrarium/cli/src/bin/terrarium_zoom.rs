...
use oneura_core::terrarium::{
    ecosystem_dashboard, PlantTissue, TerrariumSpecies, TerrariumWorld, TerrariumWorldSnapshot,
};
...
fn apply_scenario(world: &mut TerrariumWorld, preset: ScenarioPreset) {
    match preset {
        ...
        ScenarioPreset::Drought => {
            for v in world.moisture_field_mut().iter_mut() {
                *v *= 0.3;
            }
            if world.waters.len() > 1 {
                world.waters.truncate(1);
            }
        }
        ScenarioPreset::Heat => {
            for v in world.temperature_field_mut().iter_mut() {
                *v += 15.0;
            }
        }
...
fn draw_legend_bar(buf: &mut ScreenBuffer, vs: &ViewState) {
    ...
    for sx in 0..buf.width {
        buf.set(sx, y, '\u{2500}', sep_fg, bar_bg);
    }
    ...
    buf.set(x, r2, '\u{2663}', typical_plant, bar_bg);
    x += 1;
    buf.write_str(x, r2, "=Plant ", lbl_fg, bar_bg);
    x += 7;
    buf.set(x, r2, '\u{25CF}', (100, 200, 50), bar_bg);
    x += 1;
    buf.write_str(x, r2, "\u{2192}", dim_fg, bar_bg);
    x += 1;
    buf.set(x, r2, '\u{25CF}', (255, 80, 30), bar_bg);
    x += 1;
    buf.write_str(x, r2, "=Fruit ", lbl_fg, bar_bg);
    x += 7;
    buf.set(x, r2, '\u{2248}', (60, 140, 255), bar_bg);
    x += 1;
    buf.write_str(x, r2, "=Water ", lbl_fg, bar_bg);
    x += 7;
    buf.set(x, r2, '\u{2736}', (255, 230, 50), bar_bg);
    x += 1;
    buf.write_str(x, r2, "=Fly(air) ", lbl_fg, bar_bg);
    x += 10;
    buf.set(x, r2, '\u{25C6}', (255, 200, 80), bar_bg);
    x += 1;
    buf.write_str(x, r2, "=Fly(land) ", lbl_fg, bar_bg);
    x += 11;
    ...
    buf.set(x, r2, '\u{2593}', top_c, bar_bg);
    x += 1;
    buf.write_str(x, r2, "=Top ", lbl_fg, bar_bg);
    x += 5;
    buf.set(x, r2, '\u{2588}', side_c, bar_bg);
    x += 1;
    ...
    for i in 0..10 {
        let t = i as f32 / 9.0;
        buf.set(x + i, r3, '\u{2588}', soil_color(t, t * 0.4), bar_bg);
    }
    x += 10;
    buf.write_str(x, r3, "(dry", soil_color(0.0, 0.0), bar_bg);
    x += 4;
    buf.set(x, r3, '\u{2192}', dim_fg, bar_bg);
    x += 1;
    buf.write_str(x, r3, "wet) ", soil_color(1.0, 0.4), bar_bg);
    x += 5;
    buf.write_str(x, r3, "Plants: ", dim_fg, bar_bg);
    x += 8;
    let ph = plant_color(1.0, 1.0);
    let ps = plant_color(0.8, 0.2);
    buf.set(x, r3, '\u{2588}', ph, bar_bg);
    x += 1;
    buf.set(x, r3, '\u{2588}', ph, bar_bg);
    x += 1;
    buf.write_str(x, r3, "healthy ", ph, bar_bg);
    x += 8;
    buf.set(x, r3, '\u{2588}', ps, bar_bg);
    x += 1;
    buf.set(x, r3, '\u{2588}', ps, bar_bg);
    x += 1;
    ...
    let controls = if zl == ZoomLevel::Ecosystem {
        format!(
            " [H]elp [Tab]mode [WASD]pan [+/-]zoom [Space]{} [M]inimap [R]eset [Q]uit",
            if vs.paused { "resume" } else { "pause" }
        )
    } else {
        format!(
            " [H]elp [WASD]cursor [+/-]zoom [Enter]select [Esc]deselect [Space]{} [Q]uit",
            if vs.paused { "resume" } else { "pause" }
        )
    };
    buf.write_str(
    ...
    let right_info = if zl != ZoomLevel::Ecosystem {
        let sel_name = match vs.selected {
            SelectedEntity::None => "",
            SelectedEntity::Plant(_) => "Plant",
            SelectedEntity::Fly(_) => "Fly",
            SelectedEntity::Water(_) => "Water",
...
fn draw_help_overlay(buf: &mut ScreenBuffer) {
    ...
    for x in px..px + pw {
        buf.set(x, py, '\u{2550}', bfg, pbg);
        buf.set(x, py + ph - 1, '\u{2550}', bfg, pbg);
    }
    for y in py..py + ph {
        buf.set(px, y, '\u{2551}', bfg, pbg);
        buf.set(px + pw - 1, y, '\u{2551}', bfg, pbg);
    }
    buf.set(px, py, '\u{2554}', bfg, pbg);
    buf.set(px + pw - 1, py, '\u{2557}', bfg, pbg);
    buf.set(px, py + ph - 1, '\u{255A}', bfg, pbg);
    buf.set(px + pw - 1, py + ph - 1, '\u{255D}', bfg, pbg);
    ...
    buf.write_str(
        cx,
        r,
        "   oNeura Terrarium - Semantic Zoom Viewer",
        tfg,
        pbg,
    );
    r += 2;
    ...
    buf.write_str(cx, r, "\u{2500}\u{2500} CONTROLS ", hfg, pbg);
    r += 1;
    for (k, desc) in &keys {
    ...
    r += 1;
    buf.write_str(cx, r, "\u{2500}\u{2500} SEMANTIC ZOOM ", hfg, pbg);
    r += 1;
    buf.write_str(cx + 2, r, "0.4-1.5x  Ecosystem  Grid overview", txt, pbg);
    r += 1;
    buf.write_str(cx + 2, r, "1.5-2.5x  Organism   Physiology panel", txt, pbg);
    r += 1;
    buf.write_str(
        cx + 2,
        r,
        "2.5-3.5x  Cellular   Tissue metabolites",
        txt,
        pbg,
    );
    r += 1;
    buf.write_str(
        cx + 2,
        r,
        "3.5x+     Molecular  Substrate chemistry",
        txt,
        pbg,
    );
    r += 2;
    ...
    buf.write_str(cx, r, "\u{2500}\u{2500} COLORS ", hfg, pbg);
    r += 1;
    buf.write_str(cx + 2, r, "Soil: ", (255, 180, 60), pbg);
    for i in 0..5 {
        let t = i as f32 / 4.0;
        let c = soil_color(t, t * 0.5);
        buf.set(cx + 8 + i * 3, r, '\u{2588}', c, pbg);
        buf.set(cx + 9 + i * 3, r, '\u{2588}', c, pbg);
    }
    r += 1;
    buf.write_str(cx + 2, r, "Plant:", (255, 180, 60), pbg);
    for (i, &(can, vit)) in [(0.1, 1.0), (0.5, 0.8), (1.0, 1.0), (0.8, 0.3)]
        .iter()
        .enumerate()
    {
        let c = plant_color(can, vit);
        buf.set(cx + 8 + i * 3, r, '\u{2663}', c, pbg);
        buf.set(cx + 9 + i * 3, r, '\u{2663}', c, pbg);
    }
    r += 2;
    buf.write_str(
        cx,
...
fn render_isometric(
    ...
    let header = format!(
        " oNeura Terrarium 3D | F:{} | {} {} | {:.0}C | P:{} Fl:{} Fr:{}",
        fi,
        world.time_label(),
        moon_ascii,
        snap.temperature,
    ...
    let sub = format!(
        " moisture:{:.2} microbes:{:.3} CO2:{:.4} O2:{:.2} tide:{:.2} zoom:{:.1}x [{}]",
        snap.mean_soil_moisture,
        snap.mean_microbes,
        snap.mean_atmospheric_co2,
        snap.mean_atmospheric_o2,
        snap.tidal_moisture_factor,
        vs.zoom,
        vs.zoom_level().label()
    );
    buf.write_str(
    ...
    let net = photo - resp;
    let (net_label, net_color) = if net >= 0.0 {
        ("+", (80, 220, 80))
    } else {
        ("", (220, 80, 80))
    };
    let energy_line = format!(
        " Energy: photo={:.3} resp={:.3} net={}{:.3}",
        photo, resp, net_label, net
    );
    buf.write_str(
        0,
        2,
        &energy_line[..energy_line.len().min(buf.width)],
        net_color,
        (20, 20, 40),
    );
    if vs.paused {
        let ps = " PAUSED ";
        buf.write_str(
            (buf.width.saturating_sub(ps.len())) / 2,
            3,
            ps,
            (255, 80, 80),
            (60, 20, 20),
        ...
        for gx in 0..gw {
            ...
            for h in 0..=height {
                ...
                if sx >= 0 && sy >= 0 {
                    ...
                    for dx in 0..tw {
                        buf.set(sx as usize + dx, sy as usize, cc, fc, (10, 10, 20));
                    }
                }
            }
        }
        ...
        if fruit.source.alive && fruit.source.sugar_content > 0.01 {
            ...
            if ix >= 0 && iy >= 0 {
                ...
                buf.set(
                    ...
                    iy as usize,
                    '\u{25CF}',
                    lerp_color((100, 200, 50), (255, 80, 30), ripe),
                    (10, 10, 20),
                );
            }
        }
...
fn render_topdown_region(
    ...
    let header = format!(
        " Terrarium | F:{} | {} {} | {:.0}C | P:{} Fl:{} tide:{:.2} zoom:{:.1}x [{}]",
        fi,
        world.time_label(),
        td_moon,
        snap.temperature,
    ...
    let ox =
        (rx as isize + (rw as isize - (gw * scale) as isize) / 2) + vs.camera_x * scale as isize;
        ...
        for gx in 0..gw {
            ...
            if py >= 0 {
                for dx in 0..scale {
                    ...
                    if sx >= rx as isize && (sx as usize) < rx + rw {
                        buf.set(sx as usize, py as usize, cc, color, (10, 10, 20));
                    }
                }
            }
        }
                    ...
                    if wx < gw && wy < gh {
                        ...
                        if px >= 0 && py >= 0 {
                            for s in 0..scale {
                                ...
                                if sx >= rx && sx < rx + rw {
                                    buf.set(sx, py as usize, wave, (80, 160, 255), (20, 50, 100));
                                }
                            }
                        }
                    }
    ...
    for plant in &world.plants {
        ...
        if px >= rx as isize && py >= 0 && (px as usize) < rx + rw {
            ...
            if scale > 1 && (px as usize + 1) < rx + rw {
                buf.set(px as usize + 1, py as usize, cc, pc, (10, 30, 10));
            }
        }
    }
    ...
    for fly in &world.flies {
        ...
        let py = oy + gy as isize;
        if px >= rx as isize && py >= 0 && (px as usize) < rx + rw {
            buf.set(px as usize, py as usize, cc, col, (10, 10, 20));
        }
    }
}
...
fn render_heatmap(
    ...
    let moisture = world.moisture_field();
    let header = format!(
        " Heatmap | F:{} | {:.0}C | Avg:{:.3} | P:{} Fl:{} z:{:.1}x",
        fi, snap.temperature, snap.mean_soil_moisture, snap.plants, snap.flies, vs.zoom
    );
    buf.write_str(
        0,
        0,
...
fn render_dashboard(buf: &mut ScreenBuffer, history: &[TerrariumWorldSnapshot], fi: usize) {
    ...
    for (y, line) in dash.lines().enumerate() {
        ...
        buf.write_str(
            ...
            if is_h { (0, 220, 255) } else { (200, 220, 200) },
            if is_h { (20, 40, 60) } else { (15, 15, 25) },
        );
    }
    let fs = format!(" F:{fi} ");
    buf.write_str(
        buf.width.saturating_sub(fs.len() + 1),
...
fn draw_minimap(buf: &mut ScreenBuffer, world: &TerrariumWorld, mx: usize, my: usize) {
    ...
    for x in 1..=gw {
        buf.set(mx + x, my, '\u{2500}', (80, 80, 120), (10, 10, 20));
    }
    ...
    for x in 1..=gw {
        buf.set(mx + x, my + gh + 1, '\u{2500}', (80, 80, 120), (10, 10, 20));
    }
    buf.set(
        mx + gw + 1,
        my + gh + 1,
...
fn draw_detail_panel(
    ...
    for y in 0..buf.height.saturating_sub(4) {
    ...
    match (zl, vs.selected) {
        ...
        (ZoomLevel::Organism, SelectedEntity::Plant(idx)) if idx < world.plants.len() => {
            ...
            let phys = &plant.physiology;
            let cell = &plant.cellular;
            let mut r = 1;
            buf.write_str(
                cx,
                r,
            ...
            buf.write_str(
                cx,
                r,
                "\u{2500}\u{2500}\u{2500} Physiology \u{2500}\u{2500}\u{2500}",
                hfg,
                pbg,
            );
            r += 1;
            ...
            buf.write_str(cx + 10 + bw, r, &format!(" {:.2}", phys.health()), vfg, pbg);
            r += 1;
            for (name, val) in [
                ("Leaf   ", phys.leaf_biomass()),
                ("Stem   ", phys.stem_biomass()),
                ("Root   ", phys.root_biomass()),
                ("Carbon ", phys.storage_carbon()),
                ("Water  ", phys.water_buffer()),
                ("Nitro  ", phys.nitrogen_buffer()),
            ] {
                buf.write_str(cx, r, name, lfg, pbg);
                buf.write_str(cx + 9, r, &format!("{:.3}g", val), vfg, pbg);
                r += 1;
            }
            buf.write_str(
                cx,
                r,
                &format!("Height  {:.1}mm", phys.height_mm()),
                vfg,
                pbg,
            );
            r += 2;
            buf.write_str(
                cx,
                r,
                "\u{2500}\u{2500}\u{2500} Vitality \u{2500}\u{2500}\u{2500}\u{2500}\u{2500}",
                hfg,
                pbg,
            );
            r += 1;
            for (name, tissue) in [
                ("Leaf  ", PlantTissue::Leaf),
                ("Stem  ", PlantTissue::Stem),
                ("Root  ", PlantTissue::Root),
                ("Merist", PlantTissue::Meristem),
            ] {
                let s = cell.cluster_snapshot(tissue);
                buf.write_str(cx, r, name, lfg, pbg);
                buf.write_str(cx + 7, r, &bar_str(s.vitality, bw), bfg, pbg);
                buf.write_str(cx + 8 + bw, r, &format!(" {:.2}", s.vitality), vfg, pbg);
                r += 1;
            }
            r += 1;
            let ec = cell.energy_charge();
            buf.write_str(cx, r, "EC: ", lfg, pbg);
            buf.write_str(cx + 4, r, &bar_str(ec, bw), bfg, pbg);
            buf.write_str(
                cx + 5 + bw,
                r,
                &format!(" {:.2}  Cells:{:.0}", ec, cell.total_cells()),
                vfg,
                pbg,
            );
        }
        ...
        (ZoomLevel::Cellular, SelectedEntity::Plant(idx)) if idx < world.plants.len() => {
            ...
            let cell = &plant.cellular;
            let mut r = 1;
            buf.write_str(cx, r, &format!("Plant #{} Cellular", idx), tfg, pbg);
            r += 2;
            for (name, tissue) in [
                ("LEAF", PlantTissue::Leaf),
                ("STEM", PlantTissue::Stem),
                ("ROOT", PlantTissue::Root),
                ("MERI", PlantTissue::Meristem),
            ] {
                let s = cell.cluster_snapshot(tissue);
                buf.write_str(
                    cx,
                    r,
                    &format!("\u{250C}\u{2500} {} ({:.0} cells)", name, s.cell_count),
                    hfg,
                    pbg,
                );
                r += 1;
                for (mname, mval) in [
                    ("Vital", s.vitality),
                    ("ATP  ", s.state_atp.clamp(0.0, 1.0)),
                    ("Gluc ", s.state_glucose.clamp(0.0, 1.0)),
                    ("Strch", s.state_starch.clamp(0.0, 1.0)),
                    ("Water", s.state_water.clamp(0.0, 1.0)),
                    ("NO3  ", s.state_nitrate.clamp(0.0, 1.0)),
                ] {
                    buf.write_str(cx, r, "\u{2502} ", dfg, pbg);
                    buf.write_str(cx + 2, r, mname, lfg, pbg);
                    buf.write_str(cx + 8, r, &bar_str(mval, bw), bfg, pbg);
                    buf.write_str(cx + 9 + bw, r, &format!(" {:.2}", mval), vfg, pbg);
                    r += 1;
                }
                buf.write_str(
                    cx,
                    r,
                    "\u{2514}\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}",
                    dfg,
                    pbg,
                );
                r += 1;
                if r >= buf.height.saturating_sub(6) {
                    break;
                }
            }
        }
        ...
        | (ZoomLevel::Cellular, SelectedEntity::Fly(idx))
            if idx < world.flies.len() =>
        {
            let fly = &world.flies[idx];
            let b = fly.body_state();
            let mut r = 1;
            buf.write_str(
                cx,
                r,
                &format!("Fly #{} ({:.1},{:.1},{:.1})", idx, b.x, b.y, b.z),
                tfg,
                pbg,
            );
            r += 1;
            buf.write_str(
                ...
                &format!(
                    "State: {} E:{:.1}uJ",
                    if b.is_flying { "Flying" } else { "Landed" },
                    b.energy
                ),
                vfg,
                pbg,
            ...
            buf.write_str(
                cx,
                r,
                "\u{2500}\u{2500}\u{2500} Body State \u{2500}\u{2500}\u{2500}",
                hfg,
                pbg,
            );
            r += 1;
            ...
            buf.write_str(cx + 10 + bw, r, &format!(" {:.1}", b.energy), vfg, pbg);
            r += 1;
            for (name, val) in [
                ("Speed   ", b.speed),
                ("Heading ", b.heading),
                ...
                buf.write_str(cx, r, name, lfg, pbg);
                buf.write_str(cx + 9, r, &format!("{:.2}", val), vfg, pbg);
                r += 1;
            }
            r += 1;
            ...
            buf.write_str(
                cx,
                r,
                "\u{2500}\u{2500}\u{2500} Local Chem \u{2500}\u{2500}\u{2500}",
                hfg,
                pbg,
            );
            r += 1;
            for (name, sp) in [
                ("O2     ", TerrariumSpecies::OxygenGas),
                ("Glucose", TerrariumSpecies::Glucose),
                ("CO2    ", TerrariumSpecies::CarbonDioxide),
            ] {
                let field = world.substrate.species_field(sp);
                let val = if idx_2d < field.len() / depth {
                    field[idx_2d]
                } else {
                    0.0
                };
                ...
                buf.write_str(cx + 9 + bw, r, &format!(" {:.3}", val), vfg, pbg);
                r += 1;
            }
        }
        ...
        | (ZoomLevel::Cellular, SelectedEntity::Water(idx))
            if idx < world.waters.len() =>
        {
            let water = &world.waters[idx];
            let mut r = 1;
            buf.write_str(cx, r, &format!("Water [{},{}]", water.x, water.y), tfg, pbg);
            r += 1;
            buf.write_str(
                cx,
                r,
                &format!(
                    "Volume: {:.1}  Evap: {:.3}/s",
                    water.volume, water.evaporation_rate
                ),
                vfg,
                pbg,
            );
            ...
            if water.x < gw && water.y < gh {
                ...
                buf.write_str(
                    cx,
                    r,
                    "\u{2500}\u{2500}\u{2500} Dissolved \u{2500}\u{2500}\u{2500}\u{2500}",
                    hfg,
                    pbg,
                );
                r += 1;
                for (name, sp) in [
                    ("Glucose", TerrariumSpecies::Glucose),
                    ("O2     ", TerrariumSpecies::OxygenGas),
                    ("NO3    ", TerrariumSpecies::Nitrate),
                    ("NH4    ", TerrariumSpecies::Ammonium),
                    ("CO2    ", TerrariumSpecies::CarbonDioxide),
                ] {
                    let field = world.substrate.species_field(sp);
                    let val = if idx_2d < field.len() / depth {
                ...
                buf.write_str(
                    cx,
                    r,
                    "\u{2500}\u{2500}\u{2500} Ecology (mean) \u{2500}",
                    hfg,
                    pbg,
                );
                r += 1;
                buf.write_str(
                    cx,
                    r,
                    &format!("Microbes  {:.4} gC", snap.mean_microbes),
                    vfg,
                    pbg,
                );
                r += 1;
                buf.write_str(
                    cx,
                    r,
                    &format!("Symbionts {:.4} gC", snap.mean_symbionts),
                    vfg,
                    pbg,
                );
            }
        ...
        | (ZoomLevel::Cellular, SelectedEntity::SoilCell) => {
            ...
            let idx_2d = gy * gw + gx;
            let mut r = 1;
            buf.write_str(cx, r, &format!("Soil Cell [{},{}]", gx, gy), tfg, pbg);
            r += 2;
            buf.write_str(
                cx,
                r,
                "\u{2500}\u{2500}\u{2500} Chemistry \u{2500}\u{2500}\u{2500}\u{2500}",
                hfg,
                pbg,
            );
            r += 1;
            let moisture = world.moisture_field();
            let m_val = if idx_2d < moisture.len() {
                moisture[idx_2d]
            } else {
                0.0
            };
            ...
            buf.write_str(cx + 10 + bw, r, &format!(" {:.3}", m_val), vfg, pbg);
            r += 1;
            ...
            buf.write_str(
                cx + 9,
                r,
                &bar_str(snap.mean_microbes.clamp(0.0, 1.0), bw),
                bfg,
                pbg,
            );
            buf.write_str(
            ...
            buf.write_str(
                cx + 9,
                r,
                &bar_str(snap.mean_symbionts.clamp(0.0, 1.0), bw),
                bfg,
                pbg,
            );
            buf.write_str(
            ...
            buf.write_str(
                cx,
                r,
                "\u{2500}\u{2500}\u{2500} Substrate \u{2500}\u{2500}\u{2500}\u{2500}",
                hfg,
                pbg,
            );
            r += 1;
            let substrate_species = [
                ...
                ("NO3    ", TerrariumSpecies::Nitrate),
                ("CO2    ", TerrariumSpecies::CarbonDioxide),
                ("ATP flx", TerrariumSpecies::AtpFlux),
        ...
        (ZoomLevel::Molecular, _) => {
            ...
            let idx_2d = gy * gw + gx;
            let mut r = 1;
            buf.write_str(cx, r, &format!("Molecular [{},{}]", gx, gy), tfg, pbg);
            r += 2;

            // Element legend
            let elems: [(&str, (u8, u8, u8)); 6] = [
                ("C", (139, 90, 43)),
                ("H", (200, 200, 200)),
                ("O", (255, 60, 60)),
                ("N", (60, 60, 255)),
            ...
            let mut ex = cx;
            for (sym, col) in &elems {
                buf.set(ex, r, '\u{2588}', *col, pbg);
                buf.write_str(ex + 1, r, sym, *col, pbg);
                ex += sym.len() + 2;
            }
            r += 2;
            ...
            buf.write_str(
                cx,
                r,
                "\u{2500}\u{2500}\u{2500} All 14 Species \u{2500}\u{2500}",
                hfg,
                pbg,
            );
            r += 1;
            let all_sp = [
                ("Carbon  ", TerrariumSpecies::Carbon, (139, 90, 43)),
                ("Hydrogen", TerrariumSpecies::Hydrogen, (200, 200, 200)),
                ("Oxygen  ", TerrariumSpecies::Oxygen, (255, 60, 60)),
                ("Nitrogen", TerrariumSpecies::Nitrogen, (60, 60, 255)),
                ("Phosphor", TerrariumSpecies::Phosphorus, (255, 165, 0)),
                ("Sulfur  ", TerrariumSpecies::Sulfur, (255, 255, 0)),
                ("Water   ", TerrariumSpecies::Water, (60, 140, 255)),
                ("Glucose ", TerrariumSpecies::Glucose, (200, 140, 60)),
                ("O2 gas  ", TerrariumSpecies::OxygenGas, (180, 60, 60)),
                ("Ammonium", TerrariumSpecies::Ammonium, (120, 180, 60)),
                ("Nitrate ", TerrariumSpecies::Nitrate, (60, 160, 120)),
                ("CO2     ", TerrariumSpecies::CarbonDioxide, (160, 160, 160)),
                ("H+ (pH) ", TerrariumSpecies::Proton, (200, 100, 200)),
                ("ATP flux", TerrariumSpecies::AtpFlux, (255, 200, 80)),
            ];
            for (name, sp, col) in &all_sp {
                ...
                buf.write_str(cx + 10 + bw, r, &format!(" {:.4}", val), vfg, pbg);
                r += 1;
                if r >= buf.height.saturating_sub(6) {
                    break;
                }
            ...
            if r + 8 < buf.height.saturating_sub(6) {
                ...
                buf.write_str(
                    cx,
                    r,
                    "\u{2500}\u{2500}\u{2500} Particles \u{2500}\u{2500}\u{2500}\u{2500}",
                    hfg,
                    pbg,
                );
                r += 1;
                ...
                let mh = 6;
                let c_val = {
                    let f = world.substrate.species_field(TerrariumSpecies::Carbon);
                    if idx_2d < f.len() / depth {
                        f[idx_2d]
                    } else {
                        0.0
                    }
                ...
                let h_val = {
                    let f = world.substrate.species_field(TerrariumSpecies::Hydrogen);
                    if idx_2d < f.len() / depth {
                        f[idx_2d]
                    } else {
                        0.0
                ...
                let o_val = {
                    let f = world.substrate.species_field(TerrariumSpecies::Oxygen);
                    if idx_2d < f.len() / depth {
                        f[idx_2d]
                    } else {
                        0.0
                ...
                let n_val = {
                    let f = world.substrate.species_field(TerrariumSpecies::Nitrogen);
                    if idx_2d < f.len() / depth {
                        f[idx_2d]
                    } else {
                        0.0
                ...
                for dx in 0..mw {
                    buf.set(cx + 1 + dx, r, '\u{2500}', dfg, pbg);
                }
                buf.set(cx + 1 + mw, r, '\u{2510}', dfg, pbg);
                r += 1;
                for dy in 0..mh {
                    buf.set(cx, r, '\u{2502}', dfg, pbg);
                    buf.set(cx + 1 + mw, r, '\u{2502}', dfg, pbg);
                    for dx in 0..mw {
                        ...
                        let (ch, col) = if hash < thresh_c {
                            ('\u{2588}', (139, 90, 43))
                        } else if hash < thresh_h {
                            ('\u{2593}', (200, 200, 200))
                        } else if hash < thresh_o {
                            ('\u{2592}', (255, 60, 60))
                        } else if hash < thresh_n {
                            ('\u{2588}', (60, 60, 255))
                        } else {
                            (' ', pbg)
                        };
        ...
        _ => {
            ...
            let idx_2d = gy * gw + gx;
            let mut r = 1;
            buf.write_str(cx, r, &format!("Cursor [{},{}]", gx, gy), tfg, pbg);
            r += 2;
            let moisture = world.moisture_field();
            let m = if idx_2d < moisture.len() {
                moisture[idx_2d]
            } else {
                0.0
            };
            ...
            buf.write_str(cx + 10 + bw, r, &format!(" {:.2}", m), vfg, pbg);
            r += 2;
            buf.write_str(cx, r, "Press Enter to select", dfg, pbg);
            r += 1;
            buf.write_str(cx, r, "nearest entity, or", dfg, pbg);
            r += 1;
...
fn draw_cursor(buf: &mut ScreenBuffer, world: &TerrariumWorld, vs: &ViewState, fi: usize) {
    ...
    let sx = ox + cx as isize * scale;
    let sy = oy + cy as isize;
    if sx >= 0 && sy >= 0 {
        let sxu = sx as usize;
        let syu = sy as usize;
        let blink = fi % 8 < 5;
        ...
        if syu > 0 {
            buf.set(sxu, syu - 1, '\u{2502}', cfig, (10, 10, 20));
        }
        if syu + 1 < buf.height {
            buf.set(sxu, syu + 1, '\u{2502}', cfig, (10, 10, 20));
        }
        if sxu > 0 {
            buf.set(sxu - 1, syu, '\u{2500}', cfig, (10, 10, 20));
        }
        if sxu + 1 < buf.width {
            buf.set(sxu + 1, syu, '\u{2500}', cfig, (10, 10, 20));
        }
...
fn select_nearest(world: &TerrariumWorld, vs: &ViewState) -> SelectedEntity {
    let cx = vs.cursor_x as f32;
    let cy = vs.cursor_y as f32;
    let mut best_d = f32::MAX;
    let mut best = SelectedEntity::SoilCell;
    for (i, p) in world.plants.iter().enumerate() {
        let d = ((p.x as f32 - cx).powi(2) + (p.y as f32 - cy).powi(2)).sqrt();
        if d < best_d && d < 3.0 {
            best_d = d;
            best = SelectedEntity::Plant(i);
        }
    }
    for (i, f) in world.flies.iter().enumerate() {
        ...
        if d < best_d && d < 3.0 {
            best_d = d;
            best = SelectedEntity::Fly(i);
        }
    }
    for (i, w) in world.waters.iter().enumerate() {
        if w.alive {
            let d = ((w.x as f32 - cx).powi(2) + (w.y as f32 - cy).powi(2)).sqrt();
            if d < best_d && d < 2.0 {
                best_d = d;
                best = SelectedEntity::Water(i);
            }
        }
    }
    best
...
fn render_scene(
    ...
    vs: &ViewState,
    history: &[TerrariumWorldSnapshot],
    tw: usize,
    th: usize,
) {
    let zl = vs.zoom_level();
    if zl != ZoomLevel::Ecosystem && vs.mode != ViewMode::Dashboard {
        ...
        match vs.mode {
            ViewMode::Isometric => {
                ...
                if vs.show_minimap {
                    draw_minimap(
                        buf,
                        world,
                        buf.width.saturating_sub(world.config.width + 4),
                        3,
                    );
                }
            }
            ...
            ViewMode::Split => {
                let hw = tw / 2;
                let mut l = ScreenBuffer::new(hw, th);
                let mut r = ScreenBuffer::new(hw, th);
                render_isometric(&mut l, world, snap, fi, vs);
                r.clear();
                render_topdown_region(&mut r, world, snap, fi, vs, 0, hw);
                buf.clear();
                for y in 0..th {
                    for x in 0..hw {
                        let lc = &l.cells[y * hw + x];
                        buf.set(x, y, lc.ch, lc.fg, lc.bg);
                        if x < r.width {
                            let rc = &r.cells[y * r.width + x];
                            buf.set(x + hw, y, rc.ch, rc.fg, rc.bg);
                        }
                    }
                }
            }
        ...
        match arg.as_str() {
            ...
            "--mode" => {
                cli.mode = match args.next().ok_or("missing --mode")?.as_str() {
                    ...
                    o => return Err(format!("unknown mode: {o}")),
                }
            }
            "--scenario" => {
                cli.scenario = match args.next().ok_or("missing --scenario")?.as_str() {
                    "normal" => ScenarioPreset::Normal,
                    "drought" => ScenarioPreset::Drought,
                    "heat" => ScenarioPreset::Heat,
                    "competition" => ScenarioPreset::Competition,
                    "dormancy" => ScenarioPreset::Dormancy,
                    "feast" => ScenarioPreset::Feast,
...
fn main() -> ExitCode {
    ...
    let key_rx = match (raw_ok, tty) {
        ...
        _ => {
            eprintln!("Warning: no tty for interactive controls");
            None
        }
    };
    ...
    loop {
        ...
        if let Some(ref rx) = key_rx {
            while let Ok(key) = rx.try_recv() {
                ...
                match key {
                    ...
                    KeyInput::Char('\x1b') => {
                        if vs.selected != SelectedEntity::None {
                            vs.selected = SelectedEntity::None;
                        } else {
                            quit = true;
                        }
                    }
                    ...
                    KeyInput::Char('\n') => {
                        if zl != ZoomLevel::Ecosystem {
                            vs.selected = select_nearest(&world, &vs);
                        }
                    }
                    KeyInput::Char('w') | KeyInput::Up => {
                        if zl != ZoomLevel::Ecosystem {
                            vs.cursor_y = vs.cursor_y.saturating_sub(1);
                        } else {
                            vs.camera_y += 2;
                        }
                    }
                    KeyInput::Char('s') | KeyInput::Down => {
                        if zl != ZoomLevel::Ecosystem {
                            vs.cursor_y =
                                (vs.cursor_y + 1).min(world.config.height.saturating_sub(1));
                        } else {
                            vs.camera_y -= 2;
                        }
                    }
                    KeyInput::Char('a') | KeyInput::Left => {
                        if zl != ZoomLevel::Ecosystem {
                            vs.cursor_x = vs.cursor_x.saturating_sub(1);
                        } else {
                            vs.camera_x += 2;
                        }
                    }
                    KeyInput::Char('d') | KeyInput::Right => {
                        if zl != ZoomLevel::Ecosystem {
                            vs.cursor_x =
                                (vs.cursor_x + 1).min(world.config.width.saturating_sub(1));
                        } else {
                            vs.camera_x -= 2;
                        }
                    }
                    ...
                    KeyInput::Char('D') => {
                        scenario = ScenarioPreset::Drought;
                        apply_scenario(&mut world, scenario);
                    }
                    KeyInput::Char('H') => {
                        scenario = ScenarioPreset::Heat;
                        apply_scenario(&mut world, scenario);
                    }
                    KeyInput::Char('N') => {
                        scenario = ScenarioPreset::Normal;
                    ...
                    KeyInput::Char('C') => {
                        scenario = ScenarioPreset::Competition;
                        apply_scenario(&mut world, scenario);
                    }
                    KeyInput::Char('F') => {
                        scenario = ScenarioPreset::Feast;
                        apply_scenario(&mut world, scenario);
                    }
                    _ => {}
                }
        ...
        if prev_zl == ZoomLevel::Ecosystem
            && new_zl != ZoomLevel::Ecosystem
            && vs.selected == SelectedEntity::None
        {
            vs.selected = select_nearest(&world, &vs);
        }
        if new_zl == ZoomLevel::Ecosystem && prev_zl != ZoomLevel::Ecosystem {
            vs.selected = SelectedEntity::None;
        }
        ...
        buf.write_str(
            buf.width.saturating_sub(fps_str.len() + 1),
            0,
            &fps_str,
            (255, 255, 100),
            (20, 20, 40),
        ...
        if scenario != ScenarioPreset::Normal {
            ...
            buf.write_str(
                buf.width.saturating_sub(sc_str.len() + 1),
                1,
                &sc_str,
                scenario.color(),
                (40, 20, 20),
        ...
        print!("{}", buf.render(cli.use_color));
        let _ = io::stdout().flush();
