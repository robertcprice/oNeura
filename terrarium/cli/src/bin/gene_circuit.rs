...
use oneura_core::terrarium::{optimize_gene_circuit, GeneCircuitParams, GeneCircuitSpec};
...
fn print_help() {
    ...
    println!(
        "  Mean protein = (txn_rate * k_on / ((k_on + k_off) * mrna_deg)) * (tln_rate / prot_deg)"
    );
...
fn run_design(args: &[String]) {
    ...
    if json_output {
        ...
        println!();
        println!(
            "Target:  Fano={:.2}  Mean={:.1}  CV={:.4}",
            target_fano, target_mean, target_cv
        );
        println!(
            "Search:  pop={}  gen={}  seed={}",
        ...
        println!(
            "  Promoter ON rate:        {:.6}",
            result.best_params.promoter_on_rate
        );
        println!(
            "  Promoter OFF rate:       {:.6}",
            result.best_params.promoter_off_rate
        );
        println!(
            "  Transcription rate:      {:.6}",
            result.best_params.transcription_rate
        );
        println!(
            "  mRNA degradation rate:   {:.6}",
            result.best_params.mrna_degradation_rate
        );
        println!(
            "  Translation rate:        {:.6}",
            result.best_params.translation_rate
        );
        println!(
            "  Protein degradation:     {:.6}",
            result.best_params.protein_degradation_rate
        );
        println!(
            "  Burst size:              {:.2}",
            result.best_params.burst_size
        );
        println!();
        println!("ACHIEVED NOISE:");
        println!(
            "  Fano factor:  {:.4}  (target: {:.2}, error: {:.2}%)",
            result.achieved_fano,
            target_fano,
            ((result.achieved_fano - target_fano) / target_fano * 100.0).abs()
        );
        println!(
            "  Mean protein: {:.2}  (target: {:.1}, error: {:.2}%)",
            result.achieved_mean_protein,
            target_mean,
            ((result.achieved_mean_protein - target_mean) / target_mean * 100.0).abs()
        );
        println!(
            "  CV:           {:.6}  (target: {:.4}, error: {:.2}%)",
            result.achieved_cv,
            target_cv,
        ...
        println!(
            "  Proteins per burst: ~{:.0}",
            result.best_params.burst_size * burst_dur * result.best_params.transcription_rate
        );
    }
...
fn run_analyze(args: &[String]) {
    ...
    if json_output {
        ...
        println!(
            "  k_on  = {:.6}    k_off = {:.6}",
            params.promoter_on_rate, params.promoter_off_rate
        );
        println!(
            "  txn   = {:.6}    mRNA deg = {:.6}",
            params.transcription_rate, params.mrna_degradation_rate
        );
        println!(
            "  tln   = {:.6}    prot deg = {:.6}",
            params.translation_rate, params.protein_degradation_rate
        );
...
fn run_sweep(args: &[String]) {
    ...
    for &target_fano in &fano_targets {
        ...
        for &target_mean in &mean_targets {
            let spec = GeneCircuitSpec {
                target_fano,
                target_mean_protein: target_mean,
                target_cv: (target_fano / target_mean as f32).sqrt(),
            };
            let result = optimize_gene_circuit(&spec, population, generations, seed);
            seed += 1;
        ...
        .and_then(|i| args.get(i + 1))
    {
        std::fs::write(
            output_path,
            serde_json::to_string_pretty(&sweep_results).unwrap(),
        )
        .expect("Failed to write output file");
        if !json_output {
            println!();
