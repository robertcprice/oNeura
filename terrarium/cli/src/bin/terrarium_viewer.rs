...
use oneura_core::terrarium::{
    TerrariumDemoPreset, TerrariumTopdownView, TerrariumWorld, TerrariumWorldSnapshot,
};
use std::env;
...
fn print_usage() {
    eprintln!(
        "Usage: cargo run --release --bin terrarium_viewer -- [--seed <n>] [--fps <n>] [--frames <n>] [--cell <px>] [--preset <demo|terrarium|aquarium>] [--cpu-substrate]"
    );
}
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--preset" => {
                ...
                cli.preset = TerrariumDemoPreset::parse(&value).ok_or_else(|| {
                    format!("invalid preset '{value}' (expected demo, terrarium, or aquarium)")
                })?;
            }
...
fn snap_levels(value: f32, levels: usize) -> f32 {
    let levels = levels.max(2) as f32;
    ((value.clamp(0.0, 1.0) * (levels - 1.0)).round()) / (levels - 1.0)
}
...
fn field_color(
    ...
    let v = snap_levels(value.clamp(0.0, 1.0).sqrt(), 6);
    let day = snap_levels(daylight.clamp(0.2, 1.0), 5);
    match (preset, view) {
        (TerrariumDemoPreset::MicroAquarium, TerrariumTopdownView::Terrain) => blend(
            lerp_color((12, 22, 34), (40, 88, 116), v),
            rgb(142, 212, 224),
            0.14 * day,
        ),
        (TerrariumDemoPreset::MicroAquarium, TerrariumTopdownView::SoilMoisture) => {
            lerp_color((18, 36, 52), (42, 166, 204), v)
        }
        (TerrariumDemoPreset::MicroAquarium, TerrariumTopdownView::Canopy) => {
            lerp_color((10, 42, 36), (88, 176, 124), v)
        }
        (TerrariumDemoPreset::MicroAquarium, TerrariumTopdownView::Chemistry) => {
            lerp_color((14, 28, 38), (212, 190, 98), v)
        }
        (TerrariumDemoPreset::MicroAquarium, TerrariumTopdownView::Odor) => {
            lerp_color((12, 18, 22), (224, 116, 74), v)
        }
        (TerrariumDemoPreset::MicroAquarium, TerrariumTopdownView::GasExchange) => {
            lerp_color((8, 26, 32), (118, 224, 210), v)
        }
        (_, TerrariumTopdownView::Terrain) => blend(
            lerp_color((40, 30, 24), (118, 96, 68), v),
            rgb(206, 194, 154),
            0.16 * day,
        ),
        (_, TerrariumTopdownView::SoilMoisture) => lerp_color((66, 40, 24), (62, 132, 164), v),
        (_, TerrariumTopdownView::Canopy) => lerp_color((16, 44, 20), (118, 184, 84), v),
        (_, TerrariumTopdownView::Chemistry) => lerp_color((16, 20, 28), (232, 168, 64), v),
        (_, TerrariumTopdownView::Odor) => lerp_color((12, 12, 14), (214, 88, 40), v),
        (_, TerrariumTopdownView::GasExchange) => lerp_color((8, 18, 26), (106, 214, 196), v),
    }
}
...
fn draw_pixel_cell(
    ...
    let x = grid_x * cell_px;
    let y = grid_y * cell_px;
    draw_rect(buffer, screen_w, screen_h, x, y, cell_px, cell_px, base);
    let border = blend(base, rgb(10, 10, 12), 0.18);
    draw_rect(buffer, screen_w, screen_h, x, y, cell_px, 1, border);
    draw_rect(buffer, screen_w, screen_h, x, y, 1, cell_px, border);
    if cell_px < 6 {
        return;
    }
    let detail = (cell_px / 4).max(1);
    let highlight = blend(base, rgb(236, 236, 228), 0.16);
    let shadow = blend(base, rgb(10, 10, 12), 0.26);
    draw_rect(
        buffer,
        screen_w,
        screen_h,
    ...
    draw_rect(
        ...
        screen_w,
        screen_h,
        x + cell_px.saturating_sub(detail + 1),
        y + cell_px.saturating_sub(detail + 1),
        detail,
        detail,
        shadow,
    );
    if (grid_x + grid_y) % 2 == 0 && cell_px >= 10 {
        draw_rect(
            buffer,
            screen_w,
            screen_h,
            x + cell_px / 2,
            y + cell_px / 2,
            detail.saturating_sub(1).max(1),
            detail.saturating_sub(1).max(1),
            blend(base, rgb(248, 238, 198), 0.08),
        );
    }
}
...
fn draw_world(
    ...
    buffer.fill(if preset == TerrariumDemoPreset::MicroAquarium {
        rgb(8, 16, 20)
    } else {
        rgb(14, 16, 18)
    });
    ...
    for y in 0..world_h {
        for x in 0..world_w {
            let idx = y * world_w + x;
            let color = field_color(preset, view, field[idx], snapshot.light);
            draw_pixel_cell(buffer, screen_w, screen_h, x, y, cell_px, color);
        }
    }
    ...
    for water in &world.waters {
        if water.alive {
            ...
            draw_rect(
                ...
                (cell_px / 2).max(2),
                (cell_px / 2).max(2),
                if preset == TerrariumDemoPreset::MicroAquarium {
                    rgb(112, 214, 240)
                } else {
                    rgb(72, 168, 220)
                },
    ...
    for plant in &world.plants {
        ...
        draw_rect(
            ...
            (cell_px / 6).max(1),
            (cell_px / 3).max(1),
            blend(accent, rgb(12, 18, 12), 0.34),
        );
    }
...
fn draw_panel(
    ...
    let info = [
        ...
        format!("voxel {:.2}mm", world.config.cell_size_mm),
...
fn update_title(
    ...
    window.set_title(&format!(
        "oNeura {} | time={} | view={} | plants={} fruits={} flies={} | food={:.2} | cells={:.0} | substrate={}",
        preset.label(),
        world.time_label(),
        view.label(),
        snapshot.plants,
...
fn main() -> ExitCode {
    ...
    let mut world = match TerrariumWorld::demo_preset(cli.seed, !cli.cpu_substrate, cli.preset) {
        Ok(world) => world,
        Err(err) => {
    ...
    let mut window = match Window::new(
        &format!("oNeura {}", cli.preset.label()),
        width,
        height,
        WindowOptions {
            resize: true,
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        if window.is_key_pressed(Key::R, KeyRepeat::No) {
            match TerrariumWorld::demo_preset(cli.seed, !cli.cpu_substrate, cli.preset) {
        ...
        update_title(&mut window, &snapshot, &world, cli.preset, view);
