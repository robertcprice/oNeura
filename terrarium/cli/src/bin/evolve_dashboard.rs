...
use oneura_core::terrarium::GenerationTelemetry;
use std::env;
use std::fs;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                ...
                println!(
                    "  --output <DIR>      Output directory for SVG files (default: figures/)"
                );
...
fn plot_param_heatmap(records: &[GenerationTelemetry], title_prefix: &str) -> String {
    ...
    for (gi, rec) in records.iter().enumerate() {
        ...
        for pi in 0..n_params.min(params.len()) {
            ...
            let g_color = ((1.0 - (2.0 * val as f32 - 1.0).abs()) * 128.0) as u8;
...
fn generate_summary(records: &[GenerationTelemetry], title_prefix: &str) -> String {
    ...
    let mode = records[0].mode.as_deref().unwrap_or("unknown");
    ...
    if records.len() >= 2 {
        ...
        report.push_str(&format!("**Fitness improvement**: {:.1}%\n\n", improvement));
    }
...
fn main() {
    ...
    eprintln!(
        "\nDashboard complete: {} files written to {}",
        files_written,
        args.output.display()
    );
}
