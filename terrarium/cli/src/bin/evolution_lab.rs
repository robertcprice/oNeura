...
use oneura_core::terrarium::{
    price_equation, EcoEvoSimulator, EnvironmentalShift, FitnessLandscape, PhyloTraits, PhyloTree,
    WrightFisherSim,
};
...
fn run_simulation(cfg: &Config) {
    ...
    // Header
    eprintln!();
    eprintln!("{BOLD}{CYAN}=== Evolution Lab: Eco-Evolutionary Dynamics ==={RESET}");
    eprintln!(
    ...
    for gen in 0..cfg.generations {
        ...
        if !prev_population_snapshot.is_empty() && !sim.population.is_empty() {
            for t in 0..cfg.n_traits.min(1) {
                let (sel, trans) = price_equation(&prev_population_snapshot, &sim.population, t);
                total_selection += sel;
        ...
        if sim.population.is_empty() {
            eprintln!();
            eprintln!("{BOLD}{BG_RED} EXTINCTION at generation {} {RESET}", gen);
            eprintln!("Population could not survive the environmental shift.");
            break;
        }
    }
    ...
    eprintln!("{BOLD}{CYAN}=== Population Genetics Summary ==={RESET}");
    ...
    if !sim.population.is_empty() {
        ...
        let s_mag: f64 = selection_gradient.iter().map(|s| s * s).sum::<f64>().sqrt();
    ...
    if cfg.mode == Mode::Rescue {
        eprintln!();
        eprintln!("{BOLD}{CYAN}=== Evolutionary Rescue Analysis ==={RESET}");
        if rescue.rescue_detected {
            ...
            eprintln!("{BG_RED}{BOLD} RESCUE IN PROGRESS {RESET} -- population still declining");
            if let Some(dg) = rescue.decline_start {
                eprintln!("  Decline began: gen {}", dg);
            }
        } else if sim.population.is_empty() {
            eprintln!("{BG_RED}{BOLD} RESCUE FAILED -- EXTINCTION {RESET}");
        } else {
    ...
    if cfg.mode == Mode::Drift {
        eprintln!();
        eprintln!("{BOLD}{CYAN}=== Drift Analysis: Wright-Fisher Comparison ==={RESET}");
        if let Some(ref wf) = wf_sim {
            eprintln!("Wright-Fisher final allele freq: {:.4}", wf.allele_freq);
            eprintln!(
            ...
            if wf_freq_history.len() > 10 && eco_freq_history.len() > 10 {
                ...
                eprintln!("WF allele freq variance:         {:.6}", wf_var);
                eprintln!("Eco-evo trait freq variance:      {:.6}", eco_var);
    ...
    if cfg.mode == Mode::Speciation {
        eprintln!();
        eprintln!("{BOLD}{CYAN}=== Speciation Analysis ==={RESET}");
        ...
        if !displacement.is_empty() {
            eprintln!();
            eprintln!("{BOLD}Character Displacement:{RESET}");
        ...
        for org in &sim.population {
            if let Some(entry) = lineage_counts
                .iter_mut()
                .find(|(id, _)| *id == org.lineage_id)
            {
                entry.1 += 1;
            } else {
    ...
    eprintln!();
    eprintln!("{BOLD}{CYAN}=== Phylogenetic Summary ==={RESET}");
    let living = tree.living_nodes();
    let phylo_spec_events = tree.speciation_events(0.3);
    let faith_pd = tree.phylogenetic_diversity();

    eprintln!("Total lineages:     {BOLD}{}{RESET}", tree.len());
    eprintln!("Living lineages:    {BOLD}{}{RESET}", living.len());
    eprintln!("Tree depth:         {} generations", tree.tree_depth());
    eprintln!("Mean branch length: {:.2}", tree.mean_branch_length());
    eprintln!(
    ...
    if cfg.export_newick {
        eprintln!();
        eprintln!("{BOLD}{CYAN}=== Newick Tree ==={RESET}");
    ...
    // ── Final summary ──────────────────────────────────────────────────────
    eprintln!();
    eprintln!("{BOLD}{MAGENTA}=== Simulation Complete ==={RESET}");
    eprintln!(
...
fn build_event_string(
    cfg: &Config,
    gen: usize,
    result: &oneura_core::terrarium::EcoEvoStepResult,
    sim: &EcoEvoSimulator,
) -> String {
    ...
    if gen as u32 == cfg.shift_gen && cfg.shift_type != ShiftType::None {
        match cfg.mode {
            ...
            Mode::Adaptation => {
                events.push(format!("{YELLOW}SHIFT!{RESET} Optimum -> [0.8, ...]"));
            }
            Mode::Speciation => {
                events.push(format!("{MAGENTA}Disruptive selection activated{RESET}"));
            }
            Mode::Drift => {}
        }
    }
