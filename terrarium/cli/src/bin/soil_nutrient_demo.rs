...
use oneura_core::nutrient_cycling::{
    limiting_nutrient, DecompositionStage, LitterCohort, NutrientCycler, OrganicPool,
};
use oneura_core::terrarium::{ClimateEngine, ClimateScenario};
use oneura_core::terrarium::{CommunityAssembler, CrossFeedingLink, MicrobialTaxon, Resource};
...
fn run_simulation(
    ...
    for yr_offset in 0..=years {
        ...
        for event in &extremes {
            match event {
                oneura_core::climate_scenarios::ExtremeEvent::Drought { severity, .. } => {
                ...
                oneura_core::climate_scenarios::ExtremeEvent::Heatwave { .. } => {
                    // Heatwave boosts decomposers, stresses others
                    community.perturb_resource(0, 1.2);
                    community.perturb_taxon(1, 0.1);
                }
                oneura_core::climate_scenarios::ExtremeEvent::Flood { magnitude } => {
        ...
        let limit_str = match limit {
            oneura_core::nutrient_cycling::LimitingNutrient::Nitrogen => "N",
            oneura_core::nutrient_cycling::LimitingNutrient::Phosphorus => "P",
            oneura_core::nutrient_cycling::LimitingNutrient::CoLimited => "Co-limited",
            oneura_core::nutrient_cycling::LimitingNutrient::Neither => "Neither",
        };
