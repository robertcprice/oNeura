...
use super::color::{blend, rgb_f, u32_to_v3};
use super::lighting::{moon_position, sky_color_at, sky_gradient};
use super::math::*;
use super::mesh::{EntityTag, Triangle};
use super::{
    CELL_SIZE, FOG_FAR, FOG_NEAR, HEIGHT_SCALE, NEAR, SHADOW_DARKEN, SHADOW_STEPS, SHADOW_STEP_SIZE,
};
use rayon::prelude::*;
...
impl Rasterizer {
    ...
    pub fn clear(&mut self, light: f32) {
        ...
        self.world_pos_buf
            .par_iter_mut()
            .for_each(|p| *p = [0.0; 3]);
        self.world_normal_buf
            .par_iter_mut()
            .for_each(|n| *n = [0.0, 1.0, 0.0]);
        self.entity_tag_buf
            .par_iter_mut()
            .for_each(|t| *t = EntityTag::None);
    }
        ...
        for tri in tris {
            ...
            for py in min_y..=max_y {
                ...
                for px in min_x..=max_x {
                    ...
                    self.z_buf[idx] = z;
                    let wp = add3(
                        add3(scale3(tri.v[0].pos, b0), scale3(tri.v[1].pos, b1)),
                        scale3(tri.v[2].pos, b2),
                    );
                    let wn = normalize3(add3(
                        add3(scale3(tri.v[0].normal, b0), scale3(tri.v[1].normal, b1)),
                        scale3(tri.v[2].normal, b2),
                    ));
                    self.world_pos_buf[idx] = wp;
                    self.world_normal_buf[idx] = wn;
                    self.entity_tag_buf[idx] = tri.tag;
                    let base_color = add3(
                        add3(scale3(tri.v[0].color, b0), scale3(tri.v[1].color, b1)),
                        scale3(tri.v[2].color, b2),
                    );
                    if !realistic {
                    ...
                    let shininess =
                        tri.v[0].shininess * b0 + tri.v[1].shininess * b1 + tri.v[2].shininess * b2;
                    let ambient = 0.28;
    ...
    pub fn shadow_pass(&mut self, moisture: &[f32], gw: usize, gh: usize, sun_dir: V3) {
        ...
        let shadow_flags: Vec<bool> = (0..npx)
            .into_par_iter()
            .map(|idx| {
                if z_buf[idx] >= f32::MAX * 0.9 {
                    return false;
                }
                ...
                for step in 1..=SHADOW_STEPS {
                    ...
                    let terrain_h = (m00 * (1.0 - fx) * (1.0 - fz)
                        + m10 * fx * (1.0 - fz)
                        + m01 * (1.0 - fx) * fz
                        + m11 * fx * fz)
                        * HEIGHT_SCALE;
                    if sample[1] < terrain_h {
                        return true;
                    }
                    if sample[1] > HEIGHT_SCALE * 1.5 + 1.0 {
                        return false;
                    }
                }
                false
        ...
        self.color_buf
            .par_iter_mut()
            .zip(shadow_flags.par_iter())
            .for_each(|(c, &in_shadow)| {
                if in_shadow {
                    let cv = u32_to_v3(*c);
                    *c = rgb_f(
                        cv[0] * SHADOW_DARKEN,
    ...
    pub fn draw_selection_outline(&mut self, tag: EntityTag) {
        ...
        for y in 1..(h - 1) {
            for x in 1..(w - 1) {
                let idx = y * w + x;
                if self.entity_tag_buf[idx] != tag {
                    continue;
                }
                let is_edge = self.entity_tag_buf[idx - 1] != tag
