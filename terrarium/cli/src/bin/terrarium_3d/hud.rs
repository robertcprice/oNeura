...
use super::{CELL_SIZE, PANEL_W, TOTAL_H, TOTAL_W, VIEWPORT_W};
...
pub fn draw_panel(
    ...
    draw_text(
        ...
        x,
        y,
        &format!("mode  {}", if realistic { "REALISTIC" } else { "FLAT" }),
        if realistic {
            rgb(230, 200, 88)
        } else {
            rgb(160, 160, 170)
    ...
    for line in [
        ...
        draw_text(buffer, TOTAL_W, TOTAL_H, x, y, &line, rgb(216, 220, 226));
        y += 12;
    }
    y += 8;

    draw_text(buffer, TOTAL_W, TOTAL_H, x, y, "LIGHT", rgb(210, 214, 220));
    y += 10;
    draw_bar(
        buffer,
        TOTAL_W,
    ...
    draw_text(
        ...
        x,
        y,
        &format!("Moon: {} {}", moon_art, moon_name),
        rgb(200, 210, 230),
    );
    y += 12;
    draw_text(
    ...
    draw_text(buffer, TOTAL_W, TOTAL_H, x, y, &tide_label, tide_color);
    y += 14;

    // Circadian rhythm display — show average fly circadian phase
    let metabolisms = &world.fly_metabolisms;
    if !metabolisms.is_empty() {
        let avg_phase: f32 = metabolisms
            .iter()
            .map(|m| m.circadian.phase_hours)
            .sum::<f32>()
            / metabolisms.len() as f32;
        let avg_activity: f32 = metabolisms
            .iter()
            .map(|m| m.circadian.activity_level)
            .sum::<f32>()
            / metabolisms.len() as f32;
        let phase_label = format!(
            "Circadian: {:02.0}:{:02.0}",
            avg_phase.floor(),
            (avg_phase.fract() * 60.0).floor()
        );
        draw_text(
            buffer,
            TOTAL_W,
    ...
    let net_color = if net_carbon >= 0.0 {
        rgb(80, 200, 100)
    } else {
        rgb(230, 80, 70)
    };
    draw_text(buffer, TOTAL_W, TOTAL_H, x, y, &net_str, net_color);
    y += 14;
    ...
    y += 18;
    draw_text(buffer, TOTAL_W, TOTAL_H, x, y, "ENERGY", rgb(210, 214, 220));
    y += 10;
    draw_bar(
        buffer,
        TOTAL_W,
    ...
    for line in [
        ...
        format!("CO2  {:.4}", snapshot.mean_atmospheric_co2),
        format!("O2   {:.2}", snapshot.mean_atmospheric_o2),
    ] {
        draw_text(buffer, TOTAL_W, TOTAL_H, x, y, &line, rgb(184, 190, 198));
        y += 12;
    }
    y += 6;
    ...
    if pop_history.len() > 2 {
        ...
        for (i, (plants, flies)) in pop_history.iter().enumerate() {
            ...
            for dy in 0..ph {
                ...
                if gx < TOTAL_W && py < TOTAL_H {
                    buffer[py * TOTAL_W + gx] = rgb(40, 180, 60);
                }
            }
            ...
            for dy in 0..fh {
                let py = y + graph_h - 2 - dy;
                if gx < TOTAL_W && py < TOTAL_H {
                    buffer[py * TOTAL_W + gx] = rgb(230, 210, 40);
                }
            }
        }
        y += graph_h + 2;
    ...
    // Camera info
    draw_text(buffer, TOTAL_W, TOTAL_H, x, y, "CAMERA", rgb(210, 214, 220));
    y += 10;
    draw_text(
        buffer,
        TOTAL_W,
        ...
        EntityTag::Plant(i) => {
            if let Some(plant) = world.plants.get(i) {
                ...
                draw_text(
                    ...
                    x,
                    y,
                    &format!("ht {:.1}mm  cells {:.0}", h, cells),
                    rgb(200, 210, 200),
                );
                y += 11;
                draw_text(
                ...
                draw_text(buffer, TOTAL_W, TOTAL_H, x, y, "ENERGY", rgb(180, 190, 180));
                y += 9;
                draw_bar(
                    buffer,
                    TOTAL_W,
        ...
        EntityTag::Fly(i) => {
            if let Some(fly) = world.flies.get(i) {
                ...
                draw_text(
                    ...
                    x,
                    y,
                    &format!("E {:.2} spd {:.2}", b.energy, b.speed),
                    rgb(220, 218, 180),
                );
                y += 11;
                draw_text(
        ...
        EntityTag::Fruit(i) => {
            if let Some(fruit) = world.fruits.get(i) {
                draw_text(
                    ...
                    &format!(
                        "sugar {:.3} ripe {:.2}",
                        fruit.source.sugar_content, fruit.source.ripeness
                    ),
                    rgb(220, 180, 150),
                );
                y += 11;
...
fn draw_minimap(buffer: &mut [u32], world: &TerrariumWorld, x: usize, y: usize) {
    ...
    for water in &world.waters {
        if water.alive {
            ...
            if px < TOTAL_W && py < TOTAL_H {
                buffer[py * TOTAL_W + px] = rgb(60, 120, 220);
            }
        }
    }
    // Plant dots (green)
    for plant in &world.plants {
        ...
        if px < TOTAL_W && py < TOTAL_H {
            buffer[py * TOTAL_W + px] = rgb(40, 200, 60);
        }
    }
    // Fly dots (yellow)
    for fly in &world.flies {
        ...
        if px < TOTAL_W && py < TOTAL_H {
            buffer[py * TOTAL_W + px] = rgb(240, 230, 40);
        }
    }
    // Fruit dots (orange)
    for fruit in &world.fruits {
        if fruit.source.alive && fruit.source.sugar_content > 0.01 {
            ...
            if px < TOTAL_W && py < TOTAL_H {
                buffer[py * TOTAL_W + px] = rgb(240, 140, 40);
            }
        }
    }
