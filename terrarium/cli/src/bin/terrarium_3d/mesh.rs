...
pub fn make_cylinder(p0: V3, p1: V3, radius: f32, color: V3, shininess: f32) -> Vec<Triangle> {
    ...
    for i in 0..n_sides {
        ...
        let offset = add3(
            scale3(right, angle.cos() * radius),
            scale3(fwd, angle.sin() * radius),
        );
    ...
    for i in 0..n_sides {
        let j = (i + 1) % n_sides;
        let n = normalize3(cross3(
            sub3(top_ring[i], bottom_ring[i]),
            sub3(bottom_ring[j], bottom_ring[i]),
        ));
        tris.push(Triangle {
            v: [
                Vertex {
