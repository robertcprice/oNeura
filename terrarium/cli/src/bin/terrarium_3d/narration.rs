...
use super::{TOTAL_H, TOTAL_W, VIEWPORT_W};
use oneura_core::terrarium::TerrariumWorldSnapshot;
...
impl Narration {
    ...
    pub fn update(
        ...
        if let Some(prev) = prev_snap {
            ...
            if prev.plants > 3 && snap.plants < prev.plants * 7 / 10 {
                self.set_event(
                    NarrationEvent::PopulationCrash,
    ...
    pub fn draw(&self, buffer: &mut [u32]) {
        if self.current_text.is_empty() || self.fade_timer == 0 {
            return;
        }
