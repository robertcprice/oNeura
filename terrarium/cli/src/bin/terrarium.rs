...
use oneura_core::terrarium::{
    evaluate_stress_metrics_for_best, evolve, evolve_coevolution, evolve_pareto,
    evolve_pareto_stressed, evolve_stress_test, evolve_with_environment, telemetry_from_result,
    CoevolutionMode, EnvironmentalSchedule, EvolutionConfig, FitnessConfig, FitnessObjective,
...
use oneura_core::terrarium_web_handlers::{
    annotations_handler, archive_handler, auth_token, checkpoint_handler, export_bundle,
    ...
    snapshot_history_handler, three_js_handler, tournament_delete, tournament_genome,
...
use oneura_core::terrarium::{
    optimize_drug_protocol, optimize_gene_circuit, telemetry_to_csv, DrugProtocol, GeneCircuitSpec,
};
...
fn run_evolve(args: &[String]) {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--crossover-rate" => {
                config.crossover_rate = args
                    .get(i + 1)
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(config.crossover_rate);
                i += 1;
            }
            "--tournament-size" => {
                config.tournament_size = args
                    .get(i + 1)
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(config.tournament_size);
                i += 1;
            }
            "--elitism" => {
                config.elitism = args
                    .get(i + 1)
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(config.elitism);
                i += 1;
            }
            "--no-flies" => config.no_flies = true,
            "--max-flies" => {
                config.max_flies = args.get(i + 1).and_then(|s| s.parse().ok());
                i += 1;
            }
            "--max-microbes" => {
                config.max_microbes = args.get(i + 1).and_then(|s| s.parse().ok());
                i += 1;
            }
            "--telemetry" => {
                config.telemetry = args.get(i + 1).map(PathBuf::from);
                i += 1;
            }
            "--output-genome" => {
                config.output_genome = args.get(i + 1).map(PathBuf::from);
                i += 1;
            }
            "--snapshot-interval" => {
                config.snapshot_interval = args
                    .get(i + 1)
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(config.snapshot_interval);
                i += 1;
            }
            "--environment" | "--env" => {
...
fn exec_evolve(args: EvolveArgs) {
    ...
    eprintln!(
        "Population: {}  Generations: {}  Frames: {}",
        config.population_size, config.generations, config.frames_per_world
    );
            ...
            Ok(result) => {
                ...
                if let Some(ref path) = args.telemetry {
                    ...
                    let content = if args.export_csv {
                        ...
                        oneura_core::terrarium::telemetry_to_prometheus(&telemetry, "terrarium")
                    } else {
...
fn run_sensitivity(args: &[String]) {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--param" | "-p" => {
                config.param = args.get(i + 1).cloned();
                i += 1;
            }
            "--frames" | "-f" => {
                config.frames = args
            ...
            "--output" | "-o" => {
                config.output = args.get(i + 1).cloned();
                i += 1;
            }
            "--csv" => config.csv = true,
            "--lite" => config.lite = true,
...
fn exec_sensitivity(args: SensitivityArgs) {
    ...
    eprintln!(
        "Frames: {}  Resolution: {}  Seed: {}  Lite: {}",
        args.frames, args.resolution, args.seed, args.lite
    );
    ...
    let mut world = if args.lite {
        genome.build_world_lite().unwrap()
    } else {
        genome.build_world().unwrap()
    };
    ...
    eprintln!(
        "Sample fitness: {:.2} (plant cells + food)",
...
fn run_stress(args: &[String]) {
    ...
    let mut seed = 42u64;
    let mut lite = true;
    let mut _output: Option<String> = None;
    let mut _all_scenarios = false;

    let mut i = 0;
    while i < args.len() {
        match args[i].as_str() {
            "--scenario" | "-s" => {
                scenario = args.get(i + 1).cloned();
                i += 1;
            }
            "--all" | "-a" => _all_scenarios = true,
            "--frames" | "-f" => {
                frames = args
                    .get(i + 1)
                    .and_then(|s| s.parse().ok())
...
fn run_analytics(args: &[String]) {
    ...
    eprintln!(
        "Population: {}  Generations: {}  Frames: {}  Seed: {}",
        population, generations, frames, seed
    );
...
fn run_drug(args: &[String]) {
    let mode = args
        ...
        .map(|s| s.as_str())
        .unwrap_or("compare");
    let switching_rate = args
        .iter()
        .position(|a| a == "--switching-rate")
        .and_then(|i| args.get(i + 1))
        .and_then(|s| s.parse().ok())
        .unwrap_or(1e-5);
    let kill_rate = args
        .iter()
        .position(|a| a == "--kill-rate")
        .and_then(|i| args.get(i + 1))
        .and_then(|s| s.parse().ok())
        .unwrap_or(3.0);
    ...
    eprintln!(
        "Mode: {}  Switching rate: {:.2e}  Kill rate: {:.1}",
        mode, switching_rate, kill_rate
    );
    ...
    for (i, r) in results.iter().enumerate() {
        eprintln!(
            "  Protocol {}: {:.1}% survival after {:.1}h",
            i,
            r.survival_fraction * 100.0,
            r.total_time_hours
        );
    }
}
...
fn run_gene(args: &[String]) {
    let mode = args
        ...
        .map(|s| s.as_str())
        .unwrap_or("design");
    let target_fano = args
        .iter()
        .position(|a| a == "--target-fano")
        .and_then(|i| args.get(i + 1))
        .and_then(|s| s.parse().ok())
        .unwrap_or(5.0);
    let target_mean = args
        .iter()
        .position(|a| a == "--target-mean")
        .and_then(|i| args.get(i + 1))
        .and_then(|s| s.parse().ok())
        .unwrap_or(100.0);
    ...
    eprintln!(
        "Mode: {}  Target Fano: {}  Target mean: {}",
        mode, target_fano, target_mean
    );
    ...
    eprintln!(
        "Optimized: Fano={:.2} Mean={:.1} CV={:.3}",
        result.achieved_fano, result.achieved_mean_protein, result.achieved_cv
    );
}
...
fn run_web(args: &[String]) {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--fps" => {
                fps = args.get(i + 1).and_then(|s| s.parse().ok()).unwrap_or(fps);
                i += 1;
            }
    ...
    tokio::runtime::Runtime::new()
        .expect("failed to create tokio runtime for terrarium web")
        .block_on(async move {
            {
            ...
            eprintln!(
                "Terrarium web server listening on http://localhost:{}",
                port
            );
...
fn print_web_help() {
    ...
    println!("  --fps <FPS>        Target frames per second (default: 16)");
...
fn run_zoom(args: &[String]) {
    ...
    for f in 0..max_frames {
        ...
        if f % 5 == 0 {
            eprintln!(
                "Frame {}: P:{} Fl:{} Fr:{} Cells:{:.0}",
                f, snap.plants, snap.flies, snap.fruits, snap.total_plant_cells
            );
        }
    }
}
...
fn run_run(args: &[String]) {
    ...
    for f in 0..frames {
        ...
        if f % (frames / 10).max(1) == 0 {
            let snap = world.snapshot();
            eprintln!(
                "Frame {}/{}: Plants={} Flies={} Cells={:.0}",
                f, frames, snap.plants, snap.flies, snap.total_plant_cells
            );
        }
    }
    ...
    eprintln!(
        "Final: Plants={} Flies={} Cells={:.0}",
        snap.plants, snap.flies, snap.total_plant_cells
    );
    eprintln!(
        "Time: {:.2}s ({:.1} fps)",
        elapsed.as_secs_f64(),
...
fn run_ecosystem(args: &[String]) {
    ...
    let mut ecosystem = match scenario.as_str() {
        "climate" | "climate_impact" => {
            oneura_core::ecosystem_integration::climate_impact_scenario(seed)
        }
        "amr" | "amr_emergence" => oneura_core::ecosystem_integration::amr_emergence_scenario(seed),
        "soil" | "soil_health" => oneura_core::ecosystem_integration::soil_health_scenario(seed),
        other => {
    ...
    if step_by_step {
        ...
        eprintln!("=== Simulation Complete ===");
        eprintln!(
            "Days simulated: {:.1}  Snapshots: {}  Time: {:.2}s",
            timeseries.total_days,
            timeseries.snapshots.len(),
            elapsed.as_secs_f64()
        );
        ...
        if let Some(last) = timeseries.snapshots.last() {
            ...
            eprintln!(
                "  Microbes:     {} taxa, Shannon={:.2}, Biomass={:.1}",
                last.microbial_richness, last.microbial_shannon, last.total_microbial_biomass
            );
            eprintln!(
                "  Resistance:   {} events, {} MDR strains, MIC={:.1}x",
                last.resistance_events, last.mdr_strain_count, last.mean_mic_fold
            );
            eprintln!(
                "  HGT:          freq={:.3}, fitness={:.3}",
                last.hgt_resistance_freq, last.hgt_mean_fitness
            );
            eprintln!(
                "  Biofilm:      {} cells, {:.1} biomass, EPS={:.1}%, QS={:.1}%",
                last.biofilm_cell_count,
                last.biofilm_biomass,
                last.eps_coverage * 100.0,
                last.quorum_activated * 100.0
            );
            eprintln!(
                "  Eco-evo:      fitness={:.3}, variance={:.3}, births={}, deaths={}",
                last.mean_fitness,
                last.genetic_variance_sum,
                last.eco_evo_births,
                last.eco_evo_deaths
            );
            eprintln!(
                "  Phylogenetics: {} nodes, diversity={:.2}, {} speciation events",
                last.phylo_tree_size, last.phylo_diversity, last.speciation_events
            );
            eprintln!(
                "  FBA:          growth={:.4} ({})",
                last.fba_growth_rate, last.fba_status
            );
        }
