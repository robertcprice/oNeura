...
fn quantize_channel(value: f32, levels: f32) -> f32 {
    let scaled = clamp(value, 0.0, 1.0) * (levels - 1.0);
    return round(scaled) / (levels - 1.0);
}

fn posterize_rgb(rgb: vec3<f32>, levels: f32, dither: f32) -> vec3<f32> {
    let nudged = clamp(rgb + (dither - 0.5) * 0.045, vec3<f32>(0.0), vec3<f32>(1.0));
    return vec3<f32>(
        quantize_channel(nudged.x, levels),
        quantize_channel(nudged.y, levels),
        quantize_channel(nudged.z, levels),
    );
}
...
fn fragment(
    ...
    let dither = fract(
        sin(dot(in.world_position.xz, vec2<f32>(12.9898, 78.233))) * 43758.5453
    );
    out.color.rgb = posterize_rgb(out.color.rgb, 6.0, dither);
