...
fn print_usage() {
    eprintln!(
        "Usage: cargo run --bin terrarium_gpu -- [--seed <n>] [--fps <n>] [--frames <n>] [--cell <px>] [--preset <demo|terrarium|aquarium>] [--cpu-substrate]"
    );
}
...
fn clear_color_for_preset(preset: TerrariumDemoPreset) -> Color {
    match preset {
        TerrariumDemoPreset::MicroAquarium => Color::rgb(0.02, 0.06, 0.08),
        TerrariumDemoPreset::MicroTerrarium => Color::rgb(0.05, 0.05, 0.04),
        TerrariumDemoPreset::Demo => Color::rgb(0.04, 0.05, 0.06),
    }
}
...
fn handle_input(
    ...
    if keyboard.just_pressed(KeyCode::R) {
        state.world = TerrariumWorld::demo_preset(state.seed, state.use_gpu_substrate, state.preset)
...
fn main() {
    ...
    let world = TerrariumWorld::demo_preset(cli.seed, !cli.cpu_substrate, cli.preset)
    ...
    App::new()
        .insert_resource(ClearColor(clear_color_for_preset(cli.preset)))
        ...
        .add_plugins(DefaultPlugins.set(ImagePlugin::default_nearest()).set(WindowPlugin {
            primary_window: Some(Window {
                title: format!("oNeura {} GPU", cli.preset.label()),
