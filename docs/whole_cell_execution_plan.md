...
- bundle-less runtime chemistry now also stays live without bundled assets when compiled registries or explicit runtime species and reactions are present: runtime-species sync no longer deletes explicit chemistry state just because asset bundles are absent, and registry-only bundle-less paths can now bootstrap runtime species and reactions on demand during execution
- bundle-less restore and execution now also synthesize explicit operon expression state directly from persisted runtime species and reactions when bundle descriptors are absent, so explicit runtime chemistry can keep driving expression instead of collapsing back to empty default summaries
- legacy saved-state parsing now promotes old core-summary payloads into explicit chromosome state, membrane state, and complex assembly when richer persisted layers are absent, so legacy restore enters the runtime with explicit biology instead of depending entirely on synchronized scalar summaries
- legacy saved-state parsing now also promotes missing multirate scheduler clocks from saved step and time counters, so older restore payloads can carry explicit stage history instead of forcing runtime restore to rebuild clocks from scratch
- legacy saved-state parsing now also promotes coarse aggregate assembly channels into explicit named-complex inventory, distributing through asset-backed complex specs when they exist and through generic family-level carriers when they do not, while bundle-less restore preserves any persisted aggregate channels that current named-complex semantics still cannot represent exactly
- legacy saved-state parsing now also promotes coarse local-chemistry support from saved core and lattice fields when older payloads never persisted chemistry reports, so compatibility restore can recover explicit chemistry support before runtime execution
- legacy saved-state parsing now also promotes coarse per-site local-chemistry detail into explicit subsystem site reports and subsystem coupling state, and restore now reapplies site chemistry into subsystem state when older payloads never serialized persistent subsystem couplings
- legacy chemistry promotion now also aggregates an explicit whole-cell chemistry report from persisted site chemistry before falling back to coarse core or lattice summaries, so compatibility restore uses explicit local chemistry whenever site reports were already serialized
- legacy saved-state parsing now also promotes missing scheduled subsystem probes and a coarse last MD-probe report from explicit chemistry-site and subsystem state, so older compatibility payloads regain explicit probe cadence and atomistic summary state before runtime restore
- legacy probe promotion now also feeds an explicit persisted `last_md_probe` back into subsystem-state scales and `last_probe_step`, so compatibility restore preserves probe-coupled subsystem state instead of keeping the probe only as a detached summary boundary payload
- legacy probe-schedule promotion now also preserves the explicit subsystem-probe subset already carried inside `local_chemistry.scheduled_subsystem_probes`, instead of inflating schedule recovery to an all-presets fallback when the saved payload already contains a narrower explicit schedule
- legacy probe-schedule promotion now also preserves explicit chemistry-site probe subsets and focuses coarse summary-only probe recovery onto the strongest recoverable subsystem target, so compatibility restore no longer turns every chemistry-only legacy payload into a blanket all-presets probe schedule
- legacy saved-state parsing now also promotes missing operon-level expression state from persisted runtime species and reactions, or from process-registry fallbacks plus promoted chemistry support when runtime chemistry is absent, so compatibility restore carries explicit transcription-unit activity instead of leaving expression empty until runtime bootstrap
- legacy expression promotion now also falls back to explicit operon, RNA, protein, and complex bundle assets when older payloads still carry structured organism assets but no runtime chemistry or process registry, so compatibility restore keeps using source-authoritative biology instead of collapsing to empty expression on asset-only paths
- legacy expression promotion now also falls back to explicit named-complex or aggregate assembly inventory when runtime chemistry, registries, and structured bundle assets are all absent, so compatibility restore still carries explicit operon-like activity from persisted assembly state instead of dropping back to an empty expression payload
- legacy expression promotion now also falls back through aggregate assembly reconstructed from coarse core summary channels when even explicit assembly inventory was never serialized, so the coarsest compatibility payloads still regain explicit operon-like expression state before runtime restore
- legacy compatibility now carries a direct explicit `DnaA` named-complex family instead of forcing that activity through the mixed replisome channel, so bundle-less named-complex aggregation can represent initiator activity without leaning on aggregate fallback totals
- bundle-less restore and program-spec bootstrap now also supplement missing direct named-complex carriers from explicit aggregate assembly channels when assets are absent, so partial compatibility payloads retain more assembly state as explicit per-complex carriers instead of merge-time aggregate fallback
...
- whole-cell runtime modularization has begun on the native path, with `oneuro-metal/src/whole_cell/local_chemistry.rs` and `oneuro-metal/src/whole_cell/spatial.rs` now owning the local-chemistry and spatial/locality subsystems instead of keeping those boundaries embedded inside `oneuro-metal/src/whole_cell.rs`
- whole-cell membrane and division mechanics now also live in `oneuro-metal/src/whole_cell/membrane.rs`, so envelope state updates and derived geometry accessors are no longer embedded inline in the main runtime file
- whole-cell chromosome and polymer-state mechanics now also live in `oneuro-metal/src/whole_cell/chromosome.rs`, so fork progression, chromosome-domain lookup, locus state, and chromosome-derived geometry support are no longer embedded inline in the main runtime file

What is still not at the target:

- the remaining compatibility-only summary payloads and legacy serialization bridges are now concentrated in the last non-explicit restore, snapshot, or migration paths around legacy-only probe or chemistry details that still survive only when no explicit site chemistry, richer persisted probe state, probe-coupled subsystem state, explicit local probe schedule, or explicit chemistry-site probe subset exists
- `oneuro-metal/src/whole_cell.rs` is still too large; assembly/runtime-chemistry execution and bootstrap/serialization boundaries still need to be split into subsystem modules so the runtime structure matches the biology layers
...
1. Continue splitting `oneuro-metal/src/whole_cell.rs` along authoritative biology/runtime boundaries so the runtime stops accumulating unrelated systems in one file. `local_chemistry.rs`, `spatial.rs`, `membrane.rs`, and `chromosome.rs` are now extracted; next modules are assembly and named-complex execution, runtime chemistry/process execution, and bootstrap/serialization boundaries.
2. Remove the remaining compatibility-only summary payload rollups and legacy serialization bridges, so synchronized scalar summaries become boundary serialization or diagnostics only and not live execution state even on compatibility paths without bundle assets or saved-state payloads. Program-spec bootstrap now preserves explicit spatial fields, local chemistry, expression, assembly, runtime chemistry, and scheduler state; bundle-less restore plus boundary getters now preserve explicit persisted chemistry, expression, registry, runtime chemistry, and named-complex state without bundled organism descriptors; bundle-less execution can now keep runtime species and reactions live from registries or explicit payloads and can synthesize operon expression directly from that runtime chemistry when explicit expression payloads are missing; and legacy saved-state parsing now upgrades old core summaries into explicit chromosome, membrane, complex, scheduler, named-complex, coarse chemistry, site-resolved subsystem chemistry state, scheduled subsystem probes, coarse MD-probe reports, direct `DnaA` carriers, and parser-boundary operon expression before runtime restore. When runtime chemistry and registries are both absent, that expression promotion now falls back first to explicit structured bundle assets, then to explicit named-complex or aggregate assembly inventory, and finally to coarse core summary channels reconstructed into explicit assembly carriers before giving up; missing coarse chemistry reports now aggregate from explicit site chemistry before core or lattice summaries are consulted; persisted `last_md_probe` output now promotes directly into explicit subsystem state; explicit local-chemistry schedules now survive parser recovery without being widened to an all-presets fallback; and explicit chemistry-site probe subsets now survive schedule recovery without being re-expanded through synthesized subsystem defaults. Bundle-less compatibility bootstrap and restore can now also synthesize missing direct named-complex carriers from explicit aggregate assembly channels, further shrinking aggregate fallback on asset-free paths. The remaining work is the narrower legacy compatibility surface around the last summary-only probe and chemistry outputs on the coarsest legacy payloads.
3. Replace any remaining uses of scalar-rule inventory priors in non-asset compatibility code with persisted explicit state or explicit local inventories wherever possible.
4. Expand compiled species/reaction coverage for metabolites, ions, cofactors, lipids, damage states, repair states, and membrane-local species beyond the current narrow bundle set.
5. Move more reaction execution from generic process scales into explicit registry-driven reaction families with direct ownership of pools, transcripts, proteins, complexes, and local fields.
6. Make chromosome-domain chemistry fully compiled and local, not just domain-weighted over generic nucleoid pools.
7. Make membrane patch reactions explicit: synthesis, insertion, remodeling, turnover, stress, constriction support, and pole/septum differentiation.
8. Deepen membrane/division mechanics so geometry, patch composition, divisome occupancy, and chromosome occlusion are driven by explicit local state instead of mixed summary signals.
9. Deepen chromosome mechanics toward polymer-level behavior: fork barriers, supercoiling, topological strain, macrodomain constraints, segregation forces, and locus-local transcription/replication conflicts.
10. Expand spatial chemistry so RDME fields are authoritative for more resource and damage flows, with fewer well-mixed fallback shortcuts.
11. Turn atomistic refinement from a side probe into a live service that extracts local systems from runtime state, runs calibrated local chemistry or MD, and writes validated corrections back into the cell state.
12. Add organism-calibration bundles and benchmark datasets for Syn3A covering growth, ATP usage, transcript/protein abundance, division timing, chromosome progression, membrane composition, and perturbation responses.
13. Build parity reporting against MC4D-style observables and close each mismatch by improving explicit state and reaction ownership, not by reintroducing heuristic shortcuts.
14. After parity, generalize the same compiler/runtime path to additional organisms and broaden atomistic coverage to the highest-value subsystems.
