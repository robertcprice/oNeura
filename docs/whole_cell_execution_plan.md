...
- the remaining compatibility-only summary payloads and legacy serialization bridges are now concentrated in the last non-explicit restore, snapshot, or migration paths that still lack richer persisted payloads for expression, local chemistry, or named-complex detail, so some synchronized scalar state still survives there as a fallback boundary format
