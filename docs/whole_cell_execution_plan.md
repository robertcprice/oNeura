...
- the remaining compatibility-only summary payloads and legacy restore or serialization bridges still preserve synchronized scalar state for non-explicit compatibility paths instead of deriving everything from explicit state at the boundary
...
1. Remove the remaining compatibility-only summary payload rollups and legacy boundary bridges, so synchronized scalar summaries become boundary serialization or diagnostics only and not live execution state even on restore or compatibility paths.
