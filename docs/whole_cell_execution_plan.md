...
- the remaining compatibility-only summary payloads and legacy serialization bridges are now concentrated in the last non-explicit restore, snapshot, or migration paths that still lack richer persisted payloads for expression or local chemistry, plus a few assembly channels that still need richer explicit named-complex carriers before persisted aggregate assembly can disappear entirely
