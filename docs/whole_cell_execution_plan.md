...
- the remaining compatibility-only summary payloads and legacy serialization bridges are now concentrated in the last non-explicit restore, snapshot, or migration paths that still lack richer persisted payloads for MD-probe and scheduled local-chemistry detail, a few remaining expression or assembly carriers, and the last aggregate assembly channels that still need direct named-complex representations before persisted summary rollups can disappear entirely
